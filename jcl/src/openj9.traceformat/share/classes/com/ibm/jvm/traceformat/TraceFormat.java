...
public class TraceFormat
{
	...
	private static Map<TraceThread, String> indentMap = new HashMap<>();
	...
	public static void main(String[] args) throws Exception {
		...
		List<RandomAccessFile> inputFiles = (List<RandomAccessFile>) ProgramOption.getValue("input_file");
		@SuppressWarnings("unchecked")
		List<File> messageFiles = (List<File>) ProgramOption.getValue("datfile");
		@SuppressWarnings("unchecked")
		List<Long> threads = (List<Long>) ProgramOption.getValue("threads");
		...
		try {
			while (true) {
				try {
					ByteBuffer data = inputFiles.get(0).getChannel().map(FileChannel.MapMode.READ_ONLY, 0, blockSize);
					context = TraceContext.getContext(data, messageFiles.get(0), System.out, warning, error, debug);
					break;
			...
			try {
				RandomAccessFile inputFile = inputFiles.get(0);
				...
				inputFile.seek(0);
				int i = inputFile.read(header);
				if (i != length || length >= blockSize) {
					throw new Exception("received premature end of file: " + e.getMessage());
				}

				context = TraceContext.getContext(header, header.length, messageFiles.get(0), System.out, warning, error, debug);
			} catch (Exception f) {
		...
		for (int i = 0; i < inputFiles.size(); i++) {
			...
			RandomAccessFile traceFile = inputFiles.get(i);
		...
		if (!summary.booleanValue()) {
			...
			output.println("                Trace Formatted Data " + System.lineSeparator());
		...
		String indent = "";
		for (Iterator<?> itr = context.getTracepoints(); itr.hasNext();) {
			...
			if (context.getTotalRecords() != recordsProcessed) {
				...
				if (processedMbytes % 10 == 0) {
					...
					if (verbose.booleanValue()) {
						...
						float Mbps = (processedMbytes) / (float) (((end - start) / 1000.0));
		...
		if (verbose.booleanValue()) {
			...
			float Mbps = recordsInData * context.getRecordSize() / (((float) (end - start) / 1000) * (1024 * 1024));
...
final class Debug extends ProgramOption {
	int debug;
	...
	void setValue(String value) throws IllegalArgumentException {
		...
		if (debug < 0) {
			throw new IllegalArgumentException("The value \"" + value + "\" specified for debug is not valid, must be a positive integer");
		}
	}
...
final class Verbose extends ProgramOption {
	boolean verbose;
...
final class Statistics extends ProgramOption {
	boolean statistics;

	@Override
	String getDescription() {
		return "If specified this will cause some statistics to be reported:" + System.lineSeparator()
				+ "  bytes of data by component" + System.lineSeparator()
				+ "  total bytes" + System.lineSeparator()
				+ "  hit counts by trace point" + System.lineSeparator()
...
final class MessageFile extends ProgramOption {
	List<File> messageFiles = new LinkedList<>();
...
final class Threads extends ProgramOption {
...
final class Timezone extends ProgramOption {
	Integer timezone;
...
final class Indent extends ProgramOption {
	boolean indent;
...
final class Summary extends ProgramOption {
	boolean summary;
...
final class InputFile extends ProgramOption {
	List<RandomAccessFile> inputFiles = new LinkedList<>();
	...
	void setDefault() {
		...
		setValue(ProgramOption.AnonymousArgs.get(0));
	}
...
final class FormatTimestamp extends ProgramOption {
...
final class OutputFile extends ProgramOption {
	...
	void setDefault() {
		...
		if (ProgramOption.AnonymousArgs.size() == 1) {
			...
			StringBuilder sb = new StringBuilder("Usage error: too many positional arguments:" + System.lineSeparator());
			for (int i = 2; i < ProgramOption.AnonymousArgs.size(); i++) {
				sb.append(ProgramOption.AnonymousArgs.get(i)).append(System.lineSeparator());
			}
			sb.append(System.lineSeparator() + help());
...
abstract class ProgramOption {
	...
	static final HashMap<String, ProgramOption> Configuration = new HashMap<>();
	static final List<String> AnonymousArgs = new LinkedList<>();

	/* The set of available options */
	static final HashMap<String, Class<? extends ProgramOption>> options = new HashMap<>();
	static HashMap<String, Class<? extends ProgramOption>> defaultOptions = null;
	...
	static Object getValue(String optionName) {
		return Configuration.get(optionName).getValue();
	}
	...
	static void addOption(Class<? extends ProgramOption> option) {
		try {
			ProgramOption tmp = option.newInstance();
	...
	static void addArgument(String arg) throws IllegalArgumentException {
		...
		if (defaultOptions == null) {
			defaultOptions = (HashMap<String, Class<? extends ProgramOption>>) options.clone();
		}
		...
		if (!options.containsKey(key)) {
			throw new IllegalArgumentException("Usage error: unknown argument, \"" + arg + "\"" + System.lineSeparator() + System.lineSeparator() + help());
		}

		/* Find the option */
		Class<? extends ProgramOption> type = options.get(key);
		try {
			ProgramOption option = type.newInstance();
			if (value != null) {
	...
	static void applyDefaults() {
		...
		for (Class<? extends ProgramOption> type : defaultOptions.values()) {
			try {
				ProgramOption option = type.newInstance();
				option.setDefault();
				Configuration.put(option.getName(), option);
	...
	public static String help() {
		StringBuilder help = new StringBuilder("Usage:" + System.lineSeparator());
		/*[IF Sidecar18-SE-OpenJ9]
		StringBuilder synopsis = new StringBuilder("traceformat");
		/*[ELSE]*/
		StringBuilder synopsis = new StringBuilder("com.ibm.jvm.TraceFormat");
		/*[ENDIF] Sidecar18-SE-OpenJ9 */
		StringBuilder sb = new StringBuilder("Option details:" + System.lineSeparator() + System.lineSeparator());

		for (Class<? extends ProgramOption> type : options.values()) {
			try {
				ProgramOption option = type.newInstance();
				...
				sb.append(name + ": " + option.getDescription() + System.lineSeparator() + System.lineSeparator());
			} catch (Exception e) {
				System.err.println("Error while constructing default values");
			}
		}

		return help.append(synopsis)
