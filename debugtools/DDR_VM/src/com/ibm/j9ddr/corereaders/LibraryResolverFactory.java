...
public class LibraryResolverFactory
{
	...
	public static ILibraryResolver getResolverForCoreFile(ImageInputStream stream) {
		...
		for (Class<? extends ILibraryResolver> resolverClass : resolverClasses) {
			try {
				...
				logger.logp(FINE, "LibraryResolverFactory", "<clinit>",
						"Skipping Library resolver class: {0} as it does not support an ImageInputStream constructor",
						new Object[] { resolverClass.getName() });
			} catch (Exception e) {
				logger.logp(FINE, "LibraryResolverFactory", "<clinit>",
						"Failed to create instance of Library resolver class: {0}",
						new Object[] { resolverClass.getName() });
			}
		}
	...
	public static ILibraryResolver getResolverForCoreFile(File file) {
		...
		for (Class<? extends ILibraryResolver> resolverClass : resolverClasses) {
			try {
				...
				logger.logp(FINE, "LibraryResolverFactory", "<clinit>",
						"Skipping Library resolver class: {0} as it does not support a File constructor",
						new Object[] { resolverClass.getName() });
			} catch (Exception e) {
				logger.logp(FINE, "LibraryResolverFactory", "<clinit>",
						"Failed to create instance of Library resolver class: {0}",
		...
		public LibraryDataSource getLibrary(String fileName, boolean silent) throws FileNotFoundException {
			...
			if (!silent) {
				logger.logp(FINER, "LibraryResolverFactory", "getLibrary",
						"Couldn't find shared library " + fileName + ". Some data may be unavailable.");
			}
	...
	private final static class LibraryPathResolver extends BaseResolver implements ILibraryResolver {
		...
		public LibraryDataSource getLibrary(String fileName, boolean resolver) throws FileNotFoundException {
			...
			logger.logp(FINER, "LibraryResolverFactory$LibraryPathResolver", "getLibrary",
					"Looking for {0} in library path.", strippedModuleName);

			for (File path : libraryPath) {
				if (path.isDirectory()) {
					logger.logp(FINEST, "LibraryResolverFactory$LibraryPathResolver", "getLibrary",
							"Looking in directory {0}.", path);
					...
					if (withDirectories.isFile()) {
						logger.logp(FINER, "LibraryResolverFactory$LibraryPathResolver", "getLibrary",
								"Found {0} in directory {1}.", new Object[] { strippedModuleName, path });
					...
					logger.logp(FINEST, "LibraryResolverFactory$LibraryPathResolver", "getLibrary",
							"Looking in file {0}.", path);
					File libraryFile = readArchive(path, fileName, strippedModuleName);

					if (libraryFile != null) {
						logger.logp(FINER, "LibraryResolverFactory$LibraryPathResolver", "getLibrary",
								"Found {0} in zip file {1}.", new Object[] { strippedModuleName, path });
			...
			logger.logp(FINER, "LibraryResolverFactory$LibraryPathResolver", "getLibrary",
					"{0} not found in library path.", strippedModuleName);
		...
		private File readArchive(File path, String filePath, String strippedModuleName) {
			try {
				...
				logger.logp(Level.WARNING, "LibraryResolverFactory$LibraryPathResolver", "readArchive",
						"Problems reading " + path + " as a zip file", e);
			}
			return null;
		}
		...
		private File extractEntry(ZipEntry thisEntry, String moduleName, ZipFile file) throws FileNotFoundException {
			try {
				...
				logger.logp(Level.WARNING, "LibraryResolverFactory$LibraryPathResolver", "extractEntry",
						"IOException trying to extract " + thisEntry.getName() + " from " + file.getName(), e);
				return null;
			}
		}
	}
	...
	private final static class NextToCoreResolver extends BaseResolver implements ILibraryResolver {
		...
		private static File findFile(File dir, String[] parts, int matchDepth) {
		...
		public LibraryDataSource getLibrary(String fileName, boolean silent) throws FileNotFoundException {
			...
			if (!isAbsolutePath(canonicalName)) {
				...
				if (separator != null) {
					...
					if (file != null) {
						...
						canonicalName = abspath.substring(coreDirectory.getAbsolutePath().length() + 1);
					}
				}
			}
			...
			for (final File checkPath : pathsToCheck) {
				if (checkPath.getParentFile() != null && checkPath.getParentFile().isDirectory()) {
					logger.logp(FINER, "LibraryResolverFactory$NextToCoreResolver", "getLibrary",
							"Trying {0}.", withoutDirectories);
	...
	private final static class InPlaceResolver extends BaseResolver implements ILibraryResolver {
		...
		public LibraryDataSource getLibrary(String fileName, boolean silent) throws FileNotFoundException {
			...
			logger.logp(FINER, "LibraryResolverFactory$InPlaceResolver", "getLibrary",
					"Looking for {0}.", library);
