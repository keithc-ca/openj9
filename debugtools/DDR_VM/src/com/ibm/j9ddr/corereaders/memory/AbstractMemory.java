...
public abstract class AbstractMemory extends SearchableMemory implements IMemory 
{
	...
	static {
		...
		logger.logp(FINE, "AbstractMemory", "<clinit>", "System property value from {0} was {1}",
				new Object[] { MAX_CORE_FILE_CACHE_SIZE_SYSTEM_PROPERTY, maxCoreFileCacheSize });
		...
		if ("true".equalsIgnoreCase(enableCacheStats)) {
	...
	public int getBytesAt(final long address, byte[] buffer, final int offset, final int length) throws MemoryFault {
		...
		if (range.contains(maxAddress)) {
			...
			while (Addresses.lessThanOrEqual(addressPointer, maxAddress)) {
				...
				long toRead = Addresses.greaterThan(topAddress, maxAddress)
							? maxAddress - addressPointer + 1
							: topAddress - addressPointer + 1;
	...
	public int getIntAt(long address) throws MemoryFault {
		// allocate 4 bytes, do the read, then retrieve via a ByteBuffer
	...
	public long getLongAt(long address) throws MemoryFault {
		// allocate 8 bytes, do the read, then retrieve via a ByteBuffer
	...
	public short getShortAt(long address) throws MemoryFault {
		// allocate 2 bytes, do the read, then retrieve via a ByteBuffer
	...
	{
		logger.logp(FINEST, "AbstractMemory", "addMemorySource", "Added memory range {0}-{1}",
				new Object[] { Long.toHexString(source.getBaseAddress()), Long.toHexString(source.getTopAddress()) });
	 ...
	 *
	 */
	private static final class CachingMemorySource extends DelegatingMemorySource {
		...
		public int getBytes(long address, byte[] buffer, int offset, int length) throws MemoryFault {
			if (singleBlockRange) {
				...
				if (singleBlockRef == null || (block = singleBlockRef.get()) == null) {
					...
					singleBlockRef = new WeakReference<>(block);
				} else {
					if (RECORDING_CACHE_STATS) {
						cacheHit = false;
					}
				...
				while ((toRead = length - read) > 0) {
					...
					int blockSize = (int) Math.min(sizeToEndOfRange, CACHE_BLOCK_SIZE);
					boolean cacheHit = false;
					...
					long amountToReadInBlock = Math.min(remainingInBlock, toRead);
		...
		public int hashCode() {
			...
			result = prime * result + Objects.hashCode(blockMap);
			result = prime * result + (singleBlockRange ? 1231 : 1237);
			result = prime * result + Objects.hashCode(singleBlockRef);
			return result;
		}
	...
	/*
	 * Dummy memory range used when caching is off, but recording cache states is on.
	 * Increments the cache counters without doing any caching.
	 */
	...
	private static final class CacheStatsReporter implements Runnable {
		...
		public void run() {
			...
			double tlbHitRate = ((double) MemorySourceTable.tlbCacheHits
					/ (MemorySourceTable.tlbCacheHits + MemorySourceTable.tlbCacheMisses)) * 100;
			...
			logger.logp(FINE, "AbstractMemory", "CacheStatsReporter",
					"Cache hits: {0}, cache misses: {1}, cache hit rate: {2}",
					new Object[] { cacheHits, cacheMisses, cacheHitRate });
			logger.logp(FINE, "AbstractMemory", "CacheStatsReporter",
					"Bytes read from disk: {0}, from cache: {1}, purged blocks: {2}, purged bytes: {3}",
					new Object[] { bytesReadFromDisk, bytesReadFromBlockCache, purgedBlocks, purgedBytes });
			logger.logp(FINE, "AbstractMemory", "CacheStatsReporter", "Cache bytes high water mark {0}",
					new Object[] { cacheByteHighWaterMark });
			logger.logp(FINE, "AbstractMemory", "CacheStatsReporter", "TLB Cache hits: {0}, misses: {1}, hit rate:{2}",
					new Object[] { MemorySourceTable.tlbCacheHits, MemorySourceTable.tlbCacheMisses, tlbHitRate });
		}
	}
