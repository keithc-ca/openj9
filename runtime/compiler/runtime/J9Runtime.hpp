...
/*
 * The offset macros below which use sizeof (which yields a value of type size_t)
 * first cast to int32_t so the final negation doesn't yield a large unsigned value
 * that is incompatible with smaller, signed types like int32_t.
 */
