...
static TR::DataType J9ToTRMap[] = { TR::NoType, TR::Int8, TR::Int8, TR::Int16, TR::Int16, TR::Float, TR::Int32,
    TR::Double, TR::Int64, TR::Address, TR::Address };
static bool J9ToIsUnsignedMap[] = { false, true, false, true, false, false, false, false, false, false, false };
static U_32 J9ToSizeMap[] = { 0, sizeof(UDATA), sizeof(UDATA), sizeof(UDATA), sizeof(UDATA), sizeof(UDATA),
    sizeof(UDATA), sizeof(U_64), sizeof(U_64), sizeof(UDATA) };
...
static TR::ILOpCodes J9ToTRIndirectCallMap[] = { TR::calli, TR::icalli, TR::icalli, TR::icalli, TR::icalli, TR::fcalli,
    TR::icalli, TR::dcalli, TR::lcalli, TR::acalli };
static TR::ILOpCodes J9ToTRReturnOpCodeMap[] = { TR::Return, TR::ireturn, TR::ireturn, TR::ireturn, TR::ireturn,
    TR::freturn, TR::ireturn, TR::dreturn, TR::lreturn, TR::areturn };
...
static inline bool utf8Matches(struct J9UTF8 *name1, struct J9UTF8 *name2)
{
    ...
    if (J9UTF8_LENGTH(name1) == J9UTF8_LENGTH(name2)
        && memcmp(utf8Data(name1), utf8Data(name2), J9UTF8_LENGTH(name1)) == 0)
        return true;
...
UDATA getFieldType(J9ROMConstantPoolItem *cp, I_32 cpIndex)
{
    ...
    switch (j9FieldType) { // cannot use characters because on s390 the compiler will generate an EBCDIC value
...
bool TR_J9VMBase::supportsFastNanoTime()
{
 ...
 */
U_32 TR_J9VMBase::offsetOfMethodIsBreakpointedBit() { return J9_STARTPC_METHOD_BREAKPOINTED; }

TR::Method *TR_J9VMBase::createMethod(TR_Memory *trMemory, TR_OpaqueClassBlock *clazz, int32_t refOffset)
{
    return new (trMemory->trHeapMemory())
        TR_J9Method(this, trMemory, TR::Compiler->cls.convertClassOffsetToClassPtr(clazz), refOffset);
}

TR_ResolvedMethod *TR_J9VMBase::createResolvedMethod(TR_Memory *trMemory, TR_OpaqueMethodBlock *aMethod,
...
TR_ResolvedMethod *TR_J9VMBase::createResolvedMethodWithVTableSlot(TR_Memory *trMemory, uint32_t vTableSlot,
    TR_OpaqueMethodBlock *aMethod, TR_ResolvedMethod *owningMethod, TR_OpaqueClassBlock *classForNewInstance)
{
    return createResolvedMethodWithSignature(trMemory, aMethod, classForNewInstance, NULL, -1, owningMethod,
        vTableSlot);
}

TR_ResolvedMethod *TR_J9VMBase::createResolvedMethodWithSignature(TR_Memory *trMemory, TR_OpaqueMethodBlock *aMethod,
    TR_OpaqueClassBlock *classForNewInstance, char *signature, int32_t signatureLength, TR_ResolvedMethod *owningMethod,
    uint32_t vTableSlot)
{
    ...
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
        || defined(TR_HOST_ARM64))
#if defined(J9VM_OPT_JITSERVER)
        if (_compInfoPT->getMethodBeingCompiled()->_useAOTCacheCompilation)
            result = new (trMemory->trHeapMemory())
                TR_ResolvedRelocatableJ9Method(aMethod, this, trMemory, owningMethod, vTableSlot);
        else
        ...
        if (classForNewInstance) {
            ...
            TR_ASSERT(result->isNewInstanceImplThunk(),
                "createResolvedMethodWithSignature: if classForNewInstance is given this must be a thunk");
        }
    }
...
void TR_J9MethodBase::parseSignature(TR_Memory *trMemory)
{
...
uintptr_t TR_J9MethodBase::j9returnType() { return _argTypes[_paramElements]; }

uint32_t TR_J9MethodBase::numberOfExplicitParameters()
{
...
//
TR::DataType TR_J9MethodBase::returnType() { return J9ToTRMap[j9returnType()]; }

bool TR_J9MethodBase::returnTypeIsUnsigned() { return J9ToIsUnsignedMap[j9returnType()]; }

// Returns the TR type of the parmNumber'th parameter
// NB: 0-based.  ie: (IC)V has parm0=I, parm1=C, etc..
//
TR::DataType TR_J9MethodBase::parmType(uint32_t parmNumber) { return J9ToTRMap[_argTypes[parmNumber]]; }

// returns the opcode for a direct call version of the method
//
TR::ILOpCodes TR_J9MethodBase::directCallOpCode() { return J9ToTRDirectCallMap[j9returnType()]; }

// returns the opcode for an indirect call version of the method
//
TR::ILOpCodes TR_J9MethodBase::indirectCallOpCode() { return J9ToTRIndirectCallMap[j9returnType()]; }

// returns the return opcode for the method
//
TR::ILOpCodes TR_J9MethodBase::returnOpCode() { return J9ToTRReturnOpCodeMap[j9returnType()]; }

// Returns the width of the return type of the method
//
U_32 TR_J9MethodBase::returnTypeWidth() { return J9ToSizeMap[j9returnType()]; }

// returns the length of the class that this method is in.
//
U_16 TR_J9MethodBase::classNameLength() { return J9UTF8_LENGTH(_className); }

// returns the length of the method name
//
U_16 TR_J9MethodBase::nameLength() { return J9UTF8_LENGTH(_name); }

const char *TR_J9MethodBase::signature(TR_Memory *trMemory, TR_AllocationKind allocKind)
{
    if (!_fullSignature) {
        size_t len = classNameLength() + nameLength() + signatureLength() + 3;
        char *s = (char *)trMemory->allocateMemory(len, allocKind);
        snprintf(s, len, "%.*s.%.*s%.*s", classNameLength(), classNameChars(), nameLength(), nameChars(),
...
U_16 TR_J9MethodBase::signatureLength() { return J9UTF8_LENGTH(_signature); }

// returns the char * pointer to the utf8 encoded name of the class the method is in
//
char *TR_J9MethodBase::classNameChars() { return utf8Data(_className); }

// returns the char * pointer to the utf8 encoded method name
//
char *TR_J9MethodBase::nameChars() { return utf8Data(_name); }

// returns the char * pointer to the utf8 encoded method signature
//
char *TR_J9MethodBase::signatureChars() { return utf8Data(_signature); }

bool TR_J9MethodBase::isConstructor() { return nameLength() == 6 && !strncmp(nameChars(), "<init>", 6); }

bool TR_J9MethodBase::isFinalInObject()
{
    return methodIsFinalInObject(nameLength(), (U_8 *)nameChars(), signatureLength(), (U_8 *)signatureChars()) ? true
                                                                                                               : false;
}
...
J9ROMConstantPoolItem *TR_ResolvedJ9MethodBase::romLiterals() { return _romLiterals; }

TR_J9VMBase *TR_ResolvedJ9MethodBase::fej9() { return (TR_J9VMBase *)_fe; }

TR_FrontEnd *TR_ResolvedJ9MethodBase::fe() { return _fe; }

TR_ResolvedMethod *TR_ResolvedJ9MethodBase::owningMethod() { return _owningMethod; }

void TR_ResolvedJ9MethodBase::setOwningMethod(TR_ResolvedMethod *parent) { _owningMethod = parent; }

bool TR_ResolvedJ9Method::owningMethodDoesntMatter()
{
    ...
    if (aggressiveJSR292Opts && strchr(aggressiveJSR292Opts, '3')) {
        if (J9UTF8_LENGTH(className) >= 17 && !strncmp((char *)J9UTF8_DATA(className), "java/lang/invoke/", 17)) {
            ...
            switch (TR_ResolvedJ9MethodBase::getRecognizedMethod()) {
                case TR::java_lang_invoke_MethodHandle_invokeExactTargetAddress: // This is just a getter that's
                                                                                 // practically always inlined
                    return true;
                default:
                    break;
            }
...
bool TR_ResolvedJ9MethodBase::canAlwaysShareSymbolDespiteOwningMethod(TR_ResolvedMethod *otherMethod)
{
...
J9ROMFieldRef *TR_ResolvedJ9MethodBase::romCPBase() { return (J9ROMFieldRef *)romLiterals(); }

char *TR_ResolvedJ9MethodBase::classSignatureOfFieldOrStatic(I_32 cpIndex, int32_t &len)
{
    if (cpIndex == -1)
        return 0;
...
char *TR_ResolvedJ9MethodBase::classNameOfFieldOrStatic(I_32 cpIndex, int32_t &len)
{
    if (cpIndex == -1)
        return 0;
...
int32_t TR_ResolvedJ9MethodBase::classCPIndexOfFieldOrStatic(int32_t cpIndex)
{
    if (cpIndex == -1)
        return -1;
...
TR_OpaqueClassBlock *TR_ResolvedJ9MethodBase::getDeclaringClassFromFieldOrStatic(TR::Compilation *comp, int32_t cpIndex)
{
...

    {
        ...
        int32_t fieldLen;
        char *field = fieldNameChars(cpIndex, fieldLen);
        int32_t sigLen;
        char *sig = fieldSignatureChars(cpIndex, sigLen);
        vmThread->javaVM->internalVMFunctions->instanceFieldOffset(vmThread, cpClass, (U_8 *)field, fieldLen,
            (U_8 *)sig, sigLen, &containingClass, NULL, J9_LOOK_NO_JAVA);
    }
...
void TR_J9MethodBase::setSignature(char *newSignature, int32_t newSignatureLength, TR_Memory *trMemory)
{
...
void TR_J9MethodBase::setArchetypeSpecimen(bool b) { _flags.set(ArchetypeSpecimen, b); }

char *TR_ResolvedJ9Method::localName(U_32 slotNumber, U_32 bcIndex, TR_Memory *trMemory)
{
    I_32 len;
    return localName(slotNumber, bcIndex, len, trMemory);
}

char *TR_ResolvedJ9Method::localName(U_32 slotNumber, U_32 bcIndex, I_32 &len, TR_Memory *trMemory)
{
    ...
    while (NULL != values) {
        ...
        if (values->slotNumber == slotNumber
            //&& variableInfo.startVisibility <= bcIndex && bcIndex <= (variableInfo.startVisibility +
            // variableInfo.visibilityLength)
        ) {
...
char *TR_ResolvedJ9MethodBase::fieldOrStaticName(I_32 cpIndex, int32_t &len, TR_Memory *trMemory,
    TR_AllocationKind kind)
{
    ...
    J9ROMNameAndSignature *nameAndSignature = J9ROMFIELDREF_NAMEANDSIGNATURE(ref);
    J9UTF8 *declName = J9ROMCLASSREF_NAME((J9ROMClassRef *)(&romCPBase()[ref->classRefCPIndex]));
    len = J9UTF8_LENGTH(declName) + J9UTF8_LENGTH(J9ROMNAMEANDSIGNATURE_NAME(nameAndSignature))
        + J9UTF8_LENGTH(J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSignature)) + 3;

    char *s = (char *)trMemory->allocateMemory(len, kind);
    snprintf(s, len, "%.*s.%.*s %.*s", J9UTF8_LENGTH(declName), utf8Data(declName),
        J9UTF8_LENGTH(J9ROMNAMEANDSIGNATURE_NAME(nameAndSignature)),
        utf8Data(J9ROMNAMEANDSIGNATURE_NAME(nameAndSignature)),
        J9UTF8_LENGTH(J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSignature)),
        utf8Data(J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSignature)));
    return s;
}

char *TR_ResolvedJ9MethodBase::fieldName(I_32 cpIndex, TR_Memory *m, TR_AllocationKind kind)
{
    int32_t len;
    return fieldName(cpIndex, len, m, kind);
}

char *TR_ResolvedJ9MethodBase::staticName(I_32 cpIndex, TR_Memory *m, TR_AllocationKind kind)
{
    int32_t len;
    return staticName(cpIndex, len, m, kind);
}

char *TR_ResolvedJ9MethodBase::fieldName(I_32 cpIndex, int32_t &len, TR_Memory *m, TR_AllocationKind kind)
{
    if (cpIndex < 0)
        return (char *)"<internal field>";
    return fieldOrStaticName(cpIndex, len, m, kind);
}

char *TR_ResolvedJ9MethodBase::staticName(I_32 cpIndex, int32_t &len, TR_Memory *m, TR_AllocationKind kind)
{
    if (cpIndex < 0)
        return 0;
    return fieldOrStaticName(cpIndex, len, m, kind);
...
I_32 TR_ResolvedJ9MethodBase::exceptionData(J9ExceptionHandler *eh, I_32 bcOffset, I_32 exceptionNumber,
    I_32 *startIndex, I_32 *endIndex, I_32 *catchType)
{
...
static const char * const excludeArray[] = {
    ...
    "java/security/AccessController.doPrivileged(Ljava/security/PrivilegedAction;Ljava/security/"
    "AccessControlContext;)Ljava/lang/Object;",
    "java/security/AccessController.doPrivileged(Ljava/security/PrivilegedExceptionAction;Ljava/security/"
    "AccessControlContext;)Ljava/lang/Object;",
    "java/security/AccessController.doPrivileged(Ljava/security/PrivilegedAction;Ljava/security/"
    "AccessControlContext;[Ljava/security/Permission;)Ljava/lang/Object;",
    "java/security/AccessController.doPrivileged(Ljava/security/PrivilegedExceptionAction;Ljava/security/"
    "AccessControlContext;[Ljava/security/Permission;)Ljava/lang/Object;",
#endif /* JAVA_SPEC_VERSION < 24 */
    "java/lang/NullPointerException.fillInStackTrace()Ljava/lang/Throwable;",
#if (17 <= JAVA_SPEC_VERSION) && (JAVA_SPEC_VERSION <= 18)
    "jdk/internal/loader/NativeLibraries.load(Ljdk/internal/loader/NativeLibraries$NativeLibraryImpl;Ljava/lang/"
    "String;ZZZ)Z",
...
bool TR_ResolvedJ9MethodBase::isCompilable(TR_Memory *trMemory)
{
...
bool TR_ResolvedJ9MethodBase::isInlineable(TR::Compilation *comp)
{
        ...
        if (!comp->fej9()->sharedCache()->isClassInSharedCache(ramClass)) {
            ...
            if (!startupTimeMatters) {
                ...
                if (useHigherMethodCounts) {
                    ...
                    if (J9UTF8_LENGTH(className) > 5 && !strncmp(utf8Data(className), "java/", 5)) {
                        initialCount = 10000;
                    } else {
                        if (J9ROMMETHOD_HAS_BACKWARDS_BRANCHES(romMethod))
...
bool TR_ResolvedJ9MethodBase::isCold(TR::Compilation *comp, bool isIndirectCall, TR::ResolvedMethodSymbol *sym)
{
    ...
    if (((float)count) / initialCount >= .95) {
        ...
        if ((count < 0) || ((((float)count) / initialCount) < .50)) {
...
TR::SymbolReferenceTable *TR_ResolvedJ9MethodBase::_genMethodILForPeeking(TR::ResolvedMethodSymbol *methodSymbol,
    TR::Compilation *c, bool resetVisitCount, TR_PrexArgInfo *argInfo)
{
...
TR_ResolvedRelocatableJ9Method::TR_ResolvedRelocatableJ9Method(TR_OpaqueMethodBlock *aMethod, TR_FrontEnd *fe,
    TR_Memory *trMemory, TR_ResolvedMethod *owner, uint32_t vTableSlot)
        ...
        if (comp && this->TR_ResolvedMethod::getRecognizedMethod() != TR::unknownMethod) {
            if (TR_SharedCache::INVALID_CLASS_CHAIN_OFFSET != fej9->sharedCache()->rememberClass(containingClass())) {
                if (comp->getOption(TR_UseSymbolValidationManager)) {
                    ...
                    // JITServer: in baseline, if owner doesn't exist then comp doesn't exist, so thi case is not
                    // possible but in JITClient comp is initialized before creating resolved method for compilee, so
                    // need this guard.
                    ((TR_ResolvedRelocatableJ9Method *)owner)
                        ->validateArbitraryClass(comp, (J9Class *)containingClass());
                }
            } else {
...
bool TR_ResolvedRelocatableJ9Method::unresolvedFieldAttributes(I_32 cpIndex, TR::DataType *type, bool *volatileP,
    bool *isFinal, bool *isPrivate)
{
    U_32 ltype;
...
bool TR_ResolvedRelocatableJ9Method::unresolvedStaticAttributes(I_32 cpIndex, TR::DataType *type, bool *volatileP,
    bool *isFinal, bool *isPrivate)
{
    U_32 ltype;
    I_32 myVolatile, myFinal, myPrivate;
...
void TR_ResolvedRelocatableJ9Method::setAttributeResult(bool isStaticField, bool result, UDATA ltype, I_32 myVolatile,
    I_32 myFinal, I_32 myPrivate, TR::DataType *type, bool *volatileP, bool *isFinal, bool *isPrivate,
    void **fieldOffset)
{
    if (result) {
        *volatileP = myVolatile ? true : false;
        if (isFinal)
            *isFinal = myFinal ? true : false;
        if (isPrivate)
            *isPrivate = myPrivate ? true : false;
    } else {
        *volatileP = true;
        if (fieldOffset) {
            if (isStaticField)
...
bool TR_ResolvedRelocatableJ9Method::hasBackwardBranches()
{
    return J9ROMMETHOD_HAS_BACKWARDS_BRANCHES(romMethod()) ? true : false;
}

bool TR_ResolvedRelocatableJ9Method::isObjectConstructor()
{
    return J9ROMMETHOD_IS_OBJECT_CONSTRUCTOR(romMethod()) ? true : false;
}

bool TR_ResolvedRelocatableJ9Method::isNonEmptyObjectConstructor()
{
    return J9ROMMETHOD_IS_NON_EMPTY_OBJECT_CONSTRUCTOR(romMethod()) ? true : false;
}
...
bool TR_ResolvedRelocatableJ9Method::isFinal()
{
    return (methodModifiers() & J9AccFinal) || (classModifiers() & J9AccFinal) ? true : false;
}

bool TR_ResolvedRelocatableJ9Method::isInterpreted()
{
    ...
    */
    if (isJNINative() && !TR::Options::getAOTCmdLineOptions()->getOption(TR_DisableDirectToJNI)
...
bool TR_ResolvedRelocatableJ9Method::isInterpretedForHeuristics() { return TR_ResolvedJ9Method::isInterpreted(); }

bool TR_ResolvedRelocatableJ9Method::isCompilable(TR_Memory *trMemory)
{
...
void *TR_ResolvedRelocatableJ9Method::startAddressForJittedMethod() { return NULL; }

void *TR_ResolvedRelocatableJ9Method::startAddressForJNIMethod(TR::Compilation *comp)
{
...
void *TR_ResolvedRelocatableJ9Method::startAddressForJITInternalNativeMethod() { return 0; }

void *TR_ResolvedRelocatableJ9Method::startAddressForInterpreterOfJittedMethod()
{
    return ((J9Method *)getNonPersistentIdentifier())->extra;
}

void *TR_ResolvedRelocatableJ9Method::constantPool() { return romLiterals(); }

TR_OpaqueClassBlock *TR_ResolvedRelocatableJ9Method::getClassFromConstantPool(TR::Compilation *comp, uint32_t cpIndex,
    bool returnClassForAOT)
{
...
bool TR_ResolvedRelocatableJ9Method::validateClassFromConstantPool(TR::Compilation *comp, J9Class *clazz,
    uint32_t cpIndex, TR_ExternalRelocationTargetKind reloKind)
{
    if (comp->getOption(TR_UseSymbolValidationManager)) {
        return comp->getSymbolValidationManager()->addClassFromCPRecord(reinterpret_cast<TR_OpaqueClassBlock *>(clazz),
            cp(), cpIndex);
    } else {
...
bool TR_ResolvedRelocatableJ9Method::validateArbitraryClass(TR::Compilation *comp, J9Class *clazz)
{
    return storeValidationRecordIfNecessary(comp, cp(), 0, TR_ValidateArbitraryClass, ramMethod(), clazz);
}

static TR::DataType cpType2trType(UDATA cpType)
{
    switch (cpType) {
...
void *TR_ResolvedRelocatableJ9Method::stringConstant(I_32 cpIndex)
{
...
bool TR_ResolvedRelocatableJ9Method::isUnresolvedString(I_32 cpIndex, bool optimizeForAOT)
{
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
...
void *TR_ResolvedRelocatableJ9Method::methodTypeConstant(I_32 cpIndex)
{
    TR_ASSERT(false, "should be unreachable");
    return NULL;
}

bool TR_ResolvedRelocatableJ9Method::isUnresolvedMethodType(I_32 cpIndex)
{
    TR_ASSERT(false, "should be unreachable");
    return true;
}

void *TR_ResolvedRelocatableJ9Method::methodHandleConstant(I_32 cpIndex)
{
    TR_ASSERT(false, "should be unreachable");
    return NULL;
}

bool TR_ResolvedRelocatableJ9Method::isUnresolvedMethodHandle(I_32 cpIndex)
{
    TR_ASSERT(false, "should be unreachable");
    return true;
}

bool TR_ResolvedRelocatableJ9Method::isUnresolvedCallSiteTableEntry(int32_t callSiteIndex)
{
...
bool TR_ResolvedRelocatableJ9Method::isUnresolvedMethodTypeTableEntry(int32_t cpIndex)
{
    bool unresolved = true;
    J9JavaVM *javaVM = fej9()->_jitConfig->javaVM;
    if (J9_ARE_ALL_BITS_SET(javaVM->sharedClassConfig->runtimeFlags2, J9SHR_RUNTIMEFLAG2_SHARE_LAMBDAFORM)) {
...
TR_ResolvedMethod *TR_ResolvedRelocatableJ9Method::getResolvedPossiblyPrivateVirtualMethod(TR::Compilation *comp,
    int32_t cpIndex, bool ignoreRtResolve, bool *unresolvedInCP)
{
    TR_ResolvedMethod *method
        = TR_ResolvedJ9Method::getResolvedPossiblyPrivateVirtualMethod(comp, cpIndex, ignoreRtResolve, unresolvedInCP);

    return aotMaskResolvedPossiblyPrivateVirtualMethod(comp, method);
}

TR_ResolvedMethod *TR_ResolvedJ9Method::aotMaskResolvedPossiblyPrivateVirtualMethod(TR::Compilation *comp,
    TR_ResolvedMethod *method)
{
    if (method == NULL || !method->isPrivate() || comp->fej9()->isResolvedDirectDispatchGuaranteed(comp))
        return method;
...
bool TR_ResolvedRelocatableJ9Method::getUnresolvedFieldInCP(I_32 cpIndex)
{
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
#if defined(J9VM_INTERP_AOT_COMPILE_SUPPORT) && defined(J9VM_OPT_SHARED_CLASSES)                        \
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
...
bool TR_ResolvedRelocatableJ9Method::storeValidationRecordIfNecessary(TR::Compilation *comp,
    J9ConstantPool *constantPool, int32_t cpIndex, TR_ExternalRelocationTargetKind reloKind, J9Method *ramMethod,
    J9Class *definingClass)
{
...
#if defined(J9VM_OPT_JITSERVER)
    // If this compilation is tied to a remote compilation that is ignoring the client's SCC, none of the below is
    // necessary, as all of it will be done at the server.
    ...
    if (trace) {
        ...
        log->printf("\tmethod %p from class %p %.*s\n", ramMethod, J9_CLASS_FROM_METHOD(ramMethod),
            J9UTF8_LENGTH(methodClassName), J9UTF8_DATA(methodClassName));
        log->printf("\tdefiningClass %p\n", definingClass);
    }

    if (!definingClass) {
        definingClass
            = (J9Class *)TR_ResolvedJ9Method::definingClassFromCPFieldRef(comp, constantPool, cpIndex, isStatic);
    ...
    if (!aotClassInfo->empty()) {
        for (auto info = aotClassInfo->begin(); info != aotClassInfo->end(); ++info) {
            TR_ASSERT(((*info)->_reloKind == TR_ValidateInstanceField || (*info)->_reloKind == TR_ValidateStaticField
                          || (*info)->_reloKind == TR_ValidateClass || (*info)->_reloKind == TR_ValidateArbitraryClass),
                "TR::AOTClassInfo reloKind is not TR_ValidateInstanceField or TR_ValidateStaticField or "
                "TR_ValidateClass!");
            ...
            if ((*info)->_reloKind == reloKind) {
                if (isStatic)
                    ...
                    inLocalList = (classChainOffset == (*info)->_classChainOffset && cpIndex == (*info)->_cpIndex
    ...
    TR::AOTClassInfo *classInfo = new (comp->trHeapMemory()) TR::AOTClassInfo(fej9,
        (TR_OpaqueClassBlock *)definingClass, classChainOffset, (TR_OpaqueMethodBlock *)ramMethod, cpIndex, reloKind);
    if (classInfo) {
...
bool TR_ResolvedRelocatableJ9Method::fieldAttributes(TR::Compilation *comp, int32_t cpIndex, uint32_t *fieldOffset,
    TR::DataType *type, bool *volatileP, bool *isFinal, bool *isPrivate, bool isStore, bool *unresolvedInCP,
    bool needAOTValidation)
{
            ...
            if (needAOTValidation) {
                if (comp->getOption(TR_UseSymbolValidationManager)) {
                    TR_OpaqueClassBlock *clazz
                        = TR_ResolvedJ9Method::definingClassFromCPFieldRef(comp, constantPool, cpIndex, false);

                    fieldInfoCanBeUsed = comp->getSymbolValidationManager()->addDefiningClassFromCPRecord(clazz,
                        constantPool, cpIndex);
                } else {
                    fieldInfoCanBeUsed = storeValidationRecordIfNecessary(comp, constantPool, cpIndex,
                        TR_ValidateInstanceField, ramMethod());
                }
            } else {
    ...
    setAttributeResult(false, theFieldIsFromLocalClass, ltype, volatileFlag, finalFlag, privateFlag, type, volatileP,
        isFinal, isPrivate, (void **)fieldOffset);

    return theFieldIsFromLocalClass;
}

bool TR_ResolvedRelocatableJ9Method::staticAttributes(TR::Compilation *comp, int32_t cpIndex, void **address,
    TR::DataType *type, bool *volatileP, bool *isFinal, bool *isPrivate, bool isStore, bool *unresolvedInCP,
    bool needAOTValidation)
{
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
    ...
    if (needAOTValidation) {
        if (comp->getOption(TR_UseSymbolValidationManager)) {
            TR_OpaqueClassBlock *clazz
                = TR_ResolvedJ9Method::definingClassFromCPFieldRef(comp, constantPool, cpIndex, true);

            fieldInfoCanBeUsed
                = comp->getSymbolValidationManager()->addDefiningClassFromCPRecord(clazz, constantPool, cpIndex, true);
        } else {
            fieldInfoCanBeUsed
                = storeValidationRecordIfNecessary(comp, constantPool, cpIndex, TR_ValidateStaticField, ramMethod());
        }
    } else {
        fieldInfoCanBeUsed = true;
    }
    ...
    setAttributeResult(true, theFieldIsFromLocalClass, ltype, volatileFlag, finalFlag, privateFlag, type, volatileP,
        isFinal, isPrivate, address);

    return theFieldIsFromLocalClass;
}

TR_OpaqueClassBlock *TR_ResolvedRelocatableJ9Method::definingClassFromCPFieldRef(TR::Compilation *comp, I_32 cpIndex,
    bool isStatic, TR_OpaqueClassBlock **fromResolvedJ9Method)
{
    ...
    if (comp->getOption(TR_UseSymbolValidationManager))
        ...
        valid = storeValidationRecordIfNecessary(comp, cp(), cpIndex,
            isStatic ? TR_ValidateStaticField : TR_ValidateInstanceField, ramMethod());
...
char *TR_ResolvedRelocatableJ9Method::fieldSignatureChars(int32_t cpIndex, int32_t &len)
{
    return cpIndex > 0 ? fieldOrStaticSignatureChars(cpIndex, len) : 0;
}

char *TR_ResolvedRelocatableJ9Method::staticSignatureChars(int32_t cpIndex, int32_t &len)
{
    return cpIndex >= 0 ? fieldOrStaticSignatureChars(cpIndex, len) : 0;
}

TR_OpaqueClassBlock *TR_ResolvedRelocatableJ9Method::classOfStatic(int32_t cpIndex, bool returnClassForAOT)
{
...
void TR_ResolvedRelocatableJ9Method::handleUnresolvedStaticMethodInCP(int32_t cpIndex, bool *unresolvedInCP)
{
    *unresolvedInCP = getUnresolvedStaticMethodInCP(cpIndex);
}

void TR_ResolvedRelocatableJ9Method::handleUnresolvedSpecialMethodInCP(int32_t cpIndex, bool *unresolvedInCP)
{
    *unresolvedInCP = getUnresolvedSpecialMethodInCP(cpIndex);
}

void TR_ResolvedRelocatableJ9Method::handleUnresolvedVirtualMethodInCP(int32_t cpIndex, bool *unresolvedInCP)
{
    *unresolvedInCP = getUnresolvedVirtualMethodInCP(cpIndex);
}

bool TR_ResolvedRelocatableJ9Method::getUnresolvedMethodInCP(TR_OpaqueMethodBlock *aMethod)
{
    J9Method *ramMethod = (J9Method *)aMethod;
    return !ramMethod || !J9_BYTECODE_START_FROM_RAM_METHOD(ramMethod);
}

bool TR_ResolvedRelocatableJ9Method::getUnresolvedStaticMethodInCP(I_32 cpIndex)
{
...

    {
        ...
        method = jitResolveStaticMethodRef(_fe->vmThread(), (J9ConstantPool *)(J9_CP_FROM_METHOD(ramMethod())), cpIndex,
...
bool TR_ResolvedRelocatableJ9Method::getUnresolvedSpecialMethodInCP(I_32 cpIndex)
{
    J9Method *method = NULL;
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");

    {
        ...
        method = jitResolveSpecialMethodRef(_fe->vmThread(), (J9ConstantPool *)(J9_CP_FROM_METHOD(ramMethod())),
            cpIndex, J9_RESOLVE_FLAG_JIT_COMPILE_TIME);
    }

    return getUnresolvedMethodInCP((TR_OpaqueMethodBlock *)method);
}

bool TR_ResolvedRelocatableJ9Method::getUnresolvedVirtualMethodInCP(I_32 cpIndex)
{
    /*
       TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
       J9Method *ramMethod;
       I_32 rc=_fe->_vmFunctionTable->resolveVirtualMethodRef(_fe->vmThread(), (J9ConstantPool
       *)(J9_CP_FROM_METHOD((((AOTSharedCookie *)_methodCookie)->ramMethod))), cpIndex,
       J9_RESOLVE_FLAG_JIT_COMPILE_TIME, &ramMethod); if (!rc) return false;
...
TR_ResolvedMethod *TR_ResolvedRelocatableJ9Method::getResolvedImproperInterfaceMethod(TR::Compilation *comp,
    I_32 cpIndex)
{
    return aotMaskResolvedImproperInterfaceMethod(comp,
        TR_ResolvedJ9Method::getResolvedImproperInterfaceMethod(comp, cpIndex));
}

TR_ResolvedMethod *TR_ResolvedJ9Method::aotMaskResolvedImproperInterfaceMethod(TR::Compilation *comp,
    TR_ResolvedMethod *method)
{
    if (method == NULL)
        return NULL;
...
TR_ResolvedMethod *TR_ResolvedRelocatableJ9Method::createResolvedMethodFromJ9Method(TR::Compilation *comp, I_32 cpIndex,
    uint32_t vTableSlot, J9Method *j9method, TR_AOTInliningStats *aotStats)
{
...

    {
        ...
        J9Class *j9clazz
            = (J9Class *)J9_CLASS_FROM_CP(((J9RAMConstantPoolItem *)J9_CP_FROM_METHOD(((J9Method *)j9method))));
        TR_OpaqueClassBlock *clazzOfInlinedMethod = _fe->convertClassPtrToClassOffset(j9clazz);
        TR_OpaqueClassBlock *clazzOfCompiledMethod
            = _fe->convertClassPtrToClassOffset(J9_CLASS_FROM_METHOD(ramMethod()));

        if (enableAggressive) {
            isSystemClassLoader = ((void *)_fe->vmThread()->javaVM->systemClassLoader->classLoaderObject
                == (void *)_fe->getClassLoader(clazzOfInlinedMethod));
        }
        ...
        if (methodInSCC) {
            ...
            TR_J9VMBase *fej9 = (TR_J9VMBase *)_fe;
            if (resolveAOTMethods || (sameLoaders = fej9->sameClassLoaders(clazzOfInlinedMethod, clazzOfCompiledMethod))
                || isSystemClassLoader) {
                resolvedMethod = new (comp->trHeapMemory()) TR_ResolvedRelocatableJ9Method(
                    (TR_OpaqueMethodBlock *)j9method, _fe, comp->trMemory(), this, vTableSlot);
                if (!ignoringLocalSCC && comp->getOption(TR_UseSymbolValidationManager)) {
                    ...
                    if (_fe->convertClassPtrToClassOffset(J9_CLASS_FROM_METHOD(ramMethod()))
                        == _fe->convertClassPtrToClassOffset(J9_CLASS_FROM_METHOD(j9method)))
    ...
    if (resolvedMethod && ((TR_ResolvedJ9Method *)resolvedMethod)->isSignaturePolymorphicMethod()) {
        // Signature polymorphic method's signature varies at different call sites and will be different than its
        // declared signature
...
U_8 *TR_ResolvedRelocatableJ9Method::allocateException(uint32_t numBytes, TR::Compilation *comp)
{
    J9JITExceptionTable *eTbl = NULL;
    uint32_t size = 0;
    bool shouldRetryAllocation;
    eTbl = (J9JITExceptionTable *)_fe->allocateDataCacheRecord(numBytes, comp,
        _fe->needsContiguousCodeAndDataCacheAllocation(), &shouldRetryAllocation, J9_JIT_DCE_EXCEPTION_INFO, &size);
    if (!eTbl) {
...
TR_OpaqueMethodBlock *TR_ResolvedRelocatableJ9Method::getNonPersistentIdentifier()
{
    ...
    , _pendingPushSlots(-1)
{
    ...
    if (TR_J9VMBase::supportsFastJNI()) {
        ...
        if (TR::Options::getJniAccelerator() != NULL
            && TR::SimpleRegex::match(TR::Options::getJniAccelerator(), signature(trMemory))) {
            _jniProperties |= J9_FAST_JNI_RETAIN_VM_ACCESS | J9_FAST_JNI_NOT_GC_POINT | J9_FAST_NO_NATIVE_METHOD_FRAME
                | J9_FAST_JNI_NO_EXCEPTION_THROW | J9_FAST_JNI_NO_SPECIAL_TEAR_DOWN;
        }
    } else {
        _jniProperties = 0;
...
#define x(a, b, c) a, sizeof(b) - 1, b, (int16_t)strlen(c), c
    ...
    static X ArrayListMethods[] = { { x(TR::java_util_ArrayList_add, "add", "(Ljava/lang/Object;)Z") },
        ...
        { x(TR::java_util_ArrayList_set, "set", "(ILjava/lang/Object;)Ljava/lang/Object;") }, { TR::unknownMethod } };

    static X ClassMethods[] = { { x(TR::java_lang_Class_newInstancePrototype, "newInstancePrototype",
        ...
        { x(TR::java_lang_Class_getStackClass, "getStackClass", "(I)Ljava/lang/Class;") }, { TR::unknownMethod } };

    static X ClassLoaderMethods[] = { { x(TR::java_lang_ClassLoader_callerClassLoader, "callerClassLoader",
    ...
    static X HashtableMethods[] = { { x(TR::java_util_Hashtable_get, "get", "(Ljava/lang/Object;)Ljava/lang/Object;") },
        ...
        { x(TR::java_util_Hashtable_elements, "elements", "()Ljava/util/Enumeration;") }, { TR::unknownMethod } };

    static X HashMapMethods[] = { { x(TR::java_util_HashMap_rehash, "rehash", "()V") },
        ...
        { x(TR::java_util_HashMap_findNonNullKeyEntry, "findNonNullKeyEntry",
    ...
    static X WriterMethods[] = { { x(TR::java_io_Writer_write_lStringII, "write", "(Ljava/lang/String;II)V") },
        { x(TR::java_io_Writer_write_I, "write", "(I)V") }, { TR::unknownMethod } };

    static X ByteArrayOutputStreamMethods[]
        = { { x(TR::java_io_ByteArrayOutputStream_write, "write", "([BII)V") }, { TR::unknownMethod } };

    static X ObjectInputStream_BlockDataInputStreamMethods[]
        = { { x(TR::java_io_ObjectInputStream_BlockDataInputStream_read, "read", "([BIIZ)I") }, { TR::unknownMethod } };

    static X BitsMethods[] = { { x(TR::java_nio_Bits_copyToByteArray, "copyToByteArray", "(JLjava/lang/Object;JJ)V") },
        ...
        { x(TR::java_nio_Bits_getLongL, "getLongL", "(Ljava/nio/ByteBuffer;I)J") }, { TR::unknownMethod } };

    static X HeapByteBufferMethods[] = { { x(TR::java_nio_HeapByteBuffer__get, "_get", "(I)B") },
        { x(TR::java_nio_HeapByteBuffer_put, "put", "(IB)Ljava/nio/ByteBuffer;") }, { TR::unknownMethod } };

    static X MathMethods[] = { { x(TR::java_lang_Math_abs_I, "abs", "(I)I") },
        { x(TR::java_lang_Math_abs_L, "abs", "(J)J") }, { x(TR::java_lang_Math_abs_F, "abs", "(F)F") },
        { x(TR::java_lang_Math_abs_D, "abs", "(D)D") }, { x(TR::java_lang_Math_acos, "acos", "(D)D") },
        { x(TR::java_lang_Math_asin, "asin", "(D)D") }, { x(TR::java_lang_Math_atan, "atan", "(D)D") },
        { x(TR::java_lang_Math_atan2, "atan2", "(DD)D") }, { x(TR::java_lang_Math_cbrt, "cbrt", "(D)D") },
        { x(TR::java_lang_Math_ceil, "ceil", "(D)D") }, { x(TR::java_lang_Math_copySign_F, "copySign", "(FF)F") },
        { x(TR::java_lang_Math_copySign_D, "copySign", "(DD)D") }, { x(TR::java_lang_Math_cos, "cos", "(D)D") },
        { x(TR::java_lang_Math_cosh, "cosh", "(D)D") }, { x(TR::java_lang_Math_exp, "exp", "(D)D") },
        { x(TR::java_lang_Math_expm1, "expm1", "(D)D") }, { x(TR::java_lang_Math_floor, "floor", "(D)D") },
        { x(TR::java_lang_Math_hypot, "hypot", "(DD)D") },
        { x(TR::java_lang_Math_IEEEremainder, "IEEEremainder", "(DD)D") }, { x(TR::java_lang_Math_log, "log", "(D)D") },
        { x(TR::java_lang_Math_log10, "log10", "(D)D") }, { x(TR::java_lang_Math_log1p, "log1p", "(D)D") },
        { x(TR::java_lang_Math_max_I, "max", "(II)I") }, { x(TR::java_lang_Math_max_L, "max", "(JJ)J") },
        { x(TR::java_lang_Math_max_F, "max", "(FF)F") }, { x(TR::java_lang_Math_max_D, "max", "(DD)D") },
        { x(TR::java_lang_Math_min_I, "min", "(II)I") }, { x(TR::java_lang_Math_min_L, "min", "(JJ)J") },
        { x(TR::java_lang_Math_min_F, "min", "(FF)F") }, { x(TR::java_lang_Math_min_D, "min", "(DD)D") },
        { x(TR::java_lang_Math_nextAfter_F, "nextAfter", "(FD)F") },
        { x(TR::java_lang_Math_nextAfter_D, "nextAfter", "(DD)D") }, { x(TR::java_lang_Math_pow, "pow", "(DD)D") },
        { x(TR::java_lang_Math_rint, "rint", "(D)D") }, { x(TR::java_lang_Math_round_F, "round", "(F)I") },
        { x(TR::java_lang_Math_round_D, "round", "(D)J") }, { x(TR::java_lang_Math_scalb_F, "scalb", "(FI)F") },
        { x(TR::java_lang_Math_scalb_D, "scalb", "(DI)F") }, { x(TR::java_lang_Math_sin, "sin", "(D)D") },
        { x(TR::java_lang_Math_sinh, "sinh", "(D)D") }, { x(TR::java_lang_Math_sqrt, "sqrt", "(D)D") },
        { x(TR::java_lang_Math_tan, "tan", "(D)D") }, { x(TR::java_lang_Math_tanh, "tanh", "(D)D") },
        { x(TR::java_lang_Math_fma_D, "fma", "(DDD)D") }, { x(TR::java_lang_Math_fma_F, "fma", "(FFF)F") },
        { x(TR::java_lang_Math_multiplyHigh, "multiplyHigh", "(JJ)J") }, { TR::unknownMethod } };

    static X StrictMathMethods[] = { { x(TR::java_lang_StrictMath_acos, "acos", "(D)D") },
        { x(TR::java_lang_StrictMath_asin, "asin", "(D)D") }, { x(TR::java_lang_StrictMath_atan, "atan", "(D)D") },
        { x(TR::java_lang_StrictMath_atan2, "atan2", "(DD)D") }, { x(TR::java_lang_StrictMath_cbrt, "cbrt", "(D)D") },
        ...
        { x(TR::java_lang_StrictMath_cos, "cos", "(D)D") }, { x(TR::java_lang_StrictMath_cosh, "cosh", "(D)D") },
        { x(TR::java_lang_StrictMath_exp, "exp", "(D)D") }, { x(TR::java_lang_StrictMath_expm1, "expm1", "(D)D") },
        { x(TR::java_lang_StrictMath_floor, "floor", "(D)D") }, { x(TR::java_lang_StrictMath_hypot, "hypot", "(DD)D") },
        { x(TR::java_lang_StrictMath_IEEEremainder, "IEEEremainder", "(DD)D") },
        { x(TR::java_lang_StrictMath_log, "log", "(D)D") }, { x(TR::java_lang_StrictMath_log10, "log10", "(D)D") },
        { x(TR::java_lang_StrictMath_log1p, "log1p", "(D)D") }, { x(TR::java_lang_StrictMath_max_F, "max", "(FF)F") },
        { x(TR::java_lang_StrictMath_max_D, "max", "(DD)D") }, { x(TR::java_lang_StrictMath_min_F, "min", "(FF)F") },
        ...
        { x(TR::java_lang_StrictMath_pow, "pow", "(DD)D") }, { x(TR::java_lang_StrictMath_rint, "rint", "(D)D") },
        ...
        { x(TR::java_lang_StrictMath_scalb_D, "scalb", "(DI)F") }, { x(TR::java_lang_StrictMath_sin, "sin", "(D)D") },
        { x(TR::java_lang_StrictMath_sinh, "sinh", "(D)D") }, { x(TR::java_lang_StrictMath_sqrt, "sqrt", "(D)D") },
        { x(TR::java_lang_StrictMath_tan, "tan", "(D)D") }, { x(TR::java_lang_StrictMath_tanh, "tan", "(D)D") },
    ...
    static X ObjectMethods[] = {
        ...
        { x(TR::java_lang_Object_newInstancePrototype, "newInstancePrototype",
         "(Ljava/lang/Class;)Ljava/lang/Object;") },
    ...
    static X XPCryptoMethods[] = { { x(TR::com_ibm_jit_crypto_JITAESCryptInHardware_isAESSupportedByHardwareImpl,
                                       "isAESSupportedByHardwareImpl", "()Z") },
        { x(TR::com_ibm_jit_crypto_JITAESCryptInHardware_doAESInHardware, "doAESInHardware", "([BII[BI[IIZ)Z") },
        { x(TR::com_ibm_jit_crypto_JITAESCryptInHardware_expandAESKeyInHardware, "expandAESKeyInHardware",
            "([B[II)Z") },
        { TR::unknownMethod } };

    static X ZCryptoMethods[] = { { x(TR::com_ibm_jit_crypto_JITFullHardwareCrypt_z_km, "z_km", "([BII[BI[BI)V") },
    ...
    static X CryptoECC224Methods[]
        = { { x(TR::com_ibm_crypto_provider_P224PrimeField_multiply, "multiply", "([I[I[I)V") },
              ...
              { x(TR::com_ibm_crypto_provider_P224PrimeField_mod, "mod", "([I[I)V") }, { TR::unknownMethod } };
    ...
    static X CryptoECC256Methods[]
        = { { x(TR::com_ibm_crypto_provider_P256PrimeField_multiply, "multiply", "([I[I[I)V") },
              ...
              { x(TR::com_ibm_crypto_provider_P256PrimeField_mod, "mod", "([I[I)V") }, { TR::unknownMethod } };
    ...
    static X CryptoECC384Methods[]
        = { { x(TR::com_ibm_crypto_provider_P384PrimeField_multiply, "multiply", "([I[I[I)V") },
              ...
              { x(TR::com_ibm_crypto_provider_P384PrimeField_mod, "mod", "([I[I)V") }, { TR::unknownMethod } };
    ...
    static X DataAccessDecimalDataMethods[] = { { x(TR::com_ibm_dataaccess_DecimalData_JITIntrinsicsEnabled,
                                                    "JITIntrinsicsEnabled", "()Z") },

        { x(TR::com_ibm_dataaccess_DecimalData_convertPackedDecimalToInteger, "convertPackedDecimalToInteger",
            "([BIIZ)I") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertPackedDecimalToInteger_, "convertPackedDecimalToInteger_",
            "([BIIZ)I") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertPackedDecimalToInteger_ByteBuffer,
            "convertPackedDecimalToInteger", "(Ljava/nio/ByteBuffer;IIZ)I") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertPackedDecimalToInteger_ByteBuffer_,
            "convertPackedDecimalToInteger_", "(Ljava/nio/ByteBuffer;IIZJII)I") },

        { x(TR::com_ibm_dataaccess_DecimalData_convertIntegerToPackedDecimal, "convertIntegerToPackedDecimal",
            "(I[BIIZ)V") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertIntegerToPackedDecimal_, "convertIntegerToPackedDecimal_",
            "(I[BIIZ)V") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertIntegerToPackedDecimal_ByteBuffer,
            "convertIntegerToPackedDecimal", "(ILjava/nio/ByteBuffer;IIZ)V") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertIntegerToPackedDecimal_ByteBuffer_,
            "convertIntegerToPackedDecimal_", "(ILjava/nio/ByteBuffer;IIZJII)V") },

        { x(TR::com_ibm_dataaccess_DecimalData_convertPackedDecimalToLong, "convertPackedDecimalToLong", "([BIIZ)J") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertPackedDecimalToLong_, "convertPackedDecimalToLong_",
            "([BIIZ)J") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertPackedDecimalToLong_ByteBuffer, "convertPackedDecimalToLong",
            "(Ljava/nio/ByteBuffer;IIZ)J") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertPackedDecimalToLong_ByteBuffer_, "convertPackedDecimalToLong_",
            "(Ljava/nio/ByteBuffer;IIZJII)J") },

        { x(TR::com_ibm_dataaccess_DecimalData_convertLongToPackedDecimal, "convertLongToPackedDecimal", "(J[BIIZ)V") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertLongToPackedDecimal_, "convertLongToPackedDecimal_",
            "(J[BIIZ)V") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertLongToPackedDecimal_ByteBuffer, "convertLongToPackedDecimal",
            "(JLjava/nio/ByteBuffer;IIZ)V") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertLongToPackedDecimal_ByteBuffer_, "convertLongToPackedDecimal_",
            "(JLjava/nio/ByteBuffer;IIZJII)V") },

        { x(TR::com_ibm_dataaccess_DecimalData_convertExternalDecimalToInteger, "convertExternalDecimalToInteger",
            "([BIIZI)I") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertExternalDecimalToInteger_, "convertExternalDecimalToInteger_",
            "([BIIZI)I") },

        { x(TR::com_ibm_dataaccess_DecimalData_convertIntegerToExternalDecimal, "convertIntegerToExternalDecimal",
            "(I[BIIZI)V") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertIntegerToExternalDecimal_, "convertIntegerToExternalDecimal_",
            "(I[BIIZI)V") },

        { x(TR::com_ibm_dataaccess_DecimalData_convertExternalDecimalToLong, "convertExternalDecimalToLong",
            "([BIIZI)J") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertExternalDecimalToLong_, "convertExternalDecimalToLong_",
            "([BIIZI)J") },

        { x(TR::com_ibm_dataaccess_DecimalData_convertLongToExternalDecimal, "convertLongToExternalDecimal",
            "(J[BIIZI)V") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertLongToExternalDecimal_, "convertLongToExternalDecimal_",
            "(J[BIIZI)V") },

        { x(TR::com_ibm_dataaccess_DecimalData_convertPackedDecimalToExternalDecimal,
            "convertPackedDecimalToExternalDecimal", "([BI[BIII)V") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertPackedDecimalToExternalDecimal_,
            "convertPackedDecimalToExternalDecimal_", "([BI[BIII)V") },

        { x(TR::com_ibm_dataaccess_DecimalData_convertExternalDecimalToPackedDecimal,
            "convertExternalDecimalToPackedDecimal", "([BI[BIII)V") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertExternalDecimalToPackedDecimal_,
            "convertExternalDecimalToPackedDecimal_", "([BI[BIII)V") },

        { x(TR::com_ibm_dataaccess_DecimalData_convertPackedDecimalToUnicodeDecimal,
            "convertPackedDecimalToUnicodeDecimal", "([BI[CIII)V") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertPackedDecimalToUnicodeDecimal_,
            "convertPackedDecimalToUnicodeDecimal_", "([BI[CIII)V") },

        { x(TR::com_ibm_dataaccess_DecimalData_convertUnicodeDecimalToPackedDecimal,
            "convertUnicodeDecimalToPackedDecimal", "([CI[BIII)V") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertUnicodeDecimalToPackedDecimal_,
            "convertUnicodeDecimalToPackedDecimal_", "([CI[BIII)V") },

        { x(TR::com_ibm_dataaccess_DecimalData_convertPackedDecimalToBigInteger, "convertPackedDecimalToBigInteger",
            "([BIIZ)Ljava/math/BigInteger;") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertBigIntegerToPackedDecimal, "convertBigIntegerToPackedDecimal",
            "(Ljava/math/BigInteger;[BIIZ)V") },

        { x(TR::com_ibm_dataaccess_DecimalData_convertPackedDecimalToBigDecimal, "convertPackedDecimalToBigDecimal",
            "([BIIIZ)Ljava/math/BigDecimal;") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertBigDecimalToPackedDecimal, "convertBigDecimalToPackedDecimal",
            "(Ljava/math/BigDecimal;[BIIZ)V") },

        { x(TR::com_ibm_dataaccess_DecimalData_convertExternalDecimalToBigInteger, "convertExternalDecimalToBigInteger",
            "([BIIZI)Ljava/math/BigInteger;") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertBigIntegerToExternalDecimal, "convertBigIntegerToExternalDecimal",
            "(Ljava/math/BigInteger;[BIIZI)V") },

        { x(TR::com_ibm_dataaccess_DecimalData_convertExternalDecimalToBigDecimal, "convertExternalDecimalToBigDecimal",
            "([BIIIZI)Ljava/math/BigDecimal;") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertBigDecimalToExternalDecimal, "convertBigDecimalToExternalDecimal",
            "(Ljava/math/BigDecimal;[BIIZI)V") },

        { x(TR::com_ibm_dataaccess_DecimalData_convertUnicodeDecimalToInteger, "convertUnicodeDecimalToInteger",
            "([CIIZI)I") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertIntegerToUnicodeDecimal, "convertIntegerToUnicodeDecimal",
            "(I[CIIZI)V") },

        { x(TR::com_ibm_dataaccess_DecimalData_convertUnicodeDecimalToLong, "convertUnicodeDecimalToLong",
            "([CIIZI)J") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertLongToUnicodeDecimal, "convertLongToUnicodeDecimal",
            "(J[CIIZI)V") },

        { x(TR::com_ibm_dataaccess_DecimalData_convertUnicodeDecimalToBigInteger, "convertUnicodeDecimalToBigInteger",
            "([CIIZI)Ljava/math/BigInteger;") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertBigIntegerToUnicodeDecimal, "convertBigIntegerToUnicodeDecimal",
            "(Ljava/math/BigInteger;[CIIZI)V") },

        { x(TR::com_ibm_dataaccess_DecimalData_convertUnicodeDecimalToBigDecimal, "convertUnicodeDecimalToBigDecimal",
            "([CIIIZI)Ljava/math/BigDecimal;") },
        { x(TR::com_ibm_dataaccess_DecimalData_convertBigDecimalToUnicodeDecimal, "convertBigDecimalToUnicodeDecimal",
            "(Ljava/math/BigDecimal;[CIIZI)V") },

        { x(TR::com_ibm_dataaccess_DecimalData_translateArray, "translateArray", "([BII[BI[BII)V") },

        { x(TR::com_ibm_dataaccess_DecimalData_slowSignedPackedToBigDecimal, "slowSignedPackedToBigDecimal",
            "([BIIIZ)Ljava/math/BigDecimal;") },
        { x(TR::com_ibm_dataaccess_DecimalData_slowBigDecimalToSignedPacked, "slowBigDecimalToSignedPacked",
            "(Ljava/math/BigDecimal;[BIIZ)V") },

        { TR::unknownMethod } };

    static X DataAccessPackedDecimalMethods[] = { { x(TR::com_ibm_dataaccess_PackedDecimal_checkPackedDecimal,
                                                      "checkPackedDecimal", "([BIIZZ)I") },
        ...
        { x(TR::com_ibm_dataaccess_PackedDecimal_subtractPackedDecimal_, "subtractPackedDecimal_",
            "([BII[BII[BIIZ)V") },

        { x(TR::com_ibm_dataaccess_PackedDecimal_multiplyPackedDecimal, "multiplyPackedDecimal", "([BII[BII[BIIZ)V") },
        { x(TR::com_ibm_dataaccess_PackedDecimal_multiplyPackedDecimal_, "multiplyPackedDecimal_",
            "([BII[BII[BIIZ)V") },
        ...
        { x(TR::com_ibm_dataaccess_PackedDecimal_remainderPackedDecimal, "remainderPackedDecimal",
            "([BII[BII[BIIZ)V") },
        { x(TR::com_ibm_dataaccess_PackedDecimal_remainderPackedDecimal_, "remainderPackedDecimal_",
            "([BII[BII[BIIZ)V") },
        ...
        { x(TR::com_ibm_dataaccess_PackedDecimal_lessThanOrEqualsPackedDecimal, "lessThanOrEqualsPackedDecimal",
            "([BII[BII)Z") },
        { x(TR::com_ibm_dataaccess_PackedDecimal_lessThanOrEqualsPackedDecimal_, "lessThanOrEqualsPackedDecimal_",
            "([BII[BII)Z") },

        { x(TR::com_ibm_dataaccess_PackedDecimal_greaterThanPackedDecimal, "greaterThanPackedDecimal", "([BII[BII)Z") },
        { x(TR::com_ibm_dataaccess_PackedDecimal_greaterThanPackedDecimal_, "greaterThanPackedDecimal_",
            "([BII[BII)Z") },

        { x(TR::com_ibm_dataaccess_PackedDecimal_greaterThanOrEqualsPackedDecimal, "greaterThanOrEqualsPackedDecimal",
            "([BII[BII)Z") },
        { x(TR::com_ibm_dataaccess_PackedDecimal_greaterThanOrEqualsPackedDecimal_, "greaterThanOrEqualsPackedDecimal_",
            "([BII[BII)Z") },
        ...
        { x(TR::com_ibm_dataaccess_PackedDecimal_shiftRightPackedDecimal, "shiftRightPackedDecimal",
            "([BII[BIIIZZ)V") },
        { x(TR::com_ibm_dataaccess_PackedDecimal_shiftRightPackedDecimal_, "shiftRightPackedDecimal_",
            "([BII[BIIIZZ)V") },
    ...
    static X BigDecimalMethods[]
        = { { x(TR::java_math_BigDecimal_add, "add", "(Ljava/math/BigDecimal;)Ljava/math/BigDecimal;") },
              ...
              { x(TR::java_math_BigDecimal_possibleClone, "possibleClone",
              ...
              { x(TR::java_math_BigDecimal_slowSubMulAddAddMulSetScale, "slowSMAAMSS",
                  "(Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/"
                  "BigDecimal;)Ljava/math/BigDecimal;") },
              { x(TR::java_math_BigDecimal_slowSubMulSetScale, "slowSMSS",
                  "(Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/BigDecimal;)Ljava/math/BigDecimal;") },
              { x(TR::java_math_BigDecimal_slowAddAddMulSetScale, "slowAAMSS",
                  "(Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/BigDecimal;)Ljava/"
                  "math/BigDecimal;") },
              { x(TR::java_math_BigDecimal_slowMulSetScale, "slowMSS",
                  "(Ljava/math/BigDecimal;Ljava/math/BigDecimal;)Ljava/math/BigDecimal;") },
              { x(TR::java_math_BigDecimal_subMulAddAddMulSetScale, "SMAAMSS",
                  "(Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/"
                  "BigDecimal;IIII)Ljava/math/BigDecimal;") },
              { x(TR::java_math_BigDecimal_subMulSetScale, "SMSS",
                  "(Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/BigDecimal;II)Ljava/math/BigDecimal;") },
              { x(TR::java_math_BigDecimal_addAddMulSetScale, "AAMSS",
                  "(Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/BigDecimal;III)Ljava/"
                  "math/BigDecimal;") },
              { x(TR::java_math_BigDecimal_mulSetScale, "MSS",
              ...
              { x(TR::java_math_BigDecimal_longAdd, "longAdd",
                  "(Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/MathContext;Z)Ljava/math/BigDecimal;") },
              { x(TR::java_math_BigDecimal_slAdd, "slAdd",
                  "(Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/MathContext;Z)Ljava/math/BigDecimal;") },
    ...
    static X ArraysMethods[] = {
        ...
        { x(TR::java_util_Arrays_copyOf_Object2, "copyOf",
        ...
        { x(TR::java_util_Arrays_copyOfRange_Object2, "copyOfRange",
    ...
    static X StringMethods[] = {
        ...
        { x(TR::java_lang_String_encode8859_1,                      "encode8859_1",             "(B[BZ)[B") },
        { x(TR::java_lang_String_encodeASCII,                       "encodeASCII",              "(B[B)[B") },
        { x(TR::java_lang_String_encodeUTF8_UTF16,                  "encodeUTF8_UTF16",         "([BZ)[B") },
        ...
        { x(TR::java_lang_String_getBytes,                          "getBytes",                 "([BIB)V") },
        { x(TR::java_lang_String_getBytes_subString,                "getBytes",                 "([BIIBI)V") },
        ...
        /* Matching is done top to bottom so the wildcard match for "<init>" must be last or it will override the more
           specialized matches. */
        ...
        { x(TR::java_lang_String_newStringUTF8NoRepl,               "newStringUTF8NoRepl",      "([BIIZ)Ljava/lang/String;") },
        ...
        { x(TR::java_lang_String_repeat,                            "repeat",                   "(I)Ljava/lang/String;") },
        { x(TR::java_lang_String_replace,                           "replace",                  "(CC)Ljava/lang/String;") },
        { x(TR::java_lang_String_split_CIZ,                         "split",                    "(CIZ)[Ljava/lang/String;") },
    ...
    static X StringCodingMethods[]
        = { { x(TR::java_lang_StringCoding_decode, "decode", "(Ljava/nio/charset/Charset;[BII)[C") },
              ...
              { x(TR::java_lang_StringCoding_encodeUTF8, "encodeUTF8", "(B[BZ)[B") }, { TR::unknownMethod } };

    static X StringConcatHelperMethods[]
        = { { x(TR::java_lang_StringConcatHelper_newArray, "newArray", "(J)[B") }, { TR::unknownMethod } };

    static X StringCoding_StringDecoderMethods[]
        = { { x(TR::java_lang_StringCoding_StringDecoder_decode, "decode", "([BII)[C") }, { TR::unknownMethod } };

    static X StringCoding_StringEncoderMethods[]
        = { { x(TR::java_lang_StringCoding_StringEncoder_encode, "encode", "([CII)[B") }, { TR::unknownMethod } };

    static X NumberFormatMethods[]
        = { { x(TR::java_text_NumberFormat_format, "format", "(D)Ljava/lang/String;") }, { TR::unknownMethod } };

    static X StringBuilderMethods[] = { { x(TR::java_lang_StringBuilder_append_bool, "append",
        ...
        { x(TR::java_lang_StringBuilder_toString, "toString", "()Ljava/lang/String;") }, { TR::unknownMethod } };

    static X AbstractStringBuilderMethods[] = { { x(TR::java_lang_AbstractStringBuilder_append_char, "append",
                                                    "(C)Ljava/lang/AbstractStringBuilder;") },
        { x(TR::java_lang_AbstractStringBuilder_append_String, "append",
            "(Ljava/lang/String;)Ljava/lang/AbstractStringBuilder;") },
        { x(TR::java_lang_AbstractStringBuilder_appendChars, "appendChars", "([CII)V") },
    ...
    static X UnsafeMethods[] = { { x(TR::sun_misc_Unsafe_putBoolean_jlObjectJZ_V, "putBoolean",
        ...
        { x(TR::sun_misc_Unsafe_putObject_jlObjectJjlObject_V, "putObject",
            "(Ljava/lang/Object;JLjava/lang/Object;)V") },
        { x(TR::sun_misc_Unsafe_putObject_jlObjectJjlObject_V, "putReference",
            "(Ljava/lang/Object;JLjava/lang/Object;)V") },
        ...
        { x(TR::sun_misc_Unsafe_putObjectVolatile_jlObjectJjlObject_V, "putObjectVolatile",
            "(Ljava/lang/Object;JLjava/lang/Object;)V") },
        { x(TR::sun_misc_Unsafe_putObjectVolatile_jlObjectJjlObject_V, "putReferenceVolatile",
            "(Ljava/lang/Object;JLjava/lang/Object;)V") },
        ...
        { x(TR::sun_misc_Unsafe_putObjectOrdered_jlObjectJjlObject_V, "putReferenceRelease",
            "(Ljava/lang/Object;JLjava/lang/Object;)V") },

        { x(TR::jdk_internal_misc_Unsafe_putBooleanOpaque_jlObjectJZ_V, "putBooleanOpaque",
            "(Ljava/lang/Object;JZ)V") },
        ...
        { x(TR::jdk_internal_misc_Unsafe_putReferenceOpaque_jlObjectJjlObject_V, "putReferenceOpaque",
            "(Ljava/lang/Object;JLjava/lang/Object;)V") },
        ...
        { x(TR::jdk_internal_misc_Unsafe_getReferenceOpaque_jlObjectJ_jlObject, "getReferenceOpaque",
        ...
        { x(TR::sun_misc_Unsafe_getObject_jlObjectJ_jlObject, "getReference",
            "(Ljava/lang/Object;J)Ljava/lang/Object;") },
        ...
        { x(TR::sun_misc_Unsafe_getObjectVolatile_jlObjectJ_jlObject, "getObjectVolatile",
            "(Ljava/lang/Object;J)Ljava/lang/Object;") },
        { x(TR::sun_misc_Unsafe_getObjectVolatile_jlObjectJ_jlObject, "getReferenceVolatile",
            "(Ljava/lang/Object;J)Ljava/lang/Object;") },

        { x(TR::jdk_internal_misc_Unsafe_getBooleanAcquire_jlObjectJ_Z, "getBooleanAcquire",
        ...
        { x(TR::jdk_internal_misc_Unsafe_getReferenceAcquire_jlObjectJ_jlObject, "getObjectAcquire",
            "(Ljava/lang/Object;J)Ljava/lang/Object;") },
        { x(TR::jdk_internal_misc_Unsafe_getReferenceAcquire_jlObjectJ_jlObject, "getReferenceAcquire",
            "(Ljava/lang/Object;J)Ljava/lang/Object;") },
        ...
        { x(TR::sun_misc_Unsafe_compareAndSwapObject_jlObjectJjlObjectjlObject_Z, "compareAndSwapObject",
            "(Ljava/lang/Object;JLjava/lang/Object;Ljava/lang/Object;)Z") },
        { x(TR::sun_misc_Unsafe_compareAndSwapObject_jlObjectJjlObjectjlObject_Z, "compareAndSetObject",
            "(Ljava/lang/Object;JLjava/lang/Object;Ljava/lang/Object;)Z") },
        { x(TR::sun_misc_Unsafe_compareAndSwapObject_jlObjectJjlObjectjlObject_Z, "compareAndSetReference",
            "(Ljava/lang/Object;JLjava/lang/Object;Ljava/lang/Object;)Z") },

        { x(TR::jdk_internal_misc_Unsafe_compareAndExchangeInt, "compareAndExchangeInt", "(Ljava/lang/Object;JII)I") },
        { x(TR::jdk_internal_misc_Unsafe_compareAndExchangeLong, "compareAndExchangeLong",
            "(Ljava/lang/Object;JJJ)J") },
        { x(TR::jdk_internal_misc_Unsafe_compareAndExchangeObject, "compareAndExchangeObject",
            "(Ljava/lang/Object;JLjava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;") },
        { x(TR::jdk_internal_misc_Unsafe_compareAndExchangeReference, "compareAndExchangeReference",
            "(Ljava/lang/Object;JLjava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;") },
        ...
        { x(TR::sun_misc_Unsafe_putObjectOrdered_jlObjectJjlObject_V, "putOrderedObject",
            "(Ljava/lang/Object;JLjava/lang/Object;)V") },
...
#if (17 <= JAVA_SPEC_VERSION) && (JAVA_SPEC_VERSION <= 18)
        { x(TR::jdk_internal_loader_NativeLibraries_load, "load",
...
#define LANE_TYPE "Ljava/lang/Class;"
    ...
    static X VectorSupportMethods[] = {
        ...
        { x(TR::jdk_internal_vm_vector_VectorSupport_load, "load",
            "(Ljava/lang/Class;Ljava/lang/Class;ILjava/lang/Object;JLjava/lang/Object;JLjdk/internal/vm/vector/"
        ...
        { x(TR::jdk_internal_vm_vector_VectorSupport_shuffleIota, "shuffleIota",
            "(Ljava/lang/Class;Ljava/lang/Class;Ljdk/internal/vm/vector/VectorSupport$VectorSpecies;IIIILjdk/internal/"
            "vm/vector/VectorSupport$ShuffleIotaOperation;)Ljdk/internal/vm/vector/VectorSupport$VectorShuffle;") },
        { x(TR::jdk_internal_vm_vector_VectorSupport_shuffleToVector, "shuffleToVector",
            "(Ljava/lang/Class;Ljava/lang/Class;Ljava/lang/Class;Ljdk/internal/vm/vector/"
            "VectorSupport$VectorShuffle;ILjdk/internal/vm/vector/VectorSupport$ShuffleToVectorOperation;)Ljdk/"
        ...
        { x(TR::jdk_internal_vm_vector_VectorSupport_wrapShuffleIndexes, "wrapShuffleIndexes",
            "(Ljava/lang/Class;Ljava/lang/Class;Ljdk/internal/vm/vector/VectorSupport$VectorShuffle;ILjdk/internal/vm/"
            "vector/VectorSupport$WrapShuffleIndexesOperation;)Ljdk/internal/vm/vector/VectorSupport$VectorShuffle;") },

#if JAVA_SPEC_VERSION <= 21
        { x(TR::jdk_internal_vm_vector_VectorSupport_store, "store",
            "(Ljava/lang/Class;Ljava/lang/Class;ILjava/lang/Object;JLjdk/internal/vm/vector/"
...
#undef LANE_TYPE
    ...
    static X SingleByteConverterMethods[] = { { x(TR::sun_io_ByteToCharSingleByte_convert, "convert", "([BII[CII)I") },
        { x(TR::sun_io_CharToByteSingleByte_convert, "convert", "([CII[BII)I") },
        { x(TR::sun_io_ByteToCharSingleByte_JITintrinsicConvert, "JITintrinsicConvert",
            "([Ljava/nio/ByteBuffer;[CI[B)I") },
        { TR::unknownMethod } };

    static X DoubleByteConverterMethods[]
        = { { x(TR::sun_io_ByteToCharDBCS_EBCDIC_convert, "convert", "([BII[CII)I") }, { TR::unknownMethod } };

    static X ORBVMHelperMethods[] = { { x(TR::com_ibm_oti_vm_ORBVMHelpers_is32Bit, "is32Bit", "()Z") },
        ...
        { x(TR::com_ibm_oti_vm_ORBVMHelpers_getInstanceShapeFromJ9Class32, "getTotalInstanceSizeFromJ9Class32",
            "(I)I") },
        { x(TR::com_ibm_oti_vm_ORBVMHelpers_getInstanceDescriptionFromJ9Class32, "getInstanceDescriptionFromJ9Class32",
            "(I)I") },
        ...
        { x(TR::com_ibm_oti_vm_ORBVMHelpers_getInstanceShapeFromJ9Class64, "getTotalInstanceSizeFromJ9Class64",
            "(J)J") },
        { x(TR::com_ibm_oti_vm_ORBVMHelpers_getInstanceDescriptionFromJ9Class64, "getInstanceDescriptionFromJ9Class64",
            "(J)J") },
    ...
    static X JITHelperMethods[] = { { x(TR::com_ibm_jit_JITHelpers_is32Bit, "is32Bit", "()Z") },
        { x(TR::com_ibm_jit_JITHelpers_isArray, "isArray", "(Ljava/lang/Object;)Z") },
        { x(TR::com_ibm_jit_JITHelpers_intrinsicIndexOfStringLatin1, "intrinsicIndexOfStringLatin1",
            "(Ljava/lang/Object;ILjava/lang/Object;II)I") },
        { x(TR::com_ibm_jit_JITHelpers_intrinsicIndexOfStringUTF16, "intrinsicIndexOfStringUTF16",
            "(Ljava/lang/Object;ILjava/lang/Object;II)I") },
        ...
        { x(TR::com_ibm_jit_JITHelpers_getClassDepthAndFlagsFromJ9Class32, "getClassDepthAndFlagsFromJ9Class32",
            "(I)I") },
        ...
        { x(TR::com_ibm_jit_JITHelpers_getClassDepthAndFlagsFromJ9Class64, "getClassDepthAndFlagsFromJ9Class64",
            "(J)J") },
        ...
        { x(TR::com_ibm_jit_JITHelpers_getIntFromObjectVolatile, "getIntFromObjectVolatile",
            "(Ljava/lang/Object;J)I") },
        { x(TR::com_ibm_jit_JITHelpers_getLongFromObject, "getLongFromObject", "(Ljava/lang/Object;J)J") },
        { x(TR::com_ibm_jit_JITHelpers_getLongFromObjectVolatile, "getLongFromObjectVolatile",
            "(Ljava/lang/Object;J)J") },
        { x(TR::com_ibm_jit_JITHelpers_getObjectFromObject, "getObjectFromObject",
            "(Ljava/lang/Object;J)Ljava/lang/Object;") },
        { x(TR::com_ibm_jit_JITHelpers_getObjectFromObjectVolatile, "getObjectFromObjectVolatile",
            "(Ljava/lang/Object;J)Ljava/lang/Object;") },
        ...
        { x(TR::com_ibm_jit_JITHelpers_putObjectInObject, "putObjectInObject",
            "(Ljava/lang/Object;JLjava/lang/Object;)V") },
        { x(TR::com_ibm_jit_JITHelpers_putObjectInObjectVolatile, "putObjectInObjectVolatile",
            "(Ljava/lang/Object;JLjava/lang/Object;)V") },
        { x(TR::com_ibm_jit_JITHelpers_compareAndSwapIntInObject, "compareAndSwapIntInObject",
            "(Ljava/lang/Object;JII)Z") },
        { x(TR::com_ibm_jit_JITHelpers_compareAndSwapLongInObject, "compareAndSwapLongInObject",
            "(Ljava/lang/Object;JJJ)Z") },
        { x(TR::com_ibm_jit_JITHelpers_compareAndSwapObjectInObject, "compareAndSwapObjectInObject",
            "(Ljava/lang/Object;JLjava/lang/Object;Ljava/lang/Object;)Z") },
        ...
        { x(TR::com_ibm_jit_JITHelpers_getByteFromArrayVolatile, "getByteFromArrayVolatile",
        ...
        { x(TR::com_ibm_jit_JITHelpers_getCharFromArrayVolatile, "getCharFromArrayVolatile",
        ...
        { x(TR::com_ibm_jit_JITHelpers_getLongFromArrayVolatile, "getLongFromArrayVolatile",
            "(Ljava/lang/Object;J)J") },
        { x(TR::com_ibm_jit_JITHelpers_getObjectFromArray, "getObjectFromArray",
            "(Ljava/lang/Object;J)Ljava/lang/Object;") },
        { x(TR::com_ibm_jit_JITHelpers_getObjectFromArrayVolatile, "getObjectFromArrayVolatile",
            "(Ljava/lang/Object;J)Ljava/lang/Object;") },
        ...
        { x(TR::com_ibm_jit_JITHelpers_putObjectInArray, "putObjectInArray",
            "(Ljava/lang/Object;JLjava/lang/Object;)V") },
        { x(TR::com_ibm_jit_JITHelpers_putObjectInArrayVolatile, "putObjectInArrayVolatile",
            "(Ljava/lang/Object;JLjava/lang/Object;)V") },
        { x(TR::com_ibm_jit_JITHelpers_compareAndSwapIntInArray, "compareAndSwapIntInArray",
            "(Ljava/lang/Object;JII)Z") },
        { x(TR::com_ibm_jit_JITHelpers_compareAndSwapLongInArray, "compareAndSwapLongInArray",
            "(Ljava/lang/Object;JJJ)Z") },
        { x(TR::com_ibm_jit_JITHelpers_compareAndSwapObjectInArray, "compareAndSwapObjectInArray",
            "(Ljava/lang/Object;JLjava/lang/Object;Ljava/lang/Object;)Z") },
    ...
    static X StringLatin1Methods[] = {
        ...
        { x(TR::java_lang_StringLatin1_toBytes_III,          "toBytes",     "([III)[B") },
    ...
    static X StringUTF16Methods[] = {
        ...
        { x(TR::java_lang_StringUTF16_compress_III,         "compress",             "([III)[B") },
        { x(TR::java_lang_StringUTF16_getChar,              "getChar",              "([BI)C") },
        { x(TR::java_lang_StringUTF16_getChars_ByteArray,   "getChars",             "([BII[CI)V") },
        ...
        { x(TR::java_lang_StringUTF16_replace_char,         "replace",              "([BCC)Ljava/lang/String;") },
        { x(TR::java_lang_StringUTF16_replace_CharSequence, "replace",              "([BIZ[BIZ[BIZ)Ljava/lang/String;") },
        { x(TR::java_lang_StringUTF16_toBytes,              "toBytes",              "([CII)[B") },
        { x(TR::java_lang_StringUTF16_toBytes_C,            "toBytes",              "(C)[B") },
        { x(TR::java_lang_StringUTF16_toBytesSup,           "toBytesSupplementary", "(I)[B") },
    ...
    static X DecimalFormatHelperMethods[]
        = { { x(TR::com_ibm_jit_DecimalFormatHelper_formatAsDouble, "formatAsDouble",
                "(Ljava/text/DecimalFormat;Ljava/math/BigDecimal;)Ljava/lang/String;") },
              { x(TR::com_ibm_jit_DecimalFormatHelper_formatAsFloat, "formatAsFloat",
                  "(Ljava/text/DecimalFormat;Ljava/math/BigDecimal;)Ljava/lang/String;") },
              { TR::unknownMethod } };
    ...
    static X WASMethods[] = { { x(TR::com_ibm_ws_webcontainer_channel_WCCByteBufferOutputStream_printUnencoded,
                                  "printUnencoded", "(Ljava/lang/String;II)V") },
        { TR::unknownMethod } };
    ...
    static X ByteOrderMethods[]
        = { { x(TR::java_nio_ByteOrder_nativeOrder, "nativeOrder", "()Ljava/nio/ByteOrder;") }, { TR::unknownMethod } };
    ...
    static X CRC32Methods[] = { { x(TR::java_util_zip_CRC32_update, "update", "(II)I") },
        ...
        { x(TR::java_util_zip_CRC32_updateByteBuffer0, "updateByteBuffer0", "(IJII)I") }, { TR::unknownMethod } };

    static X CRC32CMethods[] = { { x(TR::java_util_zip_CRC32C_updateBytes, "updateBytes", "(I[BII)I") },
    ...
    static X X10Methods[] = { { x(TR::x10JITHelpers_speculateIndex, "speculateIndex", "(II)I") },
        ...
        { x(TR::x10JITHelpers_checkHighBounds, "checkHighBound", "(II)I") }, { TR::unknownMethod } };

    static X VMInternalsMethods[] = { { x(TR::java_lang_Class_newInstanceImpl, "newInstanceImpl",
                                          "(Ljava/lang/Class;)Ljava/lang/Object;") },
        ...
        { x(TR::java_lang_J9VMInternals_getInstanceDescriptionFromJ9Class32, "getInstanceDescriptionFromJ9Class32",
            "(I)I") },
        ...
        { x(TR::java_lang_J9VMInternals_getInstanceDescriptionFromJ9Class64, "getInstanceDescriptionFromJ9Class64",
            "(J)J") },
    ...
    static X ReferenceMethods[] = { { x(TR::java_lang_ref_Reference_getImpl, "getImpl", "()Ljava/lang/Object;") },
        { x(TR::java_lang_ref_Reference_reachabilityFence, "reachabilityFence", "(Ljava/lang/Object;)V") },
        { x(TR::java_lang_ref_Reference_refersTo, "refersTo", "(Ljava/lang/Object;)Z") }, { TR::unknownMethod } };
    ...
    static X JavaUtilConcurrentAtomicIntegerFieldUpdaterMethods[] = {
        ...
        { x(TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_getAndAdd, "getAndAdd",
            "(Ljava/lang/Object;I)I") },
        { x(TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_getAndIncrement, "getAndIncrement",
            "(Ljava/lang/Object;)I") },
        { x(TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_getAndDecrement, "getAndDecrement",
            "(Ljava/lang/Object;)I") },
        { x(TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_getAndSet, "getAndSet",
            "(Ljava/lang/Object;I)I") },
        { x(TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_addAndGet, "addAndGet",
            "(Ljava/lang/Object;I)I") },
        { x(TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_incrementAndGet, "incrementAndGet",
            "(Ljava/lang/Object;)I") },
        { x(TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_decrementAndGet, "decrementAndGet",
            "(Ljava/lang/Object;)I") },
        { x(TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_weakCompareAndSet, "weakCompareAndSet",
    ...
    static X JavaUtilConcurrentAtomicReferenceMethods[]
        = { /*    These are disabled because the codegens don't recognize them anymore.
             ...
             *    {x(TR::java_util_concurrent_atomic_AtomicReference_getAndSet,        "getAndSet",
             "(Ljava/lang/Object;)Ljava/lang/Object;")},
                  {x(TR::java_util_concurrent_atomic_AtomicReference_weakCompareAndSet,"weakCompareAndSet",
             "(Ljava/lang/Object;Ljava/lang/Object;)Z")}, {x(TR::java_util_concurrent_atomic_AtomicReference_lazySet,
    ...
    static X JavaUtilConcurrentAtomicReferenceArrayMethods[]
        = { /*
                  {x(TR::java_util_concurrent_atomic_AtomicReferenceArray_getAndSet,        "getAndSet",
               "(ILjava/lang/Object;)Ljava/lang/Object;")},
                  {x(TR::java_util_concurrent_atomic_AtomicReferenceArray_weakCompareAndSet,"weakCompareAndSet",
               "(ILjava/lang/Object;Ljava/lang/Object;)Z")},
                  {x(TR::java_util_concurrent_atomic_AtomicReferenceArray_lazySet,          "lazySet",
    ...
    static X JavaUtilConcurrentAtomicReferenceFieldUpdaterMethods[] = {
        ...
        { x(TR::java_util_concurrent_atomic_AtomicReferenceFieldUpdater_getAndSet, "getAndSet",
            "(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;") },
        { x(TR::java_util_concurrent_atomic_AtomicReferenceFieldUpdater_weakCompareAndSet, "weakCompareAndSet",
            "(Ljava/lang/Object;Ljava/lang/Object;Ljava/lang/Object;)Z") },
        { x(TR::java_util_concurrent_atomic_AtomicReferenceFieldUpdater_lazySet, "lazySet",
    ...
    static X JavaUtilConcurrentAtomicLongArrayMethods[]
        = { /*
                  ...
                  {x(TR::java_util_concurrent_atomic_AtomicLongArray_weakCompareAndSet,  "weakCompareAndSet",
               "(IJJ)Z")}, {x(TR::java_util_concurrent_atomic_AtomicLongArray_lazySet,            "lazySet", "(IJ)V")},
            */
              { TR::unknownMethod }
          };
    ...
    static X JavaUtilConcurrentAtomicLongFieldUpdaterMethods[] = {
        ...
        { x(TR::java_util_concurrent_atomic_AtomicLongFieldUpdater_getAndIncrement, "getAndIncrement",
            "(Ljava/lang/Object;)J") },
        { x(TR::java_util_concurrent_atomic_AtomicLongFieldUpdater_getAndDecrement, "getAndDecrement",
            "(Ljava/lang/Object;)J") },
        ...
        { x(TR::java_util_concurrent_atomic_AtomicLongFieldUpdater_incrementAndGet, "incrementAndGet",
            "(Ljava/lang/Object;)J") },
        { x(TR::java_util_concurrent_atomic_AtomicLongFieldUpdater_decrementAndGet, "decrementAndGet",
            "(Ljava/lang/Object;)J") },
        { x(TR::java_util_concurrent_atomic_AtomicLongFieldUpdater_weakCompareAndSet, "weakCompareAndSet",
    ...
    static X JavaUtilConcurrentConcurrentHashMapMethods[]
        = { { x(TR::java_util_concurrent_ConcurrentHashMap_addCount, "addCount", "(JI)V") },
              ...
              { x(TR::java_util_concurrent_ConcurrentHashMap_transfer, "transfer",
              ...
              { x(TR::java_util_concurrent_ConcurrentHashMap_helpTransfer, "helpTransfer",
                  "([Ljava/util/concurrent/ConcurrentHashMap$Node;Ljava/util/concurrent/ConcurrentHashMap$Node;)[Ljava/"
                  "util/concurrent/ConcurrentHashMap$Node;") },
              { x(TR::java_util_concurrent_ConcurrentHashMap_initTable, "initTable",
                  "()[Ljava/util/concurrent/ConcurrentHashMap$Node;") },
              { x(TR::java_util_concurrent_ConcurrentHashMap_tabAt, "tabAt",
                  "([Ljava/util/concurrent/ConcurrentHashMap$Node;I)Ljava/util/concurrent/ConcurrentHashMap$Node;") },
              { x(TR::java_util_concurrent_ConcurrentHashMap_casTabAt, "casTabAt",
                  "([Ljava/util/concurrent/ConcurrentHashMap$Node;ILjava/util/concurrent/ConcurrentHashMap$Node;Ljava/"
                  "util/concurrent/ConcurrentHashMap$Node;)Z") },
              { x(TR::java_util_concurrent_ConcurrentHashMap_setTabAt, "setTabAt",
    ...
    static X JavaUtilConcurrentConcurrentHashMapTreeBinMethods[]
        = { { x(TR::java_util_concurrent_ConcurrentHashMap_TreeBin_lockRoot, "lockRoot", "") },
              ...
              { x(TR::java_util_concurrent_ConcurrentHashMap_TreeBin_find, "find", "") }, { TR::unknownMethod } };

    // Transactional Memory
    static X JavaUtilConcurrentConcurrentLinkedQueueMethods[]
        = { { x(TR::java_util_concurrent_ConcurrentLinkedQueue_tmOffer, "tmOffer",
                "(Ljava/util/concurrent/ConcurrentLinkedQueue$Node;)I") },
    ...
    static X QuadMethods[]
        = { { x(TR::com_ibm_Compiler_Internal_Quad_enableQuadOptimization, "enableQuadOptimization", "()Z") },
              { x(TR::com_ibm_Compiler_Internal_Quad_add_ql, "add",
                  "(Lcom/ibm/Compiler/Internal/Quad;J)Lcom/ibm/Compiler/Internal/Quad;") },
              { x(TR::com_ibm_Compiler_Internal_Quad_add_ll, "add", "(JJ)Lcom/ibm/Compiler/Internal/Quad;") },
              { x(TR::com_ibm_Compiler_Internal_Quad_sub_ql, "sub",
                  "(Lcom/ibm/Compiler/Internal/Quad;J)Lcom/ibm/Compiler/Internal/Quad;") },
    ...
    static X MethodHandleMethods[] = {
        ...
        { TR::java_lang_invoke_MethodHandle_invoke, 13, "invokeGeneric", (int16_t)-1,
         "*" }, // Older name from early versions of the jsr292 spec
        ...
        { x(TR::java_lang_invoke_MethodHandle_asType, "asType",
    ...
    static X DirectMethodHandleMethods[] = { { x(TR::java_lang_invoke_DirectMethodHandle_internalMemberName,
                                                 "internalMemberName", "(Ljava/lang/Object;)Ljava/lang/Object;") },
        { x(TR::java_lang_invoke_DirectMethodHandle_internalMemberNameEnsureInit, "internalMemberNameEnsureInit",
            "(Ljava/lang/Object;)Ljava/lang/Object;") },
        { x(TR::java_lang_invoke_DirectMethodHandle_constructorMethod, "constructorMethod",
            "(Ljava/lang/Object;)Ljava/lang/Object;") },
        { x(TR::java_lang_invoke_DirectMethodHandle_checkCast, "checkCast",
            "(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;") },
        { TR::unknownMethod } };
    ...
    static X ValueLayoutsAbstractValueLayoutMethods[]
        = { { x(TR::jdk_internal_foreign_layout_ValueLayouts_AbstractValueLayout_accessHandle, "accessHandle",
    ...
    static X ILGenMacrosMethods[] = {
        ...
        { x(TR::java_lang_invoke_ILGenMacros_invokeExactAndFixup, "invokeExact",
        ...
        { x(TR::java_lang_invoke_ILGenMacros_typeCheck, "typeCheck",
         "(Ljava/lang/invoke/MethodHandle;Ljava/lang/invoke/MethodType;)V") },
        { TR::unknownMethod }
    };

    static X CollectHandleMethods[] = {
        { x(TR::java_lang_invoke_CollectHandle_allocateArray, "allocateArray",
         "(Ljava/lang/invoke/CollectHandle;)Ljava/lang/Object;") },
    ...
    static X EncodeMethods[] = { { x(TR::sun_nio_cs_ISO_8859_1_Encoder_encodeArrayLoop, "encodeArrayLoop",
        ...
        { x(TR::sun_nio_cs_ISO_8859_1_Decoder_decodeISO8859_1, "decodeISO8859_1",
            "([BII[CI)I") }, // Used in IBM SDK Java 8
        ...
        { x(TR::sun_nio_cs_ext_SBCS_Encoder_encodeSBCS, "encodeSBCS", "([CII[BI[B)I") }, // Used in IBM SDK Java 8
        { x(TR::sun_nio_cs_ext_SBCS_Decoder_decodeSBCS, "decodeSBCS", "([BII[CI[C)I") }, // Used in IBM SDK Java 8
        { x(TR::sun_nio_cs_UTF_8_Encoder_encodeUTF_8, "encodeUTF_8", "([CII[BI)I") }, // Used in IBM SDK Java 8
        { x(TR::sun_nio_cs_UTF_8_Decoder_decodeUTF_8, "decodeUTF_8", "([BII[CI)I") }, // Used in IBM SDK Java 8
        { x(TR::sun_nio_cs_UTF16_Encoder_encodeUTF16Big, "encodeUTF16Big", "([CII[BI)I") }, // Used in IBM SDK Java 8
        { x(TR::sun_nio_cs_UTF16_Encoder_encodeUTF16Little, "encodeUTF16Little",
            "([CII[BI)I") }, // Used in IBM SDK Java 8
        { TR::unknownMethod } };

    static X SingleByteDecoderMethods[] = { // SingleByteDecoder_decodeToLatin1Impl is used by Semeru Runtime for z/OS.
        { x(TR::sun_nio_cs_SingleByteDecoder_decodeToLatin1Impl, "decodeToLatin1Impl", "([BII[BI[B)I") },
        { TR::unknownMethod }
    };
    static X SingleByteEncoderMethods[]
        = { // SingleByteEncoder_encodeFromLatin1Impl is used by Semeru Runtime for z/OS.
              { x(TR::sun_nio_cs_SingleByteEncoder_encodeFromLatin1Impl, "encodeFromLatin1Impl", "([BII[BI[B)I") },
              { TR::unknownMethod }
          };

    static X IBM1388EncoderMethods[]
        = { { x(TR::sun_nio_cs_ext_IBM1388_Encoder_encodeArrayLoop, "encodeArrayLoop",
                "(Ljava/nio/CharBuffer;Ljava/nio/ByteBuffer;)Ljava/nio/charset/CoderResult;") },
              { TR::unknownMethod } };
    ...
    static X ArgumentMoverHandleMethods[]
        = { { x(TR::java_lang_invoke_ArgumentMoverHandle_permuteArgs, "permuteArgs", "(I)I") }, { TR::unknownMethod } };
    ...
    static X PermuteHandleMethods[]
        = { { x(TR::java_lang_invoke_PermuteHandle_permuteArgs, "permuteArgs", "(I)I") }, { TR::unknownMethod } };

    static X GuardWithTestHandleMethods[]
        = { { x(TR::java_lang_invoke_GuardWithTestHandle_numGuardArgs, "numGuardArgs", "()I") },
              { TR::unknownMethod } };

    static X InsertHandleMethods[] = { { x(TR::java_lang_invoke_InsertHandle_numPrefixArgs, "numPrefixArgs", "()I") },
        { x(TR::java_lang_invoke_InsertHandle_numSuffixArgs, "numSuffixArgs", "()I") },
        { x(TR::java_lang_invoke_InsertHandle_numValuesToInsert, "numValuesToInsert", "()I") }, { TR::unknownMethod } };
    ...
    static X MethodHandleImplArrayAccessorMethods[]
        = { { x(TR::java_lang_invoke_MethodHandleImpl_ArrayAccessor_getElementI, "getElementI", "([II)I") },
              ...
              { x(TR::java_lang_invoke_MethodHandleImpl_ArrayAccessor_getElementL, "getElementL",
              ...
              { x(TR::java_lang_invoke_MethodHandleImpl_ArrayAccessor_setElementL, "setElementL",
    ...
    static X MethodHandleImplCountingWrapperMethods[]
        = { { x(TR::java_lang_invoke_MethodHandleImpl_CountingWrapper_getTarget, "getTarget",
                "()Ljava/lang/invoke/MethodHandle;") },
              { TR::unknownMethod } };

    static X DelegatingMethodHandleMethods[] = { { x(TR::java_lang_invoke_DelegatingMethodHandle_getTarget, "getTarget",
                                                     "()Ljava/lang/invoke/MethodHandle;") },
        { TR::unknownMethod } };

    static X DirectHandleMethods[] = { { x(TR::java_lang_invoke_DirectHandle_isAlreadyCompiled, "isAlreadyCompiled",
                                           "(J)Z") },
    ...
    static X FilterArgumentsWithCombinerHandleMethods[] = {
        ...
        { TR::java_lang_invoke_FilterArgumentsWithCombinerHandle_argumentsForCombiner, 20, "argumentsForCombiner",
         (int16_t)-1, "*" },
        { TR::unknownMethod }
    };

    static X FinallyHandleMethods[] = { { x(TR::java_lang_invoke_FinallyHandle_numFinallyTargetArgsToPassThrough,
                                            "numFinallyTargetArgsToPassThrough", "()I") },
        { TR::unknownMethod } };
    ...
    static X FilterArgumentsHandleMethods[] = {
        ...
        { x(TR::java_lang_invoke_FilterArgumentsHandle_filterArguments, "filterArguments",
         "([Ljava/lang/invoke/MethodHandle;I)I") },
    ...
    static X DirectMethodHandleAccessorMethods[] = { { x(TR::java_lang_invoke_DirectMethodHandle_Accessor_checkCast,
                                                         "checkCast", "(Ljava/lang/Object;)Ljava/lang/Object;") },
        { TR::unknownMethod } };

    static X CatchHandleMethods[] = { { x(TR::java_lang_invoke_CatchHandle_numCatchTargetArgsToPassThrough,
                                          "numCatchTargetArgsToPassThrough", "()I") },
        { TR::unknownMethod } };
    ...
    static X GregorianCalendarMethods[]
        = { { x(TR::java_util_GregorianCalendar_computeFields, "computeFields", "(II)I") }, { TR::unknownMethod } };

    static X NativeThreadMethods[]
        = { { x(TR::sun_nio_ch_NativeThread_current, "current", "()J") }, { TR::unknownMethod } };

    static X GPUMethods[] = { { x(TR::com_ibm_gpu_Kernel_blockIdxX, "getBlockIdxX", "()I") },
        ...
        { x(TR::com_ibm_gpu_Kernel_syncThreads, "syncThreads", "()V") }, { TR::unknownMethod } };

#if defined(ENABLE_SPMD_SIMD)
    static X SPMDKernelBaseMethods[] = { { x(TR::com_ibm_simt_SPMDKernel_execute, "execute", "(II)V") },
        { x(TR::com_ibm_simt_SPMDKernel_kernel, "kernel", "()V") }, { TR::unknownMethod } };
#endif

    static X JavaUtilStreamAbstractPipelineMethods[] = { { x(TR::java_util_stream_AbstractPipeline_evaluate, "evaluate",
                                                             "(Ljava/util/stream/TerminalOp;)Ljava/lang/Object;") },
        { TR::unknownMethod } };
    ...
    static X JavaLangInvokeMethodHandleImplMethods[] = {
        ...
        { x(TR::java_lang_invoke_MethodHandleImpl_profileBoolean, "profileBoolean", "(Z[I)Z") }, { TR::unknownMethod }
    };

    static X MTTenantContext[] = { { x(TR::com_ibm_tenant_TenantContext_switchTenant, "switchTenant",
                                       "(Lcom/ibm/tenant/TenantContext;)V") },
    ...
    static X JavaUtilRegexMatcherMethods[]
        = { { x(TR::java_util_regex_Matcher_init, "<init>", "(Ljava/util/regex/Pattern;Ljava/lang/CharSequence;)V") },
              { x(TR::java_util_regex_Matcher_usePattern, "usePattern",
    ...
    static X ArraysSupportMethods[] = { { x(TR::jdk_internal_util_ArraysSupport_vectorizedMismatch,
    ...
    static Y class55[] = {
        { "java/util/concurrent/atomic/AtomicReferenceFieldUpdater",
         JavaUtilConcurrentAtomicReferenceFieldUpdaterMethods },
        { 0 }
    };
    ...
    static Y *recognizedClasses[] = { 0, 0, 0, class13, class14, class15, class16, class17, class18, class19, class20,
        class21, class22, class23, class24, class25, 0, class27, class28, class29, class30, class31, class32, class33,
        class34, class35, class36, 0, class38, class39, class40, class41, class42, class43, class44, class45, class46,
        class47, class48, class49, class50, 0, 0, class53, 0, class55, 0, 0, 0, 0, class60 };

    const int32_t minRecognizedClassLength = 10;
    const int32_t maxRecognizedClassLength
        = sizeof(recognizedClasses) / sizeof(recognizedClasses[0]) + minRecognizedClassLength - 1;

    const int32_t minNonUserClassLength = 17;

    const int32_t maxNonUserClassLength = 17;
    static Y *nonUserClasses[] = { class17 };
        ...
        if (TR::Method::getMandatoryRecognizedMethod() == TR::unknownMethod) {
            ...
            if ((classNameLen == 17) && !strncmp(className, "java/util/TreeMap", 17))
                ...
                if (!strncmp(name, "asType", 6)) {
                    int32_t instanceAsTypeSigLen
                        = strlen("(Ljava/lang/invoke/MethodType;)Ljava/lang/invoke/MethodHandle;");
                    if (sigLen == instanceAsTypeSigLen
                        && !strncmp(sig, "(Ljava/lang/invoke/MethodType;)Ljava/lang/invoke/MethodHandle;",
                            instanceAsTypeSigLen)) {
            ...
            // The classes containing VarHandle operator or access methods also contain methods that provide other
            // functionality. There can be unintended consequences if we were to treat them the same way as VH
            // operator/access methods. The VarHandle operator/access methods we need to recognize all have two things
            // in common - they are static methods and their first parameter is Ljava/lang/invoke/VarHandle;, which is
            // different from the non VH operator methods that we should not be treating the same way. The VH operator
            // methods without the leading VH in the signature are just helper methods that are called from other VH
            // operator methods which fulfil the leading VH parameter check.
                ...
                } else if ((classNameLen == 56
                               && !strncmp(className, "java/lang/invoke/VarHandleBooleans$FieldInstanceReadOnly",
                                   56))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldInstanceReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 57
                               && !strncmp(className, "java/lang/invoke/VarHandleBooleans$FieldInstanceReadWrite",
                                   57))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldInstanceReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 54
                               && !strncmp(className, "java/lang/invoke/VarHandleBooleans$FieldStaticReadOnly", 54))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldStaticReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 55
                               && !strncmp(className, "java/lang/invoke/VarHandleBooleans$FieldStaticReadWrite", 55))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldStaticReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 37 && !strncmp(className, "java/lang/invoke/VarHandleBytes$Array", 37))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_Array_method);
                } else if ((classNameLen == 53
                               && !strncmp(className, "java/lang/invoke/VarHandleBytes$FieldInstanceReadOnly", 53))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldInstanceReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 54
                               && !strncmp(className, "java/lang/invoke/VarHandleBytes$FieldInstanceReadWrite", 54))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldInstanceReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 51
                               && !strncmp(className, "java/lang/invoke/VarHandleBytes$FieldStaticReadOnly", 51))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldStaticReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 52
                               && !strncmp(className, "java/lang/invoke/VarHandleBytes$FieldStaticReadWrite", 52))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldStaticReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 37 && !strncmp(className, "java/lang/invoke/VarHandleChars$Array", 37))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_Array_method);
                } else if ((classNameLen == 53
                               && !strncmp(className, "java/lang/invoke/VarHandleChars$FieldInstanceReadOnly", 53))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldInstanceReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 54
                               && !strncmp(className, "java/lang/invoke/VarHandleChars$FieldInstanceReadWrite", 54))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldInstanceReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 51
                               && !strncmp(className, "java/lang/invoke/VarHandleChars$FieldStaticReadOnly", 51))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldStaticReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 52
                               && !strncmp(className, "java/lang/invoke/VarHandleChars$FieldStaticReadWrite", 52))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldStaticReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 39 && !strncmp(className, "java/lang/invoke/VarHandleDoubles$Array", 39))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_Array_method);
                } else if ((classNameLen == 55
                               && !strncmp(className, "java/lang/invoke/VarHandleDoubles$FieldInstanceReadOnly", 55))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldInstanceReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 56
                               && !strncmp(className, "java/lang/invoke/VarHandleDoubles$FieldInstanceReadWrite",
                                   56))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldInstanceReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 53
                               && !strncmp(className, "java/lang/invoke/VarHandleDoubles$FieldStaticReadOnly", 53))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldStaticReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 54
                               && !strncmp(className, "java/lang/invoke/VarHandleDoubles$FieldStaticReadWrite", 54))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldStaticReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 38 && !strncmp(className, "java/lang/invoke/VarHandleFloats$Array", 38))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_Array_method);
                } else if ((classNameLen == 54
                               && !strncmp(className, "java/lang/invoke/VarHandleFloats$FieldInstanceReadOnly", 54))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldInstanceReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 55
                               && !strncmp(className, "java/lang/invoke/VarHandleFloats$FieldInstanceReadWrite", 55))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldInstanceReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 52
                               && !strncmp(className, "java/lang/invoke/VarHandleFloats$FieldStaticReadOnly", 52))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldStaticReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 53
                               && !strncmp(className, "java/lang/invoke/VarHandleFloats$FieldStaticReadWrite", 53))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldStaticReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 36 && !strncmp(className, "java/lang/invoke/VarHandleInts$Array", 36))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_Array_method);
                } else if ((classNameLen == 52
                               && !strncmp(className, "java/lang/invoke/VarHandleInts$FieldInstanceReadOnly", 52))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldInstanceReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 53
                               && !strncmp(className, "java/lang/invoke/VarHandleInts$FieldInstanceReadWrite", 53))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldInstanceReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 50
                               && !strncmp(className, "java/lang/invoke/VarHandleInts$FieldStaticReadOnly", 50))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldStaticReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 51
                               && !strncmp(className, "java/lang/invoke/VarHandleInts$FieldStaticReadWrite", 51))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldStaticReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 37 && !strncmp(className, "java/lang/invoke/VarHandleLongs$Array", 37))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_Array_method);
                } else if ((classNameLen == 53
                               && !strncmp(className, "java/lang/invoke/VarHandleLongs$FieldInstanceReadOnly", 53))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldInstanceReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 54
                               && !strncmp(className, "java/lang/invoke/VarHandleLongs$FieldInstanceReadWrite", 54))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldInstanceReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 51
                               && !strncmp(className, "java/lang/invoke/VarHandleLongs$FieldStaticReadOnly", 51))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldStaticReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 52
                               && !strncmp(className, "java/lang/invoke/VarHandleLongs$FieldStaticReadWrite", 52))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldStaticReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 42
                               && !strncmp(className, "java/lang/invoke/VarHandleReferences$Array", 42))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_Array_method);
                } else if ((classNameLen == 58
                               && !strncmp(className, "java/lang/invoke/VarHandleReferences$FieldInstanceReadOnly",
                                   58))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldInstanceReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 59
                               && !strncmp(className, "java/lang/invoke/VarHandleReferences$FieldInstanceReadWrite",
                                   59))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldInstanceReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 56
                               && !strncmp(className, "java/lang/invoke/VarHandleReferences$FieldStaticReadOnly",
                                   56))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldStaticReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 57
                               && !strncmp(className, "java/lang/invoke/VarHandleReferences$FieldStaticReadWrite",
                                   57))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldStaticReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 38 && !strncmp(className, "java/lang/invoke/VarHandleShorts$Array", 38))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_Array_method);
                } else if ((classNameLen == 54
                               && !strncmp(className, "java/lang/invoke/VarHandleShorts$FieldInstanceReadOnly", 54))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldInstanceReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 55
                               && !strncmp(className, "java/lang/invoke/VarHandleShorts$FieldInstanceReadWrite", 55))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldInstanceReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 52
                               && !strncmp(className, "java/lang/invoke/VarHandleShorts$FieldStaticReadOnly", 52))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldStaticReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 53
                               && !strncmp(className, "java/lang/invoke/VarHandleShorts$FieldStaticReadWrite", 53))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleX_FieldStaticReadOnlyOrReadWrite_method);
                } else if ((classNameLen == 54
                               && !strncmp(className, "java/lang/invoke/VarHandleByteArrayAsChars$ArrayHandle", 54))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleByteArrayAsX_ArrayHandle_method);
                } else if ((classNameLen == 59
                               && !strncmp(className, "java/lang/invoke/VarHandleByteArrayAsChars$ByteBufferHandle",
                                   59))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleByteArrayAsX_ByteBufferHandle_method);
                } else if ((classNameLen == 56
                               && !strncmp(className, "java/lang/invoke/VarHandleByteArrayAsDoubles$ArrayHandle",
                                   56))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleByteArrayAsX_ArrayHandle_method);
                } else if ((classNameLen == 61
                               && !strncmp(className, "java/lang/invoke/VarHandleByteArrayAsDoubles$ByteBufferHandle",
                                   61))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleByteArrayAsX_ByteBufferHandle_method);
                } else if ((classNameLen == 55
                               && !strncmp(className, "java/lang/invoke/VarHandleByteArrayAsFloats$ArrayHandle", 55))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleByteArrayAsX_ArrayHandle_method);
                } else if ((classNameLen == 60
                               && !strncmp(className, "java/lang/invoke/VarHandleByteArrayAsFloats$ByteBufferHandle",
                                   60))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleByteArrayAsX_ByteBufferHandle_method);
                } else if ((classNameLen == 53
                               && !strncmp(className, "java/lang/invoke/VarHandleByteArrayAsInts$ArrayHandle", 53))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleByteArrayAsX_ArrayHandle_method);
                } else if ((classNameLen == 58
                               && !strncmp(className, "java/lang/invoke/VarHandleByteArrayAsInts$ByteBufferHandle",
                                   58))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleByteArrayAsX_ByteBufferHandle_method);
                } else if ((classNameLen == 54
                               && !strncmp(className, "java/lang/invoke/VarHandleByteArrayAsLongs$ArrayHandle", 54))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleByteArrayAsX_ArrayHandle_method);
                } else if ((classNameLen == 59
                               && !strncmp(className, "java/lang/invoke/VarHandleByteArrayAsLongs$ByteBufferHandle",
                                   59))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleByteArrayAsX_ByteBufferHandle_method);
                } else if ((classNameLen == 55
                               && !strncmp(className, "java/lang/invoke/VarHandleByteArrayAsShorts$ArrayHandle", 55))) {
                    setRecognizedMethodInfo(TR::java_lang_invoke_VarHandleByteArrayAsX_ArrayHandle_method);
                } else if ((classNameLen == 60
                ...
                && !strncmp(className, "java/lang/invoke/ArrayVarHandle$ArrayVarHandleOperations$Op", 59)) {
                setRecognizedMethodInfo(TR::java_lang_invoke_ArrayVarHandle_ArrayVarHandleOperations_OpMethod);
            } else if ((classNameLen >= 71 + 3 && classNameLen <= 71 + 7)
                && !strncmp(className, "java/lang/invoke/StaticFieldVarHandle$StaticFieldVarHandleOperations$Op", 71)) {
                setRecognizedMethodInfo(
                    TR::java_lang_invoke_StaticFieldVarHandle_StaticFieldVarHandleOperations_OpMethod);
            } else if ((classNameLen >= 75 + 3 && classNameLen <= 75 + 7)
                && !strncmp(className, "java/lang/invoke/InstanceFieldVarHandle$InstanceFieldVarHandleOperations$Op",
                    75)) {
                setRecognizedMethodInfo(
                    TR::java_lang_invoke_InstanceFieldVarHandle_InstanceFieldVarHandleOperations_OpMethod);
            } else if ((classNameLen >= 75 + 3 && classNameLen <= 75 + 7)
                && !strncmp(className, "java/lang/invoke/ByteArrayViewVarHandle$ByteArrayViewVarHandleOperations$Op",
                    75)) {
                setRecognizedMethodInfo(
...
bool TR_ResolvedJ9Method::shouldFailSetRecognizedMethodInfoBecauseOfHCR()
{
    ...
    if (compInfo->getPersistentInfo()->getPersistentCHTable()) {
        clazzInfo
            = compInfo->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(clazz, fej9(), true);
    }
...
void TR_ResolvedJ9Method::setRecognizedMethodInfo(TR::RecognizedMethod rm)
{
    setMandatoryRecognizedMethod(rm);

    bool failBecauseOfHCR = false;

    if (!fej9()->isAOT_DEPRECATED_DO_NOT_USE() && TR::Options::getCmdLineOptions()->getOption(TR_EnableHCR)
        && !isNative()) {
        ...
        && !failBecauseOfHCR) // With HCR, non-native methods can change, so we shouldn't "recognize" them
    {
        ...
        bool disableRecMethods = !comp                                                                  ? true
            : (fej9()->getSupportsRecognizedMethods() && !comp->getOption(TR_DisableRecognizedMethods)) ? false
                                                                                                        : true;
        ...
        if (disableRecMethods && fej9()->isAOT_DEPRECATED_DO_NOT_USE()) // AOT_JIT_GAP
        {
            switch (rm) {
                ...
                case TR::sun_misc_Unsafe_putObjectOrdered_jlObjectJjlObject_V: // used in inliner for
                                                                               // TR_InlinerBase::isInlineableJNI and
                                                                               // TR_InlinerBase::inlineUnsafeCall
                ...
                case TR::org_apache_harmony_luni_platform_OSMemory_getAddress_J_J:
                    ...
                    // from bool TR::TreeEvaluator::VMinlineCallEvaluator(TR::Node *node, bool isIndirect,
                    // TR::CodeGenerator *cg)
                    // case TR::sun_misc_Unsafe_copyMemory:
...
J9RAMConstantPoolItem *TR_ResolvedJ9Method::literals()
{
    return (J9RAMConstantPoolItem *)J9_CP_FROM_METHOD(ramMethod());
}

J9Class *TR_ResolvedJ9Method::constantPoolHdr() { return J9_CLASS_FROM_CP(literals()); }

J9ROMClass *TR_ResolvedJ9Method::romClassPtr() { return constantPoolHdr()->romClass; }

TR::Method *TR_ResolvedJ9Method::convertToMethod() { return this; }

uint32_t TR_ResolvedJ9Method::numberOfParameters()
{
    return TR_J9Method::numberOfExplicitParameters() + (isStatic() ? 0 : 1);
}
...
intptr_t TR_ResolvedJ9Method::getInvocationCount() { return TR::CompilationInfo::getInvocationCount(ramMethod()); }

bool TR_ResolvedJ9Method::setInvocationCount(intptr_t oldCount, intptr_t newCount)
{
    return TR::CompilationInfo::setInvocationCount(ramMethod(), oldCount, newCount);
}

bool TR_ResolvedJ9Method::isSameMethod(TR_ResolvedMethod *m2)
{
    TR_ResolvedJ9Method *other
        = (TR_ResolvedJ9Method *)m2; // TODO: Use something safer in the presence of multiple inheritance
...
uint32_t TR_ResolvedJ9Method::classModifiers() { return romClassPtr()->modifiers; }

uint32_t TR_ResolvedJ9Method::classExtraModifiers() { return romClassPtr()->extraModifiers; }

uint32_t TR_ResolvedJ9Method::methodModifiers() { return romMethod()->modifiers; }

bool TR_ResolvedJ9Method::isConstructor() { return (nameLength() == 6 && !strncmp(nameChars(), "<init>", 6)); }
...
bool TR_ResolvedJ9Method::hasBackwardBranches()
{
    return J9ROMMETHOD_HAS_BACKWARDS_BRANCHES(romMethod()) ? true : false;
}

bool TR_ResolvedJ9Method::isObjectConstructor()
{
    return J9ROMMETHOD_IS_OBJECT_CONSTRUCTOR(romMethod()) ? true : false;
}

bool TR_ResolvedJ9Method::isNonEmptyObjectConstructor()
{
    return J9ROMMETHOD_IS_NON_EMPTY_OBJECT_CONSTRUCTOR(romMethod()) ? true : false;
}
...
bool TR_ResolvedJ9Method::isSubjectToPhaseChange(TR::Compilation *comp)
{
    ...
    return comp->getOptLevel() <= warm && comp->getPersistentInfo()->getJitState() == STARTUP_STATE && isPublic()
...
bool TR_ResolvedJ9Method::isFinal()
{
    return (methodModifiers() & J9AccFinal) || (classModifiers() & J9AccFinal) ? true : false;
}

TR_OpaqueClassBlock *TR_ResolvedJ9Method::containingClass()
{
    return _fe->convertClassPtrToClassOffset(constantPoolHdr());
}

U_16 TR_ResolvedJ9Method::numberOfParameterSlots() { return _paramSlots + (isStatic() ? 0 : 1); }

U_16 TR_ResolvedJ9Method::archetypeArgPlaceholderSlot()
{
...
U_16 TR_ResolvedJ9Method::numberOfTemps() { return J9_TEMP_COUNT_FROM_ROM_METHOD(romMethod()); }

U_16 TR_ResolvedJ9Method::numberOfPendingPushes()
{
...
void *TR_ResolvedJ9Method::constantPool() { return literals(); }

J9ClassLoader *TR_ResolvedJ9Method::getClassLoader()
{
...
J9ConstantPool *TR_ResolvedJ9Method::cp() { return (J9ConstantPool *)literals(); }

bool TR_ResolvedJ9Method::isInterpreted()
{
    if (_fe->tossingCode())
        return true;
...
bool TR_ResolvedJ9Method::isInterpretedForHeuristics() { return isInterpreted(); }

TR_OpaqueMethodBlock *TR_ResolvedJ9Method::getNonPersistentIdentifier() { return getPersistentIdentifier(); }

TR_OpaqueMethodBlock *TR_ResolvedJ9Method::getPersistentIdentifier() { return (TR_OpaqueMethodBlock *)ramMethod(); }

void *TR_ResolvedJ9Method::resolvedMethodAddress() { return (void *)getPersistentIdentifier(); }

void *TR_ResolvedJ9Method::startAddressForJittedMethod()
{
...
void TR_ResolvedJ9Method::getFaninInfo(uint32_t *count, uint32_t *weight, uint32_t *otherBucketWeight)
{
...
bool TR_ResolvedJ9Method::getCallerWeight(TR_ResolvedJ9Method *caller, uint32_t *weight, uint32_t pcIndex)
{
    TR_IProfiler *profiler = fej9()->getIProfiler();
...
void *TR_ResolvedJ9Method::startAddressForInterpreterOfJittedMethod()
{
    return TR::CompilationInfo::getJ9MethodStartPC(ramMethod());
}

void *TR_ResolvedJ9Method::startAddressForJITInternalNativeMethod() { return startAddressForJittedMethod(); }

TR_PersistentJittedBodyInfo *TR_ResolvedJ9Method::getExistingJittedBodyInfo()
{
    void *methodAddress = startAddressForInterpreterOfJittedMethod();
    return TR::Recompilation::getJittedBodyInfoFromPC(methodAddress);
}

int32_t TR_ResolvedJ9Method::virtualCallSelector()
{
    return -(int32_t)(vTableSlot() - TR::Compiler->vm.getInterpreterVTableOffset());
}

bool TR_ResolvedJ9Method::virtualMethodIsOverridden()
{
    return (UDATA)ramMethod()->constantPool & J9_STARTPC_METHOD_IS_OVERRIDDEN ? true : false;
}

void TR_ResolvedJ9Method::setVirtualMethodIsOverridden()
{
    UDATA *cp = (UDATA *)&(ramMethod()->constantPool);
    *cp |= J9_STARTPC_METHOD_IS_OVERRIDDEN;
}

void *TR_ResolvedJ9Method::addressContainingIsOverriddenBit() { return &ramMethod()->constantPool; }
...
bool TR_ResolvedJ9Method::isJITInternalNative() { return isNative() && !isJNINative() && !isInterpreted(); }

bool TR_J9MethodBase::isUnsafeCAS()
{
    TR::RecognizedMethod rm = getRecognizedMethod();
    switch (rm) {
        case TR::jdk_internal_misc_Unsafe_compareAndExchangeInt:
...
bool TR_J9MethodBase::isUnsafeWithObjectArg()
{
    TR::RecognizedMethod rm = getRecognizedMethod();
    switch (rm) {
        case TR::sun_misc_Unsafe_putByte_jlObjectJB_V:
...
bool TR_J9MethodBase::isUnsafeGetPutWithObjectArg(TR::RecognizedMethod rm)
{
    switch (rm) {
        case TR::sun_misc_Unsafe_putByte_jlObjectJB_V:
        case TR::sun_misc_Unsafe_putBoolean_jlObjectJZ_V:
...
TR::DataType TR_J9MethodBase::unsafeDataTypeForObject(TR::RecognizedMethod rm)
{
    switch (rm) {
        case TR::sun_misc_Unsafe_getBoolean_jlObjectJ_Z:
        case TR::sun_misc_Unsafe_putBoolean_jlObjectJZ_V:
...
TR::DataType TR_J9MethodBase::unsafeDataTypeForArray(TR::RecognizedMethod rm)
{
    switch (rm) {
        case TR::sun_misc_Unsafe_getBoolean_jlObjectJ_Z:
        case TR::sun_misc_Unsafe_putBoolean_jlObjectJZ_V:
...
bool TR_J9MethodBase::isVolatileUnsafe(TR::RecognizedMethod rm)
{
    switch (rm) {
        case TR::sun_misc_Unsafe_getBooleanVolatile_jlObjectJ_Z:
        case TR::sun_misc_Unsafe_getByteVolatile_jlObjectJ_B:
...
bool TR_J9MethodBase::isAcquireReleaseUnsafe(TR::RecognizedMethod rm)
{
    switch (rm) {
        case TR::jdk_internal_misc_Unsafe_getBooleanAcquire_jlObjectJ_Z:
        case TR::sun_misc_Unsafe_putBooleanOrdered_jlObjectJZ_V:
...
bool TR_J9MethodBase::isOpaqueUnsafe(TR::RecognizedMethod rm)
{
    switch (rm) {
        case TR::jdk_internal_misc_Unsafe_getBooleanOpaque_jlObjectJ_Z:
        case TR::jdk_internal_misc_Unsafe_putBooleanOpaque_jlObjectJZ_V:
...
bool TR_J9MethodBase::isUnsafeGetPutBoolean(TR::RecognizedMethod rm)
{
    switch (rm) {
        case TR::sun_misc_Unsafe_getBoolean_jlObjectJ_Z:
        case TR::sun_misc_Unsafe_getBooleanVolatile_jlObjectJ_Z:
...
bool TR_J9MethodBase::isUnsafePut(TR::RecognizedMethod rm)
{
    switch (rm) {
        case TR::sun_misc_Unsafe_getAndAddInt:
        case TR::sun_misc_Unsafe_getAndAddLong:
...
bool TR_J9MethodBase::isVarHandleOperationMethod(TR::RecognizedMethod rm)
{
    switch (rm) {
#if defined(J9VM_OPT_OPENJDK_METHODHANDLE)
...
bool TR_J9MethodBase::isVarHandleAccessMethod()
{
...
bool TR_J9MethodBase::isSignaturePolymorphicMethod()
{
    if (isVarHandleAccessMethod())
        return true;
...
bool TR_ResolvedJ9Method::methodIsNotzAAPEligible()
{
    return ((UDATA)ramMethod()->constantPool & J9_STARTPC_NATIVE_REQUIRES_SWITCHING) ? true : false;
}

U_8 *TR_ResolvedJ9Method::allocateException(uint32_t numBytes, TR::Compilation *comp)
{
...
bool TR_ResolvedJ9Method::isNewInstanceImplThunk()
{
    return (_j9classForNewInstance != NULL);
    // return getNewInstancePrototype(_fe->vmThread()) == ramMethod();
}

char *TR_ResolvedJ9Method::fieldOrStaticNameChars(I_32 cpIndex, int32_t &len)
{
    return cpIndex >= 0
        ? utf8Data(J9ROMNAMEANDSIGNATURE_NAME(J9ROMFIELDREF_NAMEANDSIGNATURE(&romCPBase()[cpIndex])), len)
        : 0;
}

char *TR_ResolvedJ9Method::fieldNameChars(I_32 cpIndex, int32_t &len) { return fieldOrStaticNameChars(cpIndex, len); }

char *TR_ResolvedJ9Method::staticNameChars(I_32 cpIndex, int32_t &len) { return fieldOrStaticNameChars(cpIndex, len); }

char *TR_ResolvedJ9Method::fieldOrStaticSignatureChars(I_32 cpIndex, int32_t &len)
{
    return cpIndex >= 0
        ? utf8Data(J9ROMNAMEANDSIGNATURE_SIGNATURE(J9ROMFIELDREF_NAMEANDSIGNATURE(&romCPBase()[cpIndex])), len)
        : 0;
}

char *TR_ResolvedJ9Method::fieldSignatureChars(I_32 cpIndex, int32_t &len)
{
    return cpIndex > 0 ? fieldOrStaticSignatureChars(cpIndex, len) : 0;
}

char *TR_ResolvedJ9Method::staticSignatureChars(I_32 cpIndex, int32_t &len)
{
    return cpIndex >= 0 ? fieldOrStaticSignatureChars(cpIndex, len) : 0;
}

TR_OpaqueClassBlock *TR_ResolvedJ9Method::getClassOfStaticFromCP(TR_J9VMBase *fej9, J9ConstantPool *cp, int32_t cpIndex)
{
    TR::VMAccessCriticalSection classOfStatic(fej9);
    TR_OpaqueClassBlock *result;
    result = fej9->convertClassPtrToClassOffset(
        cpIndex >= 0 ? jitGetClassOfFieldFromCP(fej9->vmThread(), cp, cpIndex) : 0);
...
TR_OpaqueClassBlock *TR_ResolvedJ9Method::classOfStatic(I_32 cpIndex, bool returnClassForAOT)
{
    return getClassOfStaticFromCP(fej9(), cp(), cpIndex);
}

TR_OpaqueClassBlock *TR_ResolvedJ9Method::classOfMethod()
{
    if (isNewInstanceImplThunk()) {
        TR_ASSERT(_j9classForNewInstance, "Must have the class for the newInstance");
...
uint32_t TR_ResolvedJ9Method::classCPIndexOfMethod(uint32_t methodCPIndex)
{
...
void *&TR_ResolvedJ9Method::addressOfClassOfMethod()
{
    if (isNewInstanceImplThunk()) {
...
void TR_ResolvedJ9Method::setClassForNewInstance(J9Class *c) { _j9classForNewInstance = c; }

I_32 TR_ResolvedJ9Method::exceptionData(I_32 exceptionNumber, I_32 *startIndex, I_32 *endIndex, I_32 *catchType)
{
    J9ExceptionHandler *exceptionHandler = J9EXCEPTIONINFO_HANDLERS(J9_EXCEPTION_DATA_FROM_ROM_METHOD(romMethod()));
    return TR_ResolvedJ9MethodBase::exceptionData(exceptionHandler, 0, exceptionNumber, startIndex, endIndex,
        catchType);
}

U_32 TR_ResolvedJ9Method::numberOfExceptionHandlers()
{
    return J9ROMMETHOD_HAS_EXCEPTION_INFO(romMethod()) ? J9_EXCEPTION_DATA_FROM_ROM_METHOD(romMethod())->catchCount : 0;
}

TR::DataType TR_ResolvedJ9Method::getLDCType(I_32 cpIndex)
{
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
...
bool TR_ResolvedJ9Method::isClassConstant(int32_t cpIndex)
{
    UDATA cpType = J9_CP_TYPE(J9ROMCLASS_CPSHAPEDESCRIPTION(romClassPtr()), cpIndex);
    return cpType == J9CPTYPE_CLASS;
}

bool TR_ResolvedJ9Method::isStringConstant(int32_t cpIndex)
{
    UDATA cpType = J9_CP_TYPE(J9ROMCLASS_CPSHAPEDESCRIPTION(romClassPtr()), cpIndex);
    return (cpType == J9CPTYPE_STRING) || (cpType == J9CPTYPE_ANNOTATION_UTF8);
}

bool TR_ResolvedJ9Method::isMethodTypeConstant(int32_t cpIndex)
{
    UDATA cpType = J9_CP_TYPE(J9ROMCLASS_CPSHAPEDESCRIPTION(romClassPtr()), cpIndex);
    return cpType == J9CPTYPE_METHOD_TYPE;
}

bool TR_ResolvedJ9Method::isMethodHandleConstant(int32_t cpIndex)
{
    UDATA cpType = J9_CP_TYPE(J9ROMCLASS_CPSHAPEDESCRIPTION(romClassPtr()), cpIndex);
    return cpType == J9CPTYPE_METHODHANDLE;
}

SYS_FLOAT *TR_ResolvedJ9Method::doubleConstant(I_32 cpIndex, TR_Memory *m)
{
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
    return (SYS_FLOAT *)&romLiterals()[cpIndex];
}

uint64_t TR_ResolvedJ9Method::longConstant(I_32 cpIndex) { return *((uint64_t *)&romLiterals()[cpIndex]); }

void *TR_ResolvedJ9Method::getConstantDynamicTypeFromCP(I_32 cpIndex)
{
    return jitGetConstantDynamicTypeFromCP(fej9()->vmThread(), cp(), cpIndex);
}

bool TR_ResolvedJ9Method::isConstantDynamic(I_32 cpIndex)
{
...
bool TR_ResolvedJ9Method::isUnresolvedConstantDynamic(I_32 cpIndex)
{
...
void *TR_ResolvedJ9Method::dynamicConstant(I_32 cpIndex, uintptr_t *obj)
{
    TR_ASSERT_FATAL(cpIndex != -1, "ConstantDynamic cpIndex shouldn't be -1");
...
bool TR_ResolvedJ9Method::isUnresolvedString(I_32 cpIndex, bool optimizeForAOT)
{
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
    return (bool)(((J9RAMStringRef *)literals())[cpIndex].stringObject == 0);
}

float *TR_ResolvedJ9Method::floatConstant(I_32 cpIndex)
{
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
    return ((float *)&romLiterals()[cpIndex]);
}

U_32 TR_ResolvedJ9Method::intConstant(I_32 cpIndex)
{
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
    return *((U_32 *)&romLiterals()[cpIndex]);
}

void *TR_ResolvedJ9Method::stringConstant(I_32 cpIndex)
{
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
    return &((J9RAMStringRef *)literals())[cpIndex].stringObject;
}

void *TR_ResolvedJ9Method::methodTypeConstant(I_32 cpIndex)
{
#if defined(J9VM_OPT_JITSERVER)
...
bool TR_ResolvedJ9Method::isUnresolvedMethodType(I_32 cpIndex)
{
#if defined(J9VM_OPT_JITSERVER)
    TR_ASSERT(!TR::CompilationInfo::getStream(), "no server");
#endif /* defined(J9VM_OPT_JITSERVER) */
...
void *TR_ResolvedJ9Method::methodHandleConstant(I_32 cpIndex)
{
#if defined(J9VM_OPT_JITSERVER)
    TR_ASSERT(!TR::CompilationInfo::getStream(), "no server");
#endif /* defined(J9VM_OPT_JITSERVER) */
...
bool TR_ResolvedJ9Method::isUnresolvedMethodHandle(I_32 cpIndex)
{
#if defined(J9VM_OPT_JITSERVER)
    TR_ASSERT(!TR::CompilationInfo::getStream(), "no server");
#endif /* defined(J9VM_OPT_JITSERVER) */
...
void *TR_ResolvedJ9Method::callSiteTableEntryAddress(int32_t callSiteIndex)
{
    J9Class *ramClass = constantPoolHdr();
    return ramClass->callSites + callSiteIndex;
}

bool TR_ResolvedJ9Method::isUnresolvedCallSiteTableEntry(int32_t callSiteIndex)
{
    return *(j9object_t *)callSiteTableEntryAddress(callSiteIndex) == NULL;
}

#if defined(J9VM_OPT_METHOD_HANDLE)
void *TR_ResolvedJ9Method::varHandleMethodTypeTableEntryAddress(int32_t cpIndex)
{
#if defined(J9VM_OPT_JITSERVER)
    TR_ASSERT(!TR::CompilationInfo::getStream(), "no server");
#endif /* defined(J9VM_OPT_JITSERVER) */
    ...
    TR_ASSERT(varHandleMethodTypeLookupTable[methodTypeIndex] == cpIndex,
        "Could not find the VarHandle callsite MethodType");

    return ramClass->varHandleMethodTypes + methodTypeIndex;
}

bool TR_ResolvedJ9Method::isUnresolvedVarHandleMethodTypeTableEntry(int32_t cpIndex)
{
    return *(j9object_t *)varHandleMethodTypeTableEntryAddress(cpIndex) == NULL;
}
#endif /* defined(J9VM_OPT_METHOD_HANDLE) */

void *TR_ResolvedJ9Method::methodTypeTableEntryAddress(int32_t cpIndex)
{
    J9Class *ramClass = constantPoolHdr();
...
bool TR_ResolvedJ9Method::isUnresolvedMethodTypeTableEntry(int32_t cpIndex)
{
    return *(j9object_t *)methodTypeTableEntryAddress(cpIndex) == NULL;
}

TR_OpaqueClassBlock *TR_ResolvedJ9Method::getClassFromCP(TR_J9VMBase *fej9, J9ConstantPool *cp, TR::Compilation *comp,
    uint32_t cpIndex)
{
    ...
    if (cpIndex != -1
        ...
        && (resolvedClass = fej9->_vmFunctionTable->resolveClassRef(fej9->vmThread(), cp, cpIndex,
                J9_RESOLVE_FLAG_JIT_COMPILE_TIME))) {
...
bool TR_ResolvedJ9Method::isStable(int32_t cpIndex, TR::Compilation *comp)
{
...
TR_OpaqueClassBlock *TR_ResolvedJ9Method::getClassFromConstantPool(TR::Compilation *comp, uint32_t cpIndex, bool)
{
...
bool TR_ResolvedJ9Method::validateClassFromConstantPool(TR::Compilation *comp, J9Class *clazz, uint32_t cpIndex,
    TR_ExternalRelocationTargetKind reloKind)
{
    return true;
}

bool TR_ResolvedJ9Method::validateArbitraryClass(TR::Compilation *comp, J9Class *clazz) { return true; }

char *TR_ResolvedJ9Method::getClassNameFromConstantPool(uint32_t cpIndex, uint32_t &length)
{
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
    return utf8Data(J9ROMCLASSREF_NAME((J9ROMClassRef *)&romLiterals()[cpIndex]), length);
}

char *TR_ResolvedJ9Method::getMethodSignatureFromConstantPool(I_32 cpIndex, int32_t &len)
{
...
char *TR_ResolvedJ9Method::getMethodNameFromConstantPool(int32_t cpIndex, int32_t &len)
{
    I_32 realCPIndex = jitGetRealCPIndex(_fe->vmThread(), romClassPtr(), cpIndex);
    if (realCPIndex == -1)
        return 0;
...
const char *TR_ResolvedJ9Method::newInstancePrototypeSignature(TR_Memory *m, TR_AllocationKind allocKind)
{
    int32_t clen;
    TR_ASSERT(_j9classForNewInstance, "Must have the class for newInstance");
    J9Class *clazz
        = _j9classForNewInstance; //((J9Class*)((uintptr_t)(ramMethod()->extra) & ~J9_STARTPC_NOT_TRANSLATED);
...
U_32 TR_ResolvedJ9Method::vTableSlot() { return _vTableSlot; }

bool TR_ResolvedJ9Method::isCompilable(TR_Memory *trMemory)
{
...
bool TR_ResolvedJ9Method::isInvokePrivateVTableOffset(UDATA vTableOffset)
{
    return vTableOffset == J9VTABLE_INVOKE_PRIVATE_OFFSET;
}

TR_OpaqueMethodBlock *TR_ResolvedJ9Method::getVirtualMethod(TR_J9VMBase *fej9, J9ConstantPool *cp, I_32 cpIndex,
    UDATA *vTableOffset, bool *unresolvedInCP)
{
    ...
    *vTableOffset >>= 8;
    if (J9VTABLE_INITIAL_VIRTUAL_OFFSET == *vTableOffset) {
        ...
        *vTableOffset = fej9->_vmFunctionTable->resolveVirtualMethodRefInto(fej9->vmThread(), cp, cpIndex,
...
TR_OpaqueClassBlock *TR_ResolvedJ9Method::getInterfaceITableIndexFromCP(TR_J9VMBase *fej9, J9ConstantPool *cp,
    int32_t cpIndex, UDATA *pITableIndex)
{
    if (cpIndex == -1)
        return NULL;
...
TR_OpaqueClassBlock *TR_ResolvedJ9Method::getResolvedInterfaceMethod(I_32 cpIndex, UDATA *pITableIndex)
{
    TR_OpaqueClassBlock *result;
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
...
U_32 TR_ResolvedJ9Method::getResolvedInterfaceMethodOffset(TR_OpaqueClassBlock *classObject, I_32 cpIndex)
{
...
#else
    {
        ...
        vTableOffset = jitGetInterfaceVTableOffsetFromCP(_fe->vmThread(), cp(), cpIndex,
 ...
 */
TR_ResolvedMethod *TR_ResolvedJ9Method::getResolvedImproperInterfaceMethod(TR::Compilation *comp, I_32 cpIndex)
{
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
#if TURN_OFF_INLINING
    return 0;
    ...
    if (comp->getOption(TR_UseSymbolValidationManager) && j9method) {
        if (!comp->getSymbolValidationManager()->addImproperInterfaceMethodFromCPRecord(
                (TR_OpaqueMethodBlock *)j9method, cp(), cpIndex))
            j9method = NULL;
    }
...
TR_ResolvedMethod *TR_ResolvedJ9Method::getResolvedInterfaceMethod(TR::Compilation *comp,
    TR_OpaqueClassBlock *classObject, I_32 cpIndex)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)_fe;
    J9Method *ramMethod = (J9Method *)fej9->getResolvedInterfaceMethod(getPersistentIdentifier(), classObject, cpIndex);
    ...
    if (ramMethod && J9_BYTECODE_START_FROM_RAM_METHOD(ramMethod)) {
        ...
        if (m) {
            ...
            if (c) {
                ...
                TR::DebugCounter::incStaticDebugCounter(comp, "resources.resolvedMethods/interface:#bytes",
                    sizeof(TR_ResolvedJ9Method));
                return m;
            }
        }
    }
...
TR_ResolvedMethod *TR_ResolvedJ9Method::getResolvedStaticMethod(TR::Compilation *comp, I_32 cpIndex,
    bool *unresolvedInCP)
{
    TR_ResolvedMethod *resolvedMethod = NULL;
    ...
    if (comp->compileRelocatableCode() && comp->getOption(TR_UseSymbolValidationManager) && ramMethod) {
        if (!comp->getSymbolValidationManager()->addStaticMethodFromCPRecord((TR_OpaqueMethodBlock *)ramMethod, cp(),
                cpIndex))
            ramMethod = NULL;
    }
...
TR_ResolvedMethod *TR_ResolvedJ9Method::getResolvedSpecialMethod(TR::Compilation *comp, I_32 cpIndex,
    bool *unresolvedInCP)
{
    TR_ResolvedMethod *resolvedMethod = NULL;
    ...
    if (!((_fe->_jitConfig->runtimeFlags & J9JIT_RUNTIME_RESOLVE)
        ...
        J9Method *ramMethod
            = jitResolveSpecialMethodRef(_fe->vmThread(), cp(), cpIndex, J9_RESOLVE_FLAG_JIT_COMPILE_TIME);
        if (ramMethod) {
            bool createResolvedMethod = true;

            if (comp->getOption(TR_UseSymbolValidationManager)) {
                if (!comp->getSymbolValidationManager()->addSpecialMethodFromCPRecord((TR_OpaqueMethodBlock *)ramMethod,
                        cp(), cpIndex))
...
bool TR_ResolvedJ9Method::shouldCompileTimeResolveMethod(I_32 cpIndex)
{
    ...
    if (classNameLength == strlen(JSR292_MethodHandle) && !strncmp(className, JSR292_MethodHandle, classNameLength)) {
        ...
        //
        if ((methodNameLength == 11 && !strncmp(methodName, "invokeBasic", methodNameLength))
            || (methodNameLength == 12
            ...
            || (methodNameLength == 15 && !strncmp(methodName, "linkToInterface", methodNameLength)))
            return true;
    }
...
TR_ResolvedMethod *TR_ResolvedJ9Method::getResolvedPossiblyPrivateVirtualMethod(TR::Compilation *comp, I_32 cpIndex,
    bool ignoreRtResolve, bool *unresolvedInCP)
{
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
#if TURN_OFF_INLINING
    return 0;
    ...
    if (!((_fe->_jitConfig->runtimeFlags & J9JIT_RUNTIME_RESOLVE)
        ...
        || ignoreRtResolve || shouldCompileTimeResolve) {
        ...
        if (comp->compileRelocatableCode() && ramMethod && comp->getOption(TR_UseSymbolValidationManager)) {
            if (!comp->getSymbolValidationManager()->addVirtualMethodFromCPRecord((TR_OpaqueMethodBlock *)ramMethod,
                    cp(), cpIndex))
                createResolvedMethod = false;
        }
    ...
    TR_ASSERT_FATAL(resolvedMethod || !shouldCompileTimeResolve, "Method has to be resolved in %s at cpIndex  %d",
        signature(comp->trMemory()), cpIndex);
    ...
    if (resolvedMethod == NULL) {
        ...
        TR::DebugCounter::incStaticDebugCounter(comp, "resources.resolvedMethods/virtual:#bytes",
            sizeof(TR_ResolvedJ9Method));
    }
...
TR_ResolvedMethod *TR_ResolvedJ9Method::getResolvedVirtualMethod(TR::Compilation *comp, I_32 cpIndex,
    bool ignoreRtResolve, bool *unresolvedInCP)
{
    TR_ResolvedMethod *method = getResolvedPossiblyPrivateVirtualMethod(comp, cpIndex, ignoreRtResolve, unresolvedInCP);

    return (method == NULL || method->isPrivate()) ? NULL : method;
}

TR_ResolvedMethod *TR_ResolvedJ9Method::createResolvedMethodFromJ9Method(TR::Compilation *comp, I_32 cpIndex,
    uint32_t vTableSlot, J9Method *j9Method, TR_AOTInliningStats *aotStats)
{
    TR_ResolvedMethod *m = new (comp->trHeapMemory())
        TR_ResolvedJ9Method((TR_OpaqueMethodBlock *)j9Method, _fe, comp->trMemory(), this, vTableSlot);
...
void TR_ResolvedJ9Method::handleUnresolvedStaticMethodInCP(int32_t cpIndex, bool *unresolvedInCP) {}

void TR_ResolvedJ9Method::handleUnresolvedSpecialMethodInCP(int32_t cpIndex, bool *unresolvedInCP) {}

void TR_ResolvedJ9Method::handleUnresolvedVirtualMethodInCP(int32_t cpIndex, bool *unresolvedInCP) {}

TR_OpaqueMethodBlock *TR_ResolvedJ9Method::getTargetMethodFromMemberName(uintptr_t *invokeCacheArray,
    bool *isInvokeCacheAppendixNull)
{
    ...
    targetJ9MethodBlock = fej9()->targetMethodFromMemberName(
        (uintptr_t)fej9()->getReferenceElement(*invokeCacheArray, JSR292_invokeCacheArrayMemberNameIndex));
...
TR_ResolvedMethod *TR_ResolvedJ9Method::getResolvedDynamicMethod(TR::Compilation *comp, I_32 callSiteIndex,
    bool *unresolvedInCP, bool *isInvokeCacheAppendixNull)
{
    ...
    if (!isUnresolvedEntry) {
        ...
        // invokedynamic resolution can either result in a valid entry in the corresponding CallSite table slot if
        // successful, or an exception object otherwise. The CallSite table entry is a two-element array containing the
        // MemberName and appendix objects necessary for constructing a resolved invokedynamic adapter method call. The
        // exception object would not be an array type object, and therefore it is sufficient to check if the entry
        // obtained from the CallSite table is an array instance to determine if the resolution was successful.
        ...
        TR_OpaqueMethodBlock *targetJ9MethodBlock
            = getTargetMethodFromMemberName(invokeCacheArray, &invokeCacheAppendixNull);
        ...
        if (comp->compileRelocatableCode()) {
            ...
            if (!valid)
                comp->failCompilation<J9::AOTHasInvokeHandle>(
                    "Failed to add validation record for resolved dynamic method %p", targetJ9MethodBlock);
        }
        ...
        if (!dummyInvoke && comp->compileRelocatableCode())
            comp->failCompilation<J9::AOTHasInvokeHandle>(
                "getResolvedDynamicMethod: Failed to get method %s.%s%s from name", className, methodName,
                methodSignature);
        ...
        char *linkToStaticSignature
            = _fe->getSignatureForLinkToStaticForInvokeDynamic(comp, signature, signatureLength);
        result = _fe->createResolvedMethodWithSignature(comp->trMemory(), dummyInvoke, NULL, linkToStaticSignature,
            signatureLength, this);
    }

    if (isInvokeCacheAppendixNull)
        *isInvokeCacheAppendixNull = invokeCacheAppendixNull;
#else
    TR_OpaqueMethodBlock *dummyInvokeExact
        = _fe->getMethodFromName("java/lang/invoke/MethodHandle", "invokeExact", JSR292_invokeExactSig);
    result = _fe->createResolvedMethodWithSignature(comp->trMemory(), dummyInvokeExact, NULL, utf8Data(signature),
        J9UTF8_LENGTH(signature), this);
#endif /* J9VM_OPT_OPENJDK_METHODHANDLE */
    return result;
#endif /* TURN_OFF_INLINING */
}

TR_ResolvedMethod *TR_ResolvedJ9Method::getResolvedHandleMethod(TR::Compilation *comp, I_32 cpIndex,
    bool *unresolvedInCP, bool *isInvokeCacheAppendixNull)
{
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
    ...
    if (!isUnresolvedEntry) {
        ...
        if (comp->compileRelocatableCode()) {
            bool valid = comp->getSymbolValidationManager()->addHandleMethodFromCPIndex(targetJ9MethodBlock,
                getNonPersistentIdentifier(), cpIndex, invokeCacheAppendixNull);

            if (!valid)
                comp->failCompilation<J9::AOTHasInvokeHandle>(
                    "Failed to add validation record for resolved handle method %p", targetJ9MethodBlock);
        }
        ...
        if (!dummyInvoke && comp->compileRelocatableCode())
            comp->failCompilation<J9::AOTHasInvokeHandle>(
                "getResolvedHandleMethod: Failed to get method %s.%s%s from name", className, methodName,
                methodSignature);
    ...
    int32_t signatureLength;
    char *signature = utf8Data(J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSig), signatureLength);
    result = _fe->createResolvedMethodWithSignature(comp->trMemory(), dummyInvokeExact, NULL, signature,
        signatureLength, this);
#endif /* J9VM_OPT_OPENJDK_METHODHANDLE */
    return result;
#endif /* TURN_OFF_INLINING */
...
TR_ResolvedMethod *TR_ResolvedJ9Method::getResolvedHandleMethodWithSignature(TR::Compilation *comp, I_32 cpIndex,
    char *signature)
{
#if TURN_OFF_INLINING
    return 0;
    ...
    TR_OpaqueMethodBlock *dummyInvokeExact
        = _fe->getMethodFromName("java/lang/invoke/MethodHandle", "invokeExact", JSR292_invokeExactSig);
    TR_ResolvedMethod *resolvedMethod = _fe->createResolvedMethodWithSignature(comp->trMemory(), dummyInvokeExact, NULL,
        signature, strlen(signature), this);
    return resolvedMethod;
#endif
}

TR_ResolvedMethod *TR_ResolvedJ9Method::getResolvedVirtualMethod(TR::Compilation *comp,
    ...
    if (_fe->isAOT_DEPRECATED_DO_NOT_USE()) {
        m = ramMethod
            ? new (comp->trHeapMemory())
                  TR_ResolvedRelocatableJ9Method((TR_OpaqueMethodBlock *)ramMethod, _fe, comp->trMemory(), this)
            : 0;
    } else {
        m = ramMethod ? new (comp->trHeapMemory())
                            TR_ResolvedJ9Method((TR_OpaqueMethodBlock *)ramMethod, _fe, comp->trMemory(), this)
                      : 0;
    }
    return m;
}

bool TR_ResolvedJ9Method::fieldsAreSame(I_32 cpIndex1, TR_ResolvedMethod *m2, I_32 cpIndex2, bool &sigSame)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)_fe;
    ...
    if (utf8Matches(J9ROMNAMEANDSIGNATURE_NAME(nameAndSignature1), J9ROMNAMEANDSIGNATURE_NAME(nameAndSignature2))
        && utf8Matches(J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSignature1),
            J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSignature2))) {
...
bool TR_ResolvedJ9Method::staticsAreSame(I_32 cpIndex1, TR_ResolvedMethod *m2, I_32 cpIndex2, bool &sigSame)
{
    TR_J9VMBase *fej9 = (TR_J9VMBase *)_fe;
    if (!fej9->sameClassLoaders(classOfMethod(), m2->classOfMethod()))
        return false;
    ...
    if (J9RAMSTATICFIELDREF_IS_RESOLVED(ramCPBase1 + cpIndex1)
        && J9RAMSTATICFIELDREF_IS_RESOLVED(ramCPBase2 + cpIndex2)) {
...
bool TR_ResolvedJ9Method::fieldAttributes(TR::Compilation *comp, I_32 cpIndex, U_32 *fieldOffset, TR::DataType *type,
    bool *volatileP, bool *isFinal, bool *isPrivate, bool isStore, bool *unresolvedInCP, bool needAOTValidation)
{
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
    ...
    bool isColdOrReducedWarm
        = (comp->getMethodHotness() < warm) || (comp->getMethodHotness() == warm && comp->getOption(TR_NoOptServer));
    ...
    if (offset >= 0 && !dontResolveJITField
        && (!(_fe->_jitConfig->runtimeFlags & J9JIT_RUNTIME_RESOLVE)
            || // TODO: Probably more useful not to mark JSR292-related fields as unresolved
        ...
        *fieldOffset = offset + TR::Compiler->om.objectHeaderSizeInBytes(); // add header size
        ...
        if (isFinal)
            *isFinal = (ltype & J9AccFinal) ? true : false;
        if (isPrivate)
            *isPrivate = (ltype & J9AccPrivate) ? true : false;
    } else {
        resolved = false;
...
bool TR_ResolvedJ9Method::staticAttributes(TR::Compilation *comp, I_32 cpIndex, void **address, TR::DataType *type,
    bool *volatileP, bool *isFinal, bool *isPrivate, bool isStore, bool *unresolvedInCP, bool needAOTValidation)
{
    TR_ASSERT(cpIndex != -1, "cpIndex shouldn't be -1");
    ...
    if (!doRuntimeResolveForEarlyCompilation) {
        TR::VMAccessCriticalSection resolveForEarlyCompilation(fej9());
        backingStorage
            = jitCTResolveStaticFieldRefWithMethod(_fe->vmThread(), ramMethod(), cpIndex, isStore, &fieldShape);
    ...
    static const char *dontResolveJITStaticField = feGetEnv("TR_JITDontResolveStaticField");

    if (backingStorage && !dontResolveJITStaticField
        && (!(_fe->_jitConfig->runtimeFlags & J9JIT_RUNTIME_RESOLVE) || isMethodHandleThunk || // cmvc 195373
            !performTransformation(comp, "Setting as unresolved static attributes cpIndex=%d\n", cpIndex))) {
        resolved = true;
        ltype = fieldShape->modifiers;
        *volatileP = (ltype & J9AccVolatile) ? true : false;
...
TR_OpaqueClassBlock *TR_ResolvedJ9Method::definingClassFromCPFieldRef(TR::Compilation *comp,
    J9ConstantPool *constantPool, I_32 cpIndex, bool isStatic)
{
#if defined(J9VM_OPT_JITSERVER)
    TR_ASSERT_FATAL(!comp->isOutOfProcessCompilation(),
        "Static version of definingClassFromCPFieldRef should not be called in JITServer mode");
#endif
    J9ROMFieldShape *field;
    return definingClassAndFieldShapeFromCPFieldRef(comp, constantPool, cpIndex, isStatic, &field);
}

TR_OpaqueClassBlock *TR_ResolvedJ9Method::definingClassAndFieldShapeFromCPFieldRef(TR::Compilation *comp,
    J9ConstantPool *constantPool, I_32 cpIndex, bool isStatic, J9ROMFieldShape **field)
{
    ...
    J9Class *resolvedClass = javaVM->internalVMFunctions->resolveClassRef(vmThread, constantPool,
        romFieldRef->classRefCPIndex, J9_RESOLVE_FLAG_JIT_COMPILE_TIME);
    ...
    if (isStatic) {
        void *staticAddress = javaVM->internalVMFunctions->staticFieldAddress(vmThread, resolvedClass,
            J9UTF8_DATA(name), J9UTF8_LENGTH(name), J9UTF8_DATA(signature), J9UTF8_LENGTH(signature), &definingClass,
            (UDATA *)field, J9_LOOK_NO_JAVA, classFromCP);
    } else {
        IDATA fieldOffset = javaVM->internalVMFunctions->instanceFieldOffset(vmThread, resolvedClass, J9UTF8_DATA(name),
            J9UTF8_LENGTH(name), J9UTF8_DATA(signature), J9UTF8_LENGTH(signature), &definingClass, (UDATA *)field,
            J9_LOOK_NO_JAVA);
    }
...
TR_OpaqueClassBlock *TR_ResolvedJ9Method::definingClassFromCPFieldRef(TR::Compilation *comp, I_32 cpIndex,
    bool isStatic, TR_OpaqueClassBlock **fromResolvedJ9Method)
{
...
bool TR_ResolvedJ9Method::fieldIsFromLocalClass(int32_t cpIndex)
{
...
void TR_ResolvedJ9Method::makeParameterList(TR::ResolvedMethodSymbol *methodSym)
{
    ...
    if (methodSym->isStatic()) {
        ...
        parmSymbol
            = methodSym->comp()->getSymRefTab()->createParameterSymbol(methodSym, 0, TR::Address, knownObjectIndex);
...
TR_OpaqueMethodBlock *TR_J9VMBase::getResolvedVirtualMethod(TR_OpaqueClassBlock *classObject, I_32 virtualCallOffset,
...
TR_OpaqueMethodBlock *TR_J9VMBase::getResolvedInterfaceMethod(J9ConstantPool *ownerCP, TR_OpaqueClassBlock *classObject,
    int32_t cpIndex)
{
...
TR_OpaqueMethodBlock *TR_J9VMBase::getResolvedInterfaceMethod(TR_OpaqueMethodBlock *interfaceMethod,
    TR_OpaqueClassBlock *classObject, I_32 cpIndex)
{
    return getResolvedInterfaceMethod((J9ConstantPool *)(J9_CP_FROM_METHOD((J9Method *)interfaceMethod)), classObject,
        cpIndex);
}

TR_OpaqueMethodBlock *TR_J9SharedCacheVM::getResolvedVirtualMethod(TR_OpaqueClassBlock *classObject,
    I_32 virtualCallOffset, bool ignoreRtResolve)
{
    TR_OpaqueMethodBlock *ramMethod
        = TR_J9VMBase::getResolvedVirtualMethod(classObject, virtualCallOffset, ignoreRtResolve);
    TR::Compilation *comp = TR::comp();
    if (comp && comp->getOption(TR_UseSymbolValidationManager)) {
        if (!comp->getSymbolValidationManager()->addVirtualMethodFromOffsetRecord(ramMethod, classObject,
                virtualCallOffset, ignoreRtResolve))
            return NULL;
    }
...
TR_OpaqueMethodBlock *TR_J9SharedCacheVM::getResolvedInterfaceMethod(TR_OpaqueMethodBlock *interfaceMethod,
    TR_OpaqueClassBlock *classObject, I_32 cpIndex)
{
    ...
    if (comp && comp->getOption(TR_UseSymbolValidationManager)) {
        if (!comp->getSymbolValidationManager()->addInterfaceMethodFromCPRecord(ramMethod,
                (TR_OpaqueClassBlock *)J9_CLASS_FROM_METHOD((J9Method *)interfaceMethod), classObject, cpIndex)) {
            return NULL;
        }
    }
    return ramMethod;
...
TR_OpaqueClassBlock *TR_ResolvedRelocatableJ9Method::getDeclaringClassFromFieldOrStatic(TR::Compilation *comp,
    int32_t cpIndex)
{
...
bool mightWalkTheStackToGetToUserCode(TR::MethodSymbol *sym) { return true; }

TR_J9MethodParameterIterator::TR_J9MethodParameterIterator(TR_J9MethodBase &j9Method, TR::Compilation &comp,
    TR_ResolvedMethod *resolvedMethod)
    : TR_MethodParameterIterator(comp)
    , _j9Method(j9Method)
    , _sig(j9Method.signatureChars())
...
TR_OpaqueClassBlock *TR_J9MethodParameterIterator::getOpaqueClass()
{
    ...
    if (_nextIncrBy == 0)
        getDataType();
    return _resolvedMethod == NULL ? NULL : fej9->getClassFromSignature(_sig, _nextIncrBy, _resolvedMethod);
}
...
bool TR_J9MethodParameterIterator::isArray() { return (*_sig == '['); }

bool TR_J9MethodParameterIterator::isClass() { return (*_sig == 'L'); }

bool TR_J9MethodParameterIterator::atEnd() { return (*_sig == ')'); }
...
static J9::MethodHandleThunkDetails *getMethodHandleThunkDetails(TR_J9ByteCodeIlGenerator *ilgen, TR::Compilation *comp,
    TR::SymbolReference *macro)
{
    OMR::Logger *log = comp->log();
    ...
    if (details.isMethodHandleThunk()) {
        ...
        logprintf(trace, log, "  Conservatively leave ILGen macro '%s' as a native call for peeking\n",
            comp->getDebug()->getName(macro));
    } else {
        logprintf(trace, log, "  Conservatively abort compile due to presence of ILGen macro '%s'\n",
            comp->getDebug()->getName(macro));
...
bool TR_J9ByteCodeIlGenerator::runFEMacro(TR::SymbolReference *symRef)
{
    ...
    OMR::Logger *log = comp()->log();
    bool traceILGen = comp()->getOption(TR_TraceILGen);
    TR_J9VMBase *fej9 = (TR_J9VMBase *)fe();
    ...
    switch (rm) {
        case TR::java_lang_invoke_CollectHandle_allocateArray: {
            ...
            if (methodHandleNode->getOpCode().hasSymbolReference()
                && methodHandleNode->getSymbolReference()->hasKnownObjectIndex() && methodHandleNode->isNonNull()) {
                pop();
...
#endif /* defined(J9VM_OPT_JITSERVER) */
                {
                    ...
                    uintptr_t methodHandle
                        = knot->getPointer(methodHandleNode->getSymbolReference()->getKnownObjectIndex());
                    ...
                    uintptr_t arguments
                        = fej9->getReferenceField(fej9->getReferenceField(fej9->getReferenceField(methodHandle, "next",
                                                                              "Ljava/lang/invoke/MethodHandle;"),
                                                      "type", "Ljava/lang/invoke/MethodType;"),
                            "ptypes", "[Ljava/lang/Class;");
                    componentClazz = fej9->getComponentClassFromArrayClass(
                        fej9->getClassFromJavaLangClass(fej9->getReferenceElement(arguments, collectPosition)));
                }
        ...
        case TR::java_lang_invoke_CollectHandle_numArgsAfterCollectArray: {
...
#endif /* defined(J9VM_OPT_JITSERVER) */
            {
                ...
                arguments = fej9->getReferenceField(
                    fej9->getReferenceField(methodHandle, "type", "Ljava/lang/invoke/MethodType;"), "ptypes",
                    "[Ljava/lang/Class;");
        ...
        case TR::java_lang_invoke_ExplicitCastHandle_convertArgs: {
            bool isExplicit = symRef->getSymbol()->castToMethodSymbol()->getMandatoryRecognizedMethod()
                == TR::java_lang_invoke_ExplicitCastHandle_convertArgs;
            ...
            if (comp()->isOutOfProcessCompilation()) {
                auto stream = comp()->getStream();
                stream->write(JITServer::MessageType::runFEMacro_invokeExplicitCastHandleConvertArgs,
                    thunkDetails->getHandleRef());
            ...
            logprintf(trace(), log, "sourceSig %s targetSig %.*s\n", sourceSig, methodDescriptorLength,
                methodDescriptor);

            for (int32_t argIndex = 0; sourceSig[0] != ')'; argIndex++, sourceSig = nextSignatureArgument(sourceSig),
                         targetSig = nextSignatureArgument(targetSig)) {
                ...
                const char *sourceName = sourceBuf;
                const char *sourceType = sourceBuf;
                const char *targetName = targetBuf;
                const char *targetType = targetBuf;
                switch (sourceSig[0]) {
                    case 'L':
                    case '[':
                        sourceName = "object";
                ...
                if (strcmp(sourceType, targetType)) {
                    ...
                    TR::SymbolReference *methodSymRef = comp()->getSymRefTab()->methodSymRefFromName(_methodSymbol,
                        "java/lang/invoke/ConvertHandle$FilterHelpers", methodName, methodSignature,
                        TR::MethodSymbol::Static);
                    TR_ASSERT(methodSymRef, "assertion failure");

                    argValue = TR::Node::createWithSymRef(
                        methodSymRef->getSymbol()->castToMethodSymbol()->getMethod()->directCallOpCode(), 1, 1,
                        argValue, methodSymRef);
                    genTreeTop(argValue);
                }
                ...
                if (targetType[0] == 'L') {
                    ...
                    if (comp()->isOutOfProcessCompilation()) {
                        auto stream = comp()->getStream();
                        stream->write(JITServer::MessageType::runFEMacro_targetTypeL, thunkDetails->getHandleRef(),
                            argIndex);
...
#endif /* defined(J9VM_OPT_JITSERVER) */
                    {
                        ...
                        // We've already loaded the handle once, but must reload it because we released VM access in
                        // between.
                        methodHandle = *thunkDetails->getHandleRef();
                        targetArguments = fej9->getReferenceField(
                            fej9->getReferenceField(
                                fej9->getReferenceField(methodHandle, "next", "Ljava/lang/invoke/MethodHandle;"),
                                "type", "Ljava/lang/invoke/MethodType;"),
                            "ptypes", "[Ljava/lang/Class;");
                        targetParmClass = (TR_OpaqueClassBlock *)(intptr_t)fej9->getInt64Field(
                            fej9->getReferenceElement(targetArguments, argIndex),
                            "vmRef" /* should use fej9->getOffsetOfClassFromJavaLangClassField() */);
                        // Load callsite type and check if two types are compatible
                        sourceArguments = fej9->getReferenceField(
                            fej9->getReferenceField(methodHandle, "type", "Ljava/lang/invoke/MethodType;"), "ptypes",
                            "[Ljava/lang/Class;");
                        sourceParmClass = (TR_OpaqueClassBlock *)(intptr_t)fej9->getInt64Field(
                            fej9->getReferenceElement(sourceArguments, argIndex),
                            "vmRef" /* should use fej9->getOffsetOfClassFromJavaLangClassField() */);
                    }

                    if (fej9->isInterfaceClass(targetParmClass) && isExplicit) {
                        // explicitCastArguments specifies that we must not checkcast interfaces
                    } else if (sourceParmClass == targetParmClass
                        || fej9->isInstanceOf(sourceParmClass, targetParmClass, false /*objectTypeIsFixed*/,
                               true /*castTypeIsFixed*/)
                            == TR_yes) {
                        logprintf(trace(), log,
                            "source type and target type are compatible, omit checkcast for arg %d\n", argIndex);
                    } else {
                        push(argValue);
            ...
            //
            placeholder->setSymbolReference(
                symRefWithArtificialSignature(placeholder->getSymbolReference(), "(.*)I", methodDescriptor, 0));
        ...
        case TR::java_lang_invoke_ILGenMacros_invokeExactAndFixup: {
            ...
            if (comp()->isOutOfProcessCompilation()) {
                ...
                auto stream = comp()->getStream();
                stream->write(JITServer::MessageType::runFEMacro_invokeILGenMacrosInvokeExactAndFixup,
                    thunkDetails->getHandleRef(), listOfOffsets);
                auto recv = stream->read<std::string>();
                auto &methodDescriptorString = std::get<0>(recv);
                methodDescriptorLength = methodDescriptorString.length();
                methodDescriptor = (char *)alloca(methodDescriptorLength + 1);
...
#endif /* defined(J9VM_OPT_JITSERVER) */
            {
                ...
                methodHandle
                    = returnFromArchetype ? receiverHandle : walkReferenceChain(methodHandleExpression, receiverHandle);
                methodDescriptorRef = fej9->getReferenceField(
                    fej9->getReferenceField(methodHandle, "type", "Ljava/lang/invoke/MethodType;"), "methodDescriptor",
                    "Ljava/lang/String;");
                methodDescriptorLength = fej9->getStringUTF8Length(methodDescriptorRef);
                methodDescriptor = (char *)alloca(methodDescriptorLength + 1);
                fej9->getStringUTF8(methodDescriptorRef, methodDescriptor, methodDescriptorLength + 1);
            ...
            logprintf(traceILGen, log, "  invokeExactAndFixup: return type of %p is %s\n", methodHandle,
                returnType); // Technically methodHandle could be stale here

            // Generate an invokeHandle using a symref with the proper return type
            //
            TR_ResolvedMethod *invokeExactMacro
                = symRef->getSymbol()->castToResolvedMethodSymbol()->getResolvedMethod();
            TR::SymbolReference *invokeExact = comp()->getSymRefTab()->methodSymRefFromName(_methodSymbol,
                JSR292_MethodHandle, JSR292_invokeExact, JSR292_invokeExactSig, TR::MethodSymbol::ComputedVirtual);
            TR::SymbolReference *invokeExactWithSig
                = symRefWithArtificialSignature(invokeExact, "(.*).?", invokeExactMacro->signatureChars(),
                    1, // skip explicit MethodHandle argument -- the real invokeExact has it as a receiver
                    returnType);
            ...
            if (returnFromArchetype) {
                ...
                //
                if (callOp == TR::calli) {
                    TR::SymbolReference *placeholder
                        = symRefWithArtificialSignature(placeholderWithDummySignature(), "()I");
                    push(genNodeAndPopChildren(TR::icall, 0, placeholder));
                } else {
                    TR::SymbolReference *placeholder
                        = symRefWithArtificialSignature(placeholderWithDummySignature(), "(.?)I", returnType);
        ...
        case TR::java_lang_invoke_BruteArgumentMoverHandle_permuteArgs: {
            ...
            if (comp()->isOutOfProcessCompilation()) {
                auto stream = comp()->getStream();
                stream->write(JITServer::MessageType::runFEMacro_invokeArgumentMoverHandlePermuteArgs,
                    thunkDetails->getHandleRef());
                auto recv = stream->read<std::string>();
            ...
            logprintf(traceILGen, log, "  permuteArgs: nextHandleSignature is %s\n", nextHandleSignature);
        }
            // FALL THROUGH
        case TR::java_lang_invoke_PermuteHandle_permuteArgs: {
            ...
            if (comp()->isOutOfProcessCompilation()) {
                auto stream = comp()->getStream();
                stream->write(JITServer::MessageType::runFEMacro_invokePermuteHandlePermuteArgs,
                    thunkDetails->getHandleRef());
                ...
                oldSignature = originalArgs->getSymbolReference()
                                   ->getSymbol()
                                   ->getResolvedMethodSymbol()
                                   ->getResolvedMethod()
                                   ->signatureChars();
                newSignature = (char *)"()I";
                logprintf(traceILGen, log, "  permuteArgs: oldSignature is %s\n", oldSignature);
                for (i = 0; i < permuteLength; i++) {
                    auto argIndex = argIndices[i];
                    if (argIndex >= 0) {
                        newSignature
                            = artificialSignature(stackAlloc, "(.*.@)I", newSignature, 0, oldSignature, argIndex);
                        push(originalArgs->getChild(argIndex));
                        logprintf(traceILGen, log, "  permuteArgs:   %d: incoming argument\n", argIndex);
                    } else {
                        newSignature
                            = artificialSignature(stackAlloc, "(.*.@)I", newSignature, 0, nextHandleSignature, i);
                        ...
                        switch (argType[0]) {
                            ...
                            case '[':
                                ...
                                extraSignature = artificialSignature(stackAlloc,
                                    "(L" JSR292_ArgumentMoverHandle ";I)Ljava/lang/Object;");
                                break;
                            default:
                                snprintf(extraName, sizeof(extraName), "extra_%c", argType[0]);
                                extraSignature = artificialSignature(stackAlloc,
                                    "(L" JSR292_ArgumentMoverHandle ";I).@", nextHandleSignature, i);
                                break;
                        }
                        logprintf(traceILGen, log, "  permuteArgs:   %d: call to %s.%s%s\n", argIndex,
                            JSR292_ArgumentMoverHandle, extraName, extraSignature);
                        ...
                        if (dataType == TR::Address && extraArrayNode) {
                            if (-1 - argIndex < 5) {
                                ...
                                loadArrayElement(TR::Address, comp()->il.opCodeForIndirectArrayLoad(TR::Address),
                                    false);
                            }
                        } else {
                            TR::SymbolReference *extra = comp()->getSymRefTab()->methodSymRefFromName(_methodSymbol,
                                JSR292_ArgumentMoverHandle, extraName, extraSignature, TR::MethodSymbol::Static);
                ...
                logprintf(traceILGen, log, "  permuteArgs: permuted placeholder signature is %s\n", newSignature);
            } else
#endif /* defined(J9VM_OPT_JITSERVER) */
            {
            ...
            //
            TR::SymbolReference *placeholder
                = symRefWithArtificialSignature(placeholderWithDummySignature(), newSignature);
        ...
        case TR::java_lang_invoke_GuardWithTestHandle_numGuardArgs: {
            ...
            if (comp()->isOutOfProcessCompilation()) {
                auto stream = comp()->getStream();
                stream->write(JITServer::MessageType::runFEMacro_invokeGuardWithTestHandleNumGuardArgs,
                    thunkDetails->getHandleRef());
        ...
        case TR::java_lang_invoke_DirectHandle_directCall: {
            ...
            TR_J9VMBase::MethodOfHandle moh
                = fej9->methodOfDirectOrVirtualHandle(thunkDetails->getHandleRef(), isVirtual);
            ...
            if (isVirtual) {
                ...
                TR::Node *vftLoad = TR::Node::createWithSymRef(TR::aloadi, 1, 1, callNode->getArgument(0),
            ...
            if (isVirtual)
                ...
                kind = TR::MethodSymbol::Special; // TODO: Is this right?  Can I always consider a direct call with a
                                                  // receiver to be special?
            callNode->setSymbolReference(comp()->getSymRefTab()->findOrCreateMethodSymbol(
                _methodSymbol->getResolvedMethodIndex(), -1, method, kind));

            if (callNode->getDataType() != TR::NoType)
                push(callNode);

            if (isVirtual) {
                callNode->getSymbolReference()->setOffset(
                    -(moh.vmSlot - TR::Compiler->vm.getInterpreterVTableOffset()));
        ...
        case TR::java_lang_invoke_DirectHandle_compiledEntryPoint: {
            ...
            TR::SymbolReference *extraField
                = comp()->getSymRefTab()->findOrCreateJ9MethodExtraFieldSymbolRef(offsetof(struct J9Method, extra));
            ...
            TR::Node *extra
                = TR::Node::createWithSymRef(comp()->il.opCodeForIndirectLoad(extraField->getSymbol()->getDataType()),
                    1, 1, j9methodAddress, extraField);
            TR::Node *result = 0;
            switch (symRef->getSymbol()->castToMethodSymbol()->getMandatoryRecognizedMethod()) {
                case TR::java_lang_invoke_DirectHandle_isAlreadyCompiled: {
                    ...
                    TR::Node *zero
                        = comp()->target().is64Bit() ? TR::Node::lconst(extra, 0) : TR::Node::iconst(extra, 0);
                    TR::Node *mask = comp()->target().is64Bit() ? TR::Node::lconst(extra, J9_STARTPC_NOT_TRANSLATED)
                                                                : TR::Node::iconst(extra, J9_STARTPC_NOT_TRANSLATED);
                    result = TR::Node::create(xcmpeq, 2, TR::Node::create(xand, 2, extra, mask), zero);
                } break;
                case TR::java_lang_invoke_DirectHandle_compiledEntryPoint:
                    if (comp()->target().cpu.isI386()) {
                        ...
                        TR::SymbolReference *linkageInfoSymRef
                            = comp()->getSymRefTab()->findOrCreateStartPCLinkageInfoSymbolRef(-4);
                        TR::ILOpCodes x2a = comp()->target().is64Bit() ? TR::l2a : TR::i2a;
                        TR::Node *linkageInfo = TR::Node::createWithSymRef(TR::iloadi, 1, 1,
                            TR::Node::create(x2a, 1, extra), linkageInfoSymRef);
                        TR::Node *jitEntryOffset
                            = TR::Node::create(TR::ishr, 2, linkageInfo, TR::Node::iconst(extra, 16));
                        if (comp()->target().is64Bit())
        ...
        case TR::java_lang_invoke_SpreadHandle_arrayArg: {
            ...
            if (comp()->isOutOfProcessCompilation()) {
                auto stream = comp()->getStream();
                stream->write(JITServer::MessageType::runFEMacro_invokeSpreadHandleArrayArg,
                    thunkDetails->getHandleRef());
...
#endif /* defined(J9VM_OPT_JITSERVER) */
            {
                ...
                uint32_t spreadPositionOffset
                    = fej9->getInstanceFieldOffset(fej9->getObjectClass(methodHandle), "spreadPosition", "I");
                if (spreadPositionOffset != ~0)
                    spreadPosition = fej9->getInt32FieldAt(methodHandle, spreadPositionOffset);

                leafClassNameChars = fej9->getClassNameChars((TR_OpaqueClassBlock *)leafClass,
                    leafClassNameLength); // eww, TR_FrontEnd downcast
            ...
            //
            char *arrayClassSignature
                = (char *)alloca(arity + leafClassNameLength + 3); // 3 = 'L' + ';' + null terminator
            ...
            if (isPrimitiveClass) {
                ...
                char *arrayRomClassNameChars
                    = fej9->getClassNameChars((TR_OpaqueClassBlock *)arrayJ9Class, arrayRomClassNameLength);
                TR_ASSERT(arrayRomClassNameLength == 2,
                    "Every array romclass '%.*s' should be of the form [X where X is a single character",
                    arrayRomClassNameLength, arrayRomClassNameChars);
                snprintf(arrayClassSignature + arity - 1, leafClassNameLength + 4, "%.*s", arrayRomClassNameLength,
                    arrayRomClassNameChars);
            } else {
                snprintf(arrayClassSignature + arity, leafClassNameLength + 3, "L%.*s;", leafClassNameLength,
                    leafClassNameChars);
            }

            logprintf(traceILGen, log, "  SpreadHandle.arrayArg: Array class name is '%s'\n", arrayClassSignature);

            // Edit placeholder signature to give the right class for the array
            placeholder->setSymbolReference(
                symRefWithArtificialSignature(placeholder->getSymbolReference(), "(.?)I", arrayClassSignature));
        ...
        case TR::java_lang_invoke_SpreadHandle_spreadStart: {
            ...
            if (comp()->isOutOfProcessCompilation()) {
                ...
                stream->write(JITServer::MessageType::runFEMacro_invokeSpreadHandle, thunkDetails->getHandleRef(),
                    getSpreadPos);
                auto recv = stream->read<int32_t, int32_t, int32_t>();
...
#endif /* defined(J9VM_OPT_JITSERVER) */
            {
                ...
                methodHandle = *thunkDetails->getHandleRef();
                arguments
                    = fej9->getReferenceField(fej9->methodHandle_type(methodHandle), "ptypes", "[Ljava/lang/Class;");
                numArguments = (int32_t)fej9->getArrayLengthInElements(arguments);
                ...
                if (symRef->getSymbol()->castToMethodSymbol()->getMandatoryRecognizedMethod()
                        == TR::java_lang_invoke_SpreadHandle_spreadStart
                    || symRef->getSymbol()->castToMethodSymbol()->getMandatoryRecognizedMethod()
                        == TR::java_lang_invoke_SpreadHandle_numArgsAfterSpreadArray)
        ...
        case TR::java_lang_invoke_FoldHandle_argIndices: {
...
#endif /* defined(J9VM_OPT_JITSERVER) */
            {
                ...
                if (arrayLength != 0) {
                    ...
                    uintptr_t combiner
                        = fej9->getReferenceField(methodHandle, "combiner", "Ljava/lang/invoke/MethodHandle;");
                    uintptr_t combinerArguments
                        = fej9->getReferenceField(fej9->methodHandle_type(combiner), "ptypes", "[Ljava/lang/Class;");
        ...
        case TR::java_lang_invoke_FilterArgumentsWithCombinerHandle_argumentIndices: {
            ...
            if (comp()->isOutOfProcessCompilation()) {
                auto stream = comp()->getStream();
                stream->write(JITServer::MessageType::runFEMacro_invokeFilterArgumentsWithCombinerHandleArgumentIndices,
                    thunkDetails->getHandleRef());
        ...
        case TR::java_lang_invoke_FilterArgumentsWithCombinerHandle_filterPosition: {
            ...
            if (comp()->isOutOfProcessCompilation()) {
                auto stream = comp()->getStream();
                stream->write(JITServer::MessageType::runFEMacro_invokeFilterArgumentsWithCombinerHandleFilterPosition,
                    thunkDetails->getHandleRef());
        ...
        case TR::java_lang_invoke_FilterArgumentsWithCombinerHandle_argumentsForCombiner: {
            ...
            for (int i = 0; i < numCombinerArgs; i++) {
                ...
                newSignature = artificialSignature(stackAlloc, "(.*.@)I", newSignature, 0, placeholderSignature, index);
            }
            placeholder->setNumChildren(numCombinerArgs);
            placeholder->setSymbolReference(
                symRefWithArtificialSignature(placeholder->getSymbolReference(), newSignature));
        ...
        case TR::java_lang_invoke_FinallyHandle_numFinallyTargetArgsToPassThrough: {
            TR_ASSERT(archetypeParmCount == 0,
                "assertion failure"); // The number of arguments for numFinallyTargetArgsToPassThrough()
...
#endif /* defined(J9VM_OPT_JITSERVER) */
            {
                ...
                uintptr_t methodHandle = *thunkDetails->getHandleRef();
                uintptr_t finallyTarget
                    = fej9->getReferenceField(methodHandle, "finallyTarget", "Ljava/lang/invoke/MethodHandle;");
                ...
                uintptr_t methodDescriptorRef
                    = fej9->getReferenceField(finallyType, "methodDescriptor", "Ljava/lang/String;");
        ...
        case TR::java_lang_invoke_FilterArgumentsWithCombinerHandle_numSuffixArgs: {
            ...
            if (comp()->isOutOfProcessCompilation()) {
                auto stream = comp()->getStream();
                stream->write(JITServer::MessageType::runFEMacro_invokeFilterArgumentsWithCombinerHandleNumSuffixArgs,
                    thunkDetails->getHandleRef());
        ...
        case TR::java_lang_invoke_FilterArgumentsHandle_numArgsToFilter: {
            ...
            if (comp()->isOutOfProcessCompilation()) {
                auto stream = comp()->getStream();
                stream->write(JITServer::MessageType::runFEMacro_invokeFilterArgumentsHandle2,
                    thunkDetails->getHandleRef());
                auto recv = stream->read<int32_t, int32_t, int32_t>();
                numArguments = std::get<0>(recv);
        ...
        case TR::java_lang_invoke_FilterArgumentsHandle_filterArguments: {
...

            {
                ...
                if (comp()->isOutOfProcessCompilation()) {
                    auto stream = comp()->getStream();
                    stream->write(JITServer::MessageType::runFEMacro_invokeFilterArgumentsHandle,
                        thunkDetails->getHandleRef(), knotEnabled);
                    auto recv = stream->read<int32_t, std::string, std::vector<uint8_t>,
                        std::vector<TR::KnownObjectTable::Index>, std::vector<uintptr_t *> >();
                    ...
                    filterIndexList = knotEnabled ? (TR::KnownObjectTable::Index *)comp()->trMemory()->allocateMemory(
                                                        sizeof(TR::KnownObjectTable::Index) * numFilters, stackAlloc)
                                                  : NULL;
                    ...
                    for (int i = 0; i < numFilters; i++) {
                        ...
                        if (knotEnabled) {
                            ...
                            if (recvfilterIndexList[i] != TR::KnownObjectTable::UNKNOWN)
                                knot->updateKnownObjectTableAtServer(recvfilterIndexList[i],
                                    recvFilterObjectReferenceLocationList[i]);
                        }
                    }
                } else
#endif /* defined(J9VM_OPT_JITSERVER) */
                {
                    ...
                    uintptr_t methodHandle = *thunkDetails->getHandleRef();
                    uintptr_t filters
                        = fej9->getReferenceField(methodHandle, "filters", "[Ljava/lang/invoke/MethodHandle;");
                ...
                char *placeholderSignature
                    = placeholder->getSymbol()->castToMethodSymbol()->getMethod()->signatureChars();
                TR::Node *filterArray = pop();

                if (traceILGen) {
                    log->prints("  FilterArgumentsHandle.filterArgument:\n");
                    log->printf("    filterArray node is %p\n", filterArray);
                    log->printf("    placeholder children: %d sig: %.*s\n", placeholder->getNumChildren(),
                        placeholder->getSymbol()->castToMethodSymbol()->getMethod()->signatureLength(),
                        placeholderSignature);
                }

                int32_t firstFilteredArgIndex = 0;
                for (int32_t childIndex = firstFilteredArgIndex; childIndex < placeholder->getNumChildren();
                     childIndex++) {
                    ...
                    if (haveFilter[arrayIndex]) {
                        ...
                        if (receiverHandle->getOpCode().hasSymbolReference()
                            && receiverHandle->getSymbolReference()->getSymbol()->isArrayShadowSymbol()) {
                            if (!knotEnabled || thunkDetails->isShareable()) {
                                ...
                                TR::SymbolReference *improvedSymbol
                                    = comp()->getSymRefTab()->findOrCreateSymRefWithKnownObject(
                                        receiverHandle->getSymbolReference(), filterIndexList[arrayIndex]);
                        ...
                        //
                        TR::SymbolReference *invokeExact
                            = comp()->getSymRefTab()->methodSymRefFromName(_methodSymbol, JSR292_MethodHandle,
                                JSR292_invokeExact, JSR292_invokeExactSig, TR::MethodSymbol::ComputedVirtual);
                        TR::SymbolReference *invokeExactWithSig = symRefWithArtificialSignature(invokeExact, "(.@).@",
                            placeholderSignature, childIndex, nextSignature, argumentIndex);
                    ...
                    //
                    newSignature
                        = artificialSignature(stackAlloc, "(.*.@)I", newSignature, 0, nextSignature, argumentIndex);
                }
        ...
        case TR::java_lang_invoke_CatchHandle_numCatchTargetArgsToPassThrough: {
...
#endif /* defined(J9VM_OPT_JITSERVER) */
            {
                ...
                uintptr_t methodHandle = *thunkDetails->getHandleRef();
                uintptr_t catchTarget
                    = fej9->getReferenceField(methodHandle, "catchTarget", "Ljava/lang/invoke/MethodHandle;");
                uintptr_t catchArguments = fej9->getReferenceField(
                    fej9->getReferenceField(catchTarget, "type", "Ljava/lang/invoke/MethodType;"), "ptypes",
                    "[Ljava/lang/Class;");
        ...
        case TR::java_lang_invoke_ILGenMacros_parameterCount: {
            ...
            if (comp()->isOutOfProcessCompilation()) {
                ...
                stream->write(JITServer::MessageType::runFEMacro_invokeILGenMacrosParameterCount,
                    thunkDetails->getHandleRef(), listOfOffsets);
...
#endif /* defined(J9VM_OPT_JITSERVER) */
            {
                ...
                uintptr_t arguments = fej9->getReferenceField(
                    fej9->getReferenceField(methodHandle, "type", "Ljava/lang/invoke/MethodType;"), "ptypes",
                    "[Ljava/lang/Class;");
        ...
        case TR::java_lang_invoke_ILGenMacros_arrayLength: {
            ...
            if (comp()->isOutOfProcessCompilation()) {
                ...
                std::vector<uintptr_t> listOfOffsets;
                packReferenceChainOffsets(pop(), listOfOffsets);
                stream->write(JITServer::MessageType::runFEMacro_invokeILGenMacrosArrayLength,
                    thunkDetails->getHandleRef(), listOfOffsets);
        ...
        case TR::java_lang_invoke_ILGenMacros_getField: {
            ...
            TR_ASSERT(fieldLoad->getOpCode().isLoadIndirect(), "Unexpected opcode for ILGenMacros.getField: %s",
                fieldLoad->getOpCode().getName());
            ...
            TR_ASSERT(fieldSym->isShadow() && fieldSymRef->getCPIndex() > 0,
                "ILGenMacros.getField expecting field load; found load of %s", comp()->getDebug()->getName(symRef));
            ...
            if (comp()->isOutOfProcessCompilation()) {
                ...
                TR_ASSERT(fieldSym->getDataType() == TR::Int32,
                    "ILGenMacros.getField expecting int field; found load of %s", comp()->getDebug()->getName(symRef));
                stream->write(JITServer::MessageType::runFEMacro_invokeILGenMacrosGetField,
                    thunkDetails->getHandleRef(), fieldOffset, listOfOffsets);
        ...
        case TR::java_lang_invoke_ILGenMacros_isShareableThunk: {
            ...
            bool queryingCustom = (symRef->getSymbol()->castToMethodSymbol()->getMandatoryRecognizedMethod()
                == TR::java_lang_invoke_ILGenMacros_isCustomThunk);
...
uintptr_t TR_J9ByteCodeIlGenerator::walkReferenceChain(TR::Node *node, uintptr_t receiver)
{
    OMR::Logger *log = comp()->log();
    bool traceILGen = comp()->getOption(TR_TraceILGen);
    TR_J9VMBase *fej9 = (TR_J9VMBase *)(comp()->fe());
#if defined(J9VM_OPT_JITSERVER)
    TR_ASSERT_FATAL(!comp()->isOutOfProcessCompilation(),
        "walkReferenceChain() should not be called by JITServer because of getReferenceFieldAt() call");
#endif
    uintptr_t result = 0;
    if (node->getOpCode().isLoadDirect() && node->getType() == TR::Address) {
        TR_ASSERT(node->getSymbolReference()->getCPIndex() == 0,
            "walkReferenceChain expecting aload of 'this'; found aload of %s",
            comp()->getDebug()->getName(node->getSymbolReference()));
        result = receiver;
    } else if (node->getOpCode().isLoadIndirect() && node->getType() == TR::Address) {
        TR::SymbolReference *symRef = node->getSymbolReference();
        if (symRef->isUnresolved()) {
            logprintf(traceILGen, log, "  walkReferenceChain hit unresolved symref %s; aborting\n",
                symRef->getName(comp()->getDebug()));
            comp()->failCompilation<TR::ILGenFailure>("Symbol reference is unresolved");
        }
        TR::Symbol *sym = symRef->getSymbol();
        TR_ASSERT(sym->isShadow() && symRef->getCPIndex() > 0,
            "walkReferenceChain expecting field load; found load of %s", comp()->getDebug()->getName(symRef));
    ...
    if (traceILGen) {
        ...
        log->printf("  walkReferenceChain(%s) = %p // %s\n", comp()->getDebug()->getName(node), (void *)result,
            comp()->getDebug()->getName(node->getSymbolReference()));
    }
...
#if defined(J9VM_OPT_JITSERVER)
void TR_J9ByteCodeIlGenerator::packReferenceChainOffsets(TR::Node *node, std::vector<uintptr_t> &listOfOffsets)
{
    OMR::Logger *log = comp()->log();
    bool traceILGen = comp()->getOption(TR_TraceILGen);
    ...
    if (traceILGen) {
        TR_ASSERT(node->getOpCode().hasSymbolReference(), "Can't get here without a symref");
        log->printf("  walkReferenceChain(%s) // %s\n", comp()->getDebug()->getName(node),
            comp()->getDebug()->getName(node->getSymbolReference()));
    }
    return;
}
...
bool TR_ResolvedJ9Method::isFieldFlattened(TR::Compilation *comp, int32_t cpIndex, bool isStatic)
{
    ...
    TR_OpaqueClassBlock *containingClass
        = definingClassAndFieldShapeFromCPFieldRef(comp, cp(), cpIndex, isStatic, &fieldShape);
    ...
    return vmThread->javaVM->internalVMFunctions->isFlattenableFieldFlattened(
        reinterpret_cast<J9Class *>(containingClass), fieldShape);
}

bool TR_ResolvedJ9Method::isDAAExternalDecimalWrapperMethod()
{
    // DAA External Decimal check method
    return (
        this->TR_ResolvedMethod::getRecognizedMethod() == TR::com_ibm_dataaccess_ExternalDecimal_checkExternalDecimal);
}

bool TR_ResolvedJ9Method::isDAAWrapperMethod()
{
    return isDAAMarshallingWrapperMethod() || isDAAPackedDecimalWrapperMethod() || isDAAExternalDecimalWrapperMethod();
}

bool TR_ResolvedJ9Method::isDAAMarshallingWrapperMethod()
{
...
bool TR_ResolvedJ9Method::isDAAPackedDecimalWrapperMethod()
{
    TR::RecognizedMethod rm = this->TR_ResolvedMethod::getRecognizedMethod();
...
bool TR_ResolvedJ9Method::isDAAExternalDecimalIntrinsicMethod()
{
    // DAA External Decimal check method
    return (
        this->TR_ResolvedMethod::getRecognizedMethod() == TR::com_ibm_dataaccess_ExternalDecimal_checkExternalDecimal_);
}

bool TR_ResolvedJ9Method::isDAAIntrinsicMethod()
{
    return isDAAMarshallingIntrinsicMethod() || isDAAPackedDecimalIntrinsicMethod()
        || isDAAExternalDecimalIntrinsicMethod();
}

bool TR_ResolvedJ9Method::isDAAMarshallingIntrinsicMethod()
{
    TR::RecognizedMethod rm = this->TR_ResolvedMethod::getRecognizedMethod();
...
bool TR_ResolvedJ9Method::isDAAPackedDecimalIntrinsicMethod()
{
    TR::RecognizedMethod rm = this->TR_ResolvedMethod::getRecognizedMethod();
