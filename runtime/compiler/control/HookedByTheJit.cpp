...
void cgOnClassUnloading(void *loaderPtr)
{
...
TR::OptionSet *findOptionSet(J9Method *method, bool isAOT)
{
    ...
    if (methodSignature) {
        snprintf(methodSignature, len, "%.*s.%.*s%.*s", J9UTF8_LENGTH(className), utf8Data(className),
            J9UTF8_LENGTH(name), utf8Data(name), J9UTF8_LENGTH(signature), utf8Data(signature));
        ...
        optionSet = TR::Options::findOptionSet(index, lineNum, methodSignature,
            TR::Options::getInitialHotnessLevel(hasBackwardBranches), isAOT);
        if (len >= 1024)
...
extern void getOutOfIdleStates(TR::CompilationInfo::TR_SamplerStates expectedState, TR::CompilationInfo *compInfo,
    const char *reason);
extern void getOutOfIdleStatesUnlocked(TR::CompilationInfo::TR_SamplerStates expectedState,
    TR::CompilationInfo *compInfo, const char *reason);

//***************************************************************************************
//  Hooked by the JIT
//***************************************************************************************

int32_t encodeCount(int32_t count) { return (count == -1) ? 0 : (count << 1) + 1; }
...
bool sharedCacheContainsProfilingInfoForMethod(J9VMThread *vmThread, TR::CompilationInfo *compInfo,
    J9ROMMethod *romMethod)
{
    ...
    IDATA dataIsCorrupt;
    TR_IPBCDataStorageHeader *store
        = (TR_IPBCDataStorageHeader *)scConfig->findAttachedData(vmThread, romMethod, &descriptor, &dataIsCorrupt);
...
/// The hash value will be eventually added to variable `count` in `jitHookInitializeSendTarget()` under some
/// conditions. The formula for the hash value is:
///
...
#if defined(J9VM_OPT_JITSERVER)
static int32_t calculateCountForMethodCachedAtServer(J9ROMMethod *romMethod, TR::Options *optionsJIT,
    TR::Options *optionsAOT)
{
    ...
    if (optionsAOT) {
        ...
        if ((scount == TR_QUICKSTART_INITIAL_SCOUNT) || (scount == TR_INITIAL_SCOUNT)) {
            ...
            scount = std::min(getCount(romMethod, optionsJIT, optionsAOT), optionsAOT->getInitialSCount());
        }
    }
    return scount;
}
...
static void jitHookInitializeSendTarget(J9HookInterface **hook, UDATA eventNum, void *eventData, void *userData)
{
    ...
    if (serverAOTMethodSet != NULL) {
        ...
        if (sigLen < 1024) {
            char sigC[1024];
            snprintf(sigC, sizeof(sigC), "%.*s.%.*s%.*s", J9UTF8_LENGTH(className), utf8Data(className),
                J9UTF8_LENGTH(name), utf8Data(name), J9UTF8_LENGTH(signature), utf8Data(signature));

            // contains
            methodCachedAtServer = (serverAOTMethodSet->find(std::string(sigC)) != serverAOTMethodSet->end());

            if (TR::Options::getVerboseOption(TR_VerboseJITServer) && TR::Options::getVerboseOption(TR_VerboseCounts))
                if (methodCachedAtServer)
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "Method %s was cached at the server", sigC);
        }
    }
#endif // J9VM_OPT_JITSERVER
...

    {
        ...
        if (J9UTF8_LENGTH(className) == 36 && J9UTF8_LENGTH(name) == 21
            ...
            count = 0;
        } else if (J9UTF8_LENGTH(className) == 39 && J9UTF8_LENGTH(name) == 9
            ...
            count = 0;
        } else if (highCodeCacheOccupancyThresholdReached && !countInOptionSet
            && !TR::Options::getCountsAreProvidedByUser()) {
            count = J9ROMMETHOD_HAS_BACKWARDS_BRANCHES(romMethod) ? TR::Options::getHighCodeCacheOccupancyBCount()
    ...
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) \
        || defined(TR_HOST_ARM64))
            if (sc && sc->isClassInSharedCache(J9_CLASS_FROM_METHOD(method))) {
                ...
                if (methodExistsInSCC
                    ...
                    if (count == -1) {
                        ...
                        if (sc && sc->isHint(method, TR_HintFailedValidation, &newScount)) {
                            if ((scount == TR_QUICKSTART_INITIAL_SCOUNT)
                                || (scount == TR_INITIAL_SCOUNT)) { // If scount is not user specified (coarse way due
                                                                    // to info being lost from options parsing)
                                // TODO: Is casting the best thing to do here?
                                scount = std::min(getCount(romMethod, optionsJIT, optionsAOT),
                                    static_cast<int32_t>(
                                        newScount)); // Find what would've been normal count for this method and
                                // make sure new scount isn't larger than that
                                if (optionsAOT->getVerboseOption(TR_VerboseSCHints)
                                    || optionsJIT->getVerboseOption(TR_VerboseSCHints))
                                    TR_VerboseLog::writeLineLocked(TR_Vlog_SCHINTS,
                                        "Found hint in sc, increase scount to: %d, wanted scount: %d", scount,
                                        newScount);
                            }
                        }
                        count = scount;
                ...
                // AOT Body not in SCC, so scount was not set
                    ...
                    if (TR::Options::getCmdLineOptions()->getOption(TR_LowerCountsForAotCold)
                        ...
                        if (J9_SHARED_CACHE_DEFAULT_BOOT_SHARING(jitConfig->javaVM)
                            && J9_ARE_ALL_BITS_SET(jitConfig->javaVM->sharedClassConfig->runtimeFlags,
                                J9SHR_RUNTIMEFLAG_ENABLE_CACHE_NON_BOOT_CLASSES)) {
                            useLowerCountsForAOTCold = true;
                        } else if (compInfo->getPersistentInfo()->getElapsedTime()
                                <= (uint64_t)compInfo->getPersistentInfo()->getClassLoadingPhaseGracePeriod()
                    ...
                    // Increase counts for methods from non-bootstrap classes to improve throughput
                        ...
                        if (!fe->isClassLibraryMethod((TR_OpaqueMethodBlock *)method))
                            count = J9ROMMETHOD_HAS_BACKWARDS_BRANCHES(romMethod) ? TR_DEFAULT_INITIAL_BCOUNT
                                                                                  : TR_DEFAULT_INITIAL_COUNT;
                    }
                    ...
                    if (count == -1 && // Not yet changed
                        ...
                        && (TR_HintMethodCompiledDuringStartup
                        ...
                        if (wasCompiledDuringStartup) {
                            ...
                            if (TR::Options::getCmdLineOptions()->getOption(
                                    TR_ReduceCountsForMethodsCompiledDuringStartup))
                                count = TR::Options::getCountForMethodsCompiledDuringStartup();
                        } else // method was not compiled during startup
                        {
                            if (TR::Options::getCmdLineOptions()->getOption(
                                    TR_IncreaseCountsForMethodsCompiledOutsideStartup)
                                && TR::Options::startupTimeMatters()
                                    == TR_maybe) // For TR_no the counts are already high, for TR_yes we cannot get in
                                                 // here
                                count = J9ROMMETHOD_HAS_BACKWARDS_BRANCHES(romMethod) ? TR_DEFAULT_INITIAL_BCOUNT
                                                                                      : TR_DEFAULT_INITIAL_COUNT;
                        }
                ...
                if (optionsAOT->getOption(TR_EnableSharedCacheTiming)) {
                    sharedQueryTime
                        = j9time_hires_delta(sharedQueryTime, j9time_hires_clock(), J9PORT_TIME_DELTA_IN_MICROSECONDS);
            ...
            } else // ROM class not in shared class cache (no space or ineligible - maybe because of implicit SCC)
            {
                ...
                if (!TR::Options::getCountsAreProvidedByUser() && !countInOptionSet) {
                    if (TR::Options::getCmdLineOptions()->getOption(TR_UseLowerCountsForNonSCCMethodsDuringStartup)
                        && J9_ARE_NO_BITS_SET(jitConfig->javaVM->sharedClassConfig->runtimeFlags,
                            J9SHR_RUNTIMEFLAG_ENABLE_CACHE_NON_BOOT_CLASSES)
                        && (jitConfig->javaVM->phase != J9VM_PHASE_NOT_STARTUP)
...
#endif // defined(J9VM_INTERP_AOT_COMPILE_SUPPORT) && defined(J9VM_OPT_SHARED_CLASSES) && (defined(TR_HOST_X86) ||
       // defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) || defined(TR_HOST_ARM64))
        ...
        if (methodCachedAtServer) {
            int32_t serverCount = calculateCountForMethodCachedAtServer(romMethod, optionsJIT, optionsAOT);
            if (count == -1)
                count = serverCount;
            else
        ...
        {
            if (!TR::Options::getCountsAreProvidedByUser() && fe->isClassLibraryMethod((TR_OpaqueMethodBlock *)method))
                count = J9ROMMETHOD_HAS_BACKWARDS_BRANCHES(romMethod)
                    ? TR::Options::getCountForLoopyBootstrapMethods()
                    : TR::Options::getCountForLooplessBootstrapMethods();
            if (count == -1)
            ...
            if (optionsAOT->getOption(TR_EnableCompilationSpreading)
                || optionsJIT->getOption(TR_EnableCompilationSpreading)) {
    ...
    if (TR::Options::getJITCmdLineOptions()->getOption(TR_DumpInitialMethodNamesAndCounts)
        ...
        printf("Initial: Signature %s Count %d isLoopy %d isAOT %" OMR_PRIuPTR
            ...
            TR::Options::sharedClassCache()
                ? jitConfig->javaVM->sharedClassConfig->existsCachedCodeForROMMethod(vmThread, romMethod)
                : 0,
            TR::Options::sharedClassCache() ? TR_J9VMBase::get(jitConfig, vmThread, TR_J9VMBase::AOT_VM)
                                                  ->sharedCache()
                                                  ->isClassInSharedCache(J9_CLASS_FROM_METHOD(method))
                                            : 0,
            containsInfo);
        fflush(stdout);
    }
...
static void turnOnInterpreterProfiling(J9JavaVM *javaVM, TR::CompilationInfo *compInfo)
{
    if (interpreterProfilingState == IPROFILING_STATE_OFF) {
        ...
        if ((*hook)->J9HookRegisterWithCallSite(hook, J9HOOK_VM_PROFILING_BYTECODE_BUFFER_FULL,
                jitHookBytecodeProfiling, OMR_GET_CALLSITE(), NULL)) {
            ...
            if (TR::Options::getCmdLineOptions()->getOption(TR_VerboseInterpreterProfiling)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_IPROFILER, "t=%6u IProfiler reactivated...",
    ...
    switch (walkState->framesWalked) {
        ...
        case 3: // unused currently
            if (walkState->jitInfo != NULL || ((UDATA)walkState->pc <= J9SF_MAX_SPECIAL_FRAME_TYPE)
                || (walkState->pc == walkState->walkThread->javaVM->callInReturnPC)
                || (*walkState->bp & J9SF_A0_INVISIBLE_TAG))
                return J9_STACKWALK_STOP_ITERATING;
            break;
    }
...
static void emptyJitGCMapCheck(J9VMThread *currentThread, J9StackWalkState *walkState, j9object_t *objectSlot,
    const void *stackLocation)
{
    return;
}
...
static void jitGCMapCheck(J9VMThread *vmThread, IDATA handlerKey, void *userData)
{
    ...
    walkState.flags = J9_STACKWALK_ITERATE_O_SLOTS | J9_STACKWALK_ITERATE_HIDDEN_JIT_FRAMES
        | J9_STACKWALK_CHECK_I_SLOTS_FOR_OBJECTS;
...
#define SIG_SZ 150
    char sig[SIG_SZ];
    if (!TR::Options::canJITCompile() || TR::Options::getCmdLineOptions()->getOption(TR_FullSpeedDebug)
    ...
    dltBlock->cursor = idx;
    if (startPC || walkState.method == 0 || (romMethod->modifiers & J9AccNative)
    ...
    TR_J9ByteCode bc = TR_J9ByteCodeIterator::convertOpCodeToByteCodeEnum(
        J9_BYTECODE_START_FROM_ROM_METHOD(romMethod)[walkState.bytecodePCOffset]);
    if ((walkState.unwindSP - walkState.walkSP) == 0 && bc != J9BCinvokevirtual && bc != J9BCinvokespecial
        && bc != J9BCinvokestatic && bc != J9BCinvokeinterface && bc != J9BCinvokedynamic && bc != J9BCinvokehandle
        && bc != J9BCinvokehandlegeneric && bc != J9BCinvokespecialsplit && bc != J9BCinvokestaticsplit) {
    ...
    if (doPerformDLT) {
        ...
        if (vm->isLogSamplingSet()) {
            ...
            TR_VerboseLog::writeLineLocked(TR_Vlog_DLT, "Will try to queue DLT compilation for %s bcIndex=%d", sig,
                bcIndex);
        }
                ...
                if (count > 0) {
                    ...
                    if (TR::CompilationInfo::setInvocationCount(j9method, count, 0)) {
                        ...
                        if (vm->isAsyncCompilation()) {
                            ...
                            bool newPlanCreated;
                            TR_OptimizationPlan *jitplan
                                = TR::CompilationController::getCompilationStrategy()->processEvent(&event,
                                    &newPlanCreated);
                            if (jitplan) {
                                bool queued1 = false;
 ...
 */
static UDATA walkStackIterator(J9VMThread *currentThread, J9StackWalkState *walkState)
{
    if (walkState->userData1 == 0) {
 ...
 * J9_STACKWALK_STOP_ITERATING
 */
static UDATA walkStackIteratorReduced(J9VMThread *currentThread, J9StackWalkState *walkState)
{
    if (walkState->userData1 == 0) {
        Trc_JIT_MethodSampleStart1(currentThread, walkState->method);
        walkState->userData1 = (void *)1;
    } else if (walkState->userData2 == 0) {
        // userData2 is used as the cache. If userData2 is NULL, the current method will be stored in userData2
        // temporarily
...
static void processHWPBuffer(J9VMThread *vmThread, TR_J9VMBase *vm)
{
    ...
    if (hwProfiler->isExpired()) {
        if (IS_THREAD_RI_INITIALIZED(vmThread)) {
            if (TR::Options::isAnyVerboseOptionSet(TR_VerboseHWProfiler)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_HWPROFILER,
                    "Thread %p will be de-initialized for RI because RI expiration time was reached", vmThread);
            }
        ...
        if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableHWProfilerDataCollection) && threadInitialized
            && canUseStartUsingRI) {
...
static void jitMethodSampleInterrupt(J9VMThread *vmThread, IDATA handlerKey, void *userData)
{
    ...
    walkState.flags = J9_STACKWALK_SKIP_INLINES | J9_STACKWALK_VISIBLE_ONLY | J9_STACKWALK_INCLUDE_NATIVES
    ...
    if ((jitConfig->runtimeFlags & J9JIT_DEFER_JIT) == 0) // Reject any samples if we decided to postpone jitting
    {
        ...
        if (interpreterProfilingState == IPROFILING_STATE_OFF && !startPC
            && !TR::Options::getCmdLineOptions()->getOption(TR_DisableInterpreterProfiling)) {
            interpreterProfilingINTSamples++;
        }
#endif
        ...
        if (!TR::Options::getCmdLineOptions()->getOption(TR_MimicInterpreterFrameShape)
             ...
             */
            if (!skipSampleMethod)
                TR::Recompilation::sampleMethod(vmThread, vm, startPC, codeSize, walkState.pc, walkState.method,
...
static void jitHookClassesUnloadEnd(J9HookInterface **hookInterface, UDATA eventNum, void *eventData, void *userData)
{
    ...
    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseClassUnloading)) {
        if (TR::Options::getCmdLineOptions()->getOption(TR_PrintCodeCacheUsage)
            ...
            TR_VerboseLog::writeLineLocked(TR_Vlog_GC,
                "CodeCache after  class unloading: size=%zuKb used=%zuKb max_used=%zuKb free=%zuKb", codeCacheTotalKB,
                currTotalUsedKB, maxUsedKB, codeCacheTotalKB - currTotalUsedKB);
        }
    }
}
#endif
...
void finalizeJitPrivateThreadData(J9VMThread *currentThread)
{
    ...
    do {
        if (thread->jitPrivateData) {
            ...
            while (jitData->index % jitData->numberOfFrames != 0
                && jitData->index < (jitData->numberOfFrames) * (jitData->numberOfBuffers)) {
...
static void jitHookGlobalGCStart(J9HookInterface **hookInterface, UDATA eventNum, void *eventData, void *userData)
{
    ...
    if (TR::Options::getCmdLineOptions()->getStackPCDumpNumberOfBuffers()
        && TR::Options::getCmdLineOptions()->getStackPCDumpNumberOfFrames())
...
static void initThreadAfterCreation(J9VMThread *vmThread)
{
    ...
    if (TR::Options::getCmdLineOptions()->getOption(TR_EnableValueTracing)) {
        ...
        if (pJitConfig) {
            ...
            snprintf(fileName, sizeof(fileName), "%s_" POINTER_PRINTF_FORMAT, pJitConfig->itraceFileNamePrefix,
                vmThread);
    ...
    if (!vmThread->jitPrivateData && numberOfFrames && numberOfBuffers) {
        numberOfFrames++;
        vmThread->jitPrivateData = (J9JitPrivateThreadData *)TR_Memory::jitPersistentAlloc(
            sizeof(J9JitPrivateThreadData) + (numberOfFrames * numberOfBuffers - 1) * sizeof(U_8 *));
        J9JitPrivateThreadData *jitData = (J9JitPrivateThreadData *)vmThread->jitPrivateData;
        // 199878 : jitPersistentAlloc may return NULL and this extra diagnostics should take an extra care to not
        // segfault in such a scenario.
        if (!jitData) {
            return;
        }
...
static void jitHookThreadEnd(J9HookInterface **hookInterface, UDATA eventNum, void *eventData, void *userData)
{
    ...
    if (TR::Options::getCmdLineOptions()->getOption(TR_CountWriteBarriersRT)) {
        fprintf(stderr, "Thread %p: Executed %" OMR_PRIuPTR " barriers, %" OMR_PRIuPTR " went to slow path\n", vmThread,
            vmThread->debugEventData6, vmThread->debugEventData7);
    }
    return;
}
...
static void jitHookClassesUnload(J9HookInterface **hookInterface, UDATA eventNum, void *eventData, void *userData)
{
    ...
    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseClassUnloading)) {
        ...
        TR_VerboseLog::writeLineLocked(TR_Vlog_GC,
            "t=%lu classLoaderID=%d. Unloaded classes=%u (Total=%d). Loaded classes since last unload op=%d (Total=%d)",
            (unsigned long)compInfo->getPersistentInfo()->getElapsedTime(),
            (int)persistentInfo->getGlobalClassUnloadID(), (unsigned int)classUnloadCount,
        ...
        if (TR::Options::getCmdLineOptions()->getOption(TR_PrintCodeCacheUsage)
            ...
            TR_VerboseLog::writeLineLocked(TR_Vlog_GC,
                "CodeCache before class unloading: size=%zuKb used=%zuKb max_used=%zuKb free=%zuKb", codeCacheTotalKB,
                currTotalUsedKB, maxUsedKB, codeCacheTotalKB - currTotalUsedKB);
        }
    }
    ...
    if (table && table->isActive()) {
        ...
        if (!persistentInfo->getVisitedSuperClasses())
            persistentInfo->setVisitedSuperClasses((TR_OpaqueClassBlock **)TR_Memory::jitPersistentAlloc(
                MAX_SUPERCLASSES * sizeof(TR_OpaqueClassBlock *), TR_Memory::PersistentInfo));
...
static void jitHookClassUnload(J9HookInterface **hookInterface, UDATA eventNum, void *eventData, void *userData)
{
    ...
    if (p) {
        ...
        TR_VerboseLog::writeLineLocked(TR_Vlog_HD, "Class unloading for class=0x%p name=%.*s", j9clazz,
    ...
    if (!disableUnloadedClassRanges)
        compInfo->getPersistentInfo()->addUnloadedClass(clazz, methodsStartAddr,
            (uint32_t)(methodsEndAddr - methodsStartAddr));
...

    {
        ...
        for (J9ITable *iTableEntry = (J9ITable *)TR::Compiler->cls.convertClassOffsetToClassPtr(clazz)->iTable;
             iTableEntry; iTableEntry = iTableEntry->next) {
...
static void jitHookClassLoaderUnload(J9HookInterface **hookInterface, UDATA eventNum, void *eventData, void *userData)
{
    ...
    // Assumptions are registered during class loading event. For a class loader that has never loaded a class, the JIT
    // won't know about it
...
void setElaboratedClassPair(ElaboratedClassPair *ecp, J9JITRedefinedClass *classPair)
{
    ...
    TR_ASSERT_FATAL(ecp->freshClass != ecp->staleClass, "fresh and stale classes are the same: %p", ecp->freshClass);

    TR_ASSERT_FATAL(ecp->oldClass == ecp->freshClass || ecp->oldClass == ecp->staleClass,
        "oldClass %p matches neither freshClass %p nor staleClass %p", ecp->oldClass, ecp->freshClass, ecp->staleClass);
    ...
    ecp->newClass = ecp->oldClass == ecp->freshClass ? ecp->staleClass : ecp->freshClass;
}
    ...
    UDATA extensionsUsed)
{
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_FullSpeedDebug)) {
        for (i = 0; i < classCount; i++) {
            ...
            reportHookDetail(currentThread, "jitClassesRedefined",
                "Redefined class old=%p new=%p stale=%p fresh=%p %.*s", oldClass, newClass, staleClass, freshClass,
                length, name);
            ...
            reportHookDetail(currentThread, "jitClassesRedefined",
                "  Invalidate compilation requests for classes old=%p and new=%p", oldClass, newClass);
            ...
            for (j = 0; j < methodCount; j++) {
                ...
                TR::CodeCacheManager::instance()->onClassRedefinition(
                    reinterpret_cast<TR_OpaqueMethodBlock *>(staleMethod),
                ...
                if (staleMethod && freshMethod && (startPC = compInfo->getPCIfCompiled(staleMethod))) {
                    ...
                    if (bodyInfo) {
                        reportHookDetail(currentThread, "jitClassesRedefined",
                            "    Invalidate method body stale=%p startPC=%p", staleMethod, startPC);
                        TR::Recompilation::invalidateMethodBody(startPC, fe, TR_JitBodyInvalidations::HCR);
                    ...
                    else if (_J9ROMMETHOD_J9MODIFIER_IS_SET((J9_ROM_METHOD_FROM_RAM_METHOD(staleMethod)),
                                 J9AccNative)) {
                        reportHookDetail(currentThread, "jitClassesRedefined",
                            "No need to invalidate native method stale=%p startPC=%p", staleMethod, startPC);
                    } else {
                        reportHookDetail(currentThread, "jitClassesRedefined",
                            "WARNING!  Cannot invalidate method body stale=%p startPC=%p", staleMethod, startPC);
                        TR_ASSERT(0,
                            "JIT HCR should make all methods recompilable, so startPC=%p should have a "
                            "persistentBodyInfo",
                            startPC);
                    }
                }
            ...
            classPair = (J9JITRedefinedClass *)((char *)classPair->methodList
                + (classPair->methodCount * sizeof(struct J9JITMethodEquivalence)));
        }
    }
    ...
    for (i = 0; i < classCount; i++) {
        ...
        if (table && table->isActive() && TR::Options::sharedClassCache()
            && TR::Options::getCmdLineOptions()->getOption(TR_EnableClassChainValidationCaching)) {
            table->resetCachedCCVResult(fe, oldClass);
        }

        // Step 3  patch modified classes
        if (rat) {
            reportHookDetail(currentThread, "jitClassesRedefined", "  Notify RAT on class old=%p fresh=%p", oldClass,
                freshClass);
        ...
        for (j = 0; j < methodCount; j++) {
            ...
            if (table) {
                reportHookDetail(currentThread, "jitClassesRedefined", "    Notify CHTable on method old=%p fresh=%p",
                    oldMethod, freshMethod);
                table->methodGotOverridden(fe, compInfo->persistentMemory(), (TR_OpaqueMethodBlock *)freshMethod,
                    (TR_OpaqueMethodBlock *)oldMethod, isSMP);
            }
            // Step 4 patch modified J9Method
            if (oldMethod && newMethod && rat) {
                reportHookDetail(currentThread, "jitClassesRedefined", "    Notify RAT on method old=%p fresh=%p",
                    oldMethod, freshMethod);
                rat->notifyClassRedefinitionEvent(fe, 0, oldMethod, freshMethod);
        ...
        if (table) {
            reportHookDetail(currentThread, "jitClassesRedefined", "  Notify CHTable on class old=%p fresh=%p",
                oldClass, freshClass);
...
#endif // #if (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM) ||
       // defined(TR_HOST_ARM64))
...
void jitSetMutableCallSiteTarget(J9VMThread *vmThread, j9object_t mcs, j9object_t newTarget)
{
    ...
    uintptr_t targetOffset = fej9->getInstanceFieldOffset(mcsClass, "target", "Ljava/lang/invoke/MethodHandle;");
    ...
    if (cookie == 0) {
        if (verbose) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_HD, "%p skipping nonexistent cookie", vmThread);
        }
    } else {
        ...
        if (verbose) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_HD, "%p notifying cookie %p", vmThread, (void *)cookie);
        }

        rat->notifyMutableCallSiteChangeEvent(fej9, cookie);

        if (verbose) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_HD, "%p finished notifying cookie %p", vmThread, (void *)cookie);
        }
    }
    ...
    vmThread->javaVM->memoryManagerFunctions->j9gc_objaccess_mixedObjectStoreObject(vmThread, mcs, targetOffset,
        newTarget, 0);
}
#endif

#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
static void jitHookInterruptCompilation(J9HookInterface **hookInterface, UDATA eventNum, void *eventData,
    void *userData)
{
...
void jitUpdateMethodOverride(J9VMThread *vmThread, J9Class *cl, J9Method *overriddenMethod, J9Method *overriddingMethod)
{
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableCHOpts)) {
        ...
        compInfo->getPersistentInfo()->getPersistentCHTable()->methodGotOverridden(vm, compInfo->persistentMemory(),
            (TR_OpaqueMethodBlock *)overriddingMethod, (TR_OpaqueMethodBlock *)overriddenMethod, isSMP);
...
{
    static const char *traceIt
        = 0; //  feGetEnv("TR_TraceUpdateOverridenFlag"); //this trace should only be enabled if it is needed
    ...
    if (classDepth >= 0) // Do nothing if we don't have a superclass
    {
        ...
        while (methodCount--) {
            ...
            if (traceIt) {
                ...
                char *superSignature
                    = (char *)J9UTF8_DATA(J9ROMMETHOD_SIGNATURE(J9_ROM_METHOD_FROM_RAM_METHOD(superMethod)));
                int32_t superSigLen
                    = (int32_t)J9UTF8_LENGTH(J9ROMMETHOD_SIGNATURE(J9_ROM_METHOD_FROM_RAM_METHOD(superMethod)));
                char *superName = (char *)J9UTF8_DATA(J9ROMMETHOD_NAME(J9_ROM_METHOD_FROM_RAM_METHOD(superMethod)));
                int32_t superNameLen
                    = (int32_t)J9UTF8_LENGTH(J9ROMMETHOD_NAME(J9_ROM_METHOD_FROM_RAM_METHOD(superMethod)));
                char *subSignature
                    = (char *)J9UTF8_DATA(J9ROMMETHOD_SIGNATURE(J9_ROM_METHOD_FROM_RAM_METHOD(subMethod)));
                int32_t subSigLen
                    = (int32_t)J9UTF8_LENGTH(J9ROMMETHOD_SIGNATURE(J9_ROM_METHOD_FROM_RAM_METHOD(subMethod)));
                ...
                printf("class = %.*s superSignature = %.*s, superName = %.*s, supermodifers = %x , subSignature = "
                       "%.*s, subName = %.*s, submodifiers = %x subMethod = %p\n",
                    classNameLen, classNameChars, superSigLen, superSignature, superNameLen, superName,
                    (int)superROM->modifiers, subSigLen, subSignature, subNameLen, subName, (int)subROM->modifiers,
                    subMethod);
                printf("For subROM %p:\n", subROM);

                if (subROM->modifiers & J9AccAbstract)
                    printf("\tsubMethod is J9AccAbstract (%x)\n", J9AccAbstract);
                if (subROM->modifiers & J9AccAnnotation)
                    printf("\tsubMethod is J9AccAnnotation\n");
                if (subROM->modifiers & J9AccBridge)
                    printf("\tsubMethod is J9AccBridge\n");
                if (subROM->modifiers & J9AccEmptyMethod)
                    printf("\tsubMethod is J9AccEmptyMethod\n");
                if (subROM->modifiers & J9AccFinal)
                    printf("\tsubMethod is J9AccFinal\n");
                if (subROM->modifiers & J9AccGetterMethod)
                    printf("\tsubMethod is J9AccGetterMethod\n");
                if (subROM->modifiers & J9AccInterface)
                    printf("\tsubMethod is J9AccInterface\n");
                if (subROM->modifiers & J9AccMethodCallerSensitive)
                    printf("\tsubMethod is J9AccMethodCallerSensitive\n");
                if (subROM->modifiers & J9AccMethodFrameIteratorSkip)
                    printf("\tsubMethod is J9AccMethodFrameIteratorSkip\n");
                if (subROM->modifiers & J9AccMethodHasBackwardBranches)
                    printf("\tsubMethod is J9AccMethodHasBackwardBranches\n");
                if (subROM->modifiers & J9AccMethodHasDebugInfo)
                    printf("\tsubMethod is J9AccMethodHasDebugInfo\n");
                if (subROM->modifiers & J9AccMethodHasDefaultAnnotation)
                    printf("\tsubMethod is J9AccMethodHasDefaultAnnotation\n");
                if (subROM->modifiers & J9AccMethodHasExceptionInfo)
                    printf("\tsubMethod is J9AccMethodHasExceptionInfo\n");
                if (subROM->modifiers & J9AccMethodHasGenericSignature)
                    printf("\tsubMethod is J9AccMethodHasGenericSignature\n");
                if (subROM->modifiers & J9AccMethodHasMethodAnnotations)
                    printf("\tsubMethod is J9AccMethodHasMethodAnnotations\n");
                if (subROM->modifiers & J9AccMethodHasParameterAnnotations)
                    printf("\tsubMethod is J9AccMethodHasParameterAnnotations\n");
                if (subROM->modifiers & J9AccMethodHasStackMap)
                    printf("\tsubMethod is J9AccMethodHasStackMap\n");
                if (subROM->modifiers & J9AccMethodObjectConstructor)
                    printf("\tsubMethod is J9AccMethodObjectConstructor\n");
                if (subROM->modifiers & J9AccMethodReturn0)
                    printf("\tsubMethod is J9AccMethodReturn0\n");
                if (subROM->modifiers & J9AccMethodReturn1)
                    printf("\tsubMethod is J9AccMethodReturn1\n");
                if (subROM->modifiers & J9AccMethodReturn2)
                    printf("\tsubMethod is J9AccMethodReturn2\n");
                if (subROM->modifiers & J9AccMethodReturnA)
                    printf("\tsubMethod is J9AccMethodReturnA\n");
                if (subROM->modifiers & J9AccMethodReturnD)
                    printf("\tsubMethod is J9AccMethodReturnD\n");
                if (subROM->modifiers & J9AccMethodReturnF)
                    printf("\tsubMethod is J9AccMethodReturnF\n");
                if (subROM->modifiers & J9AccMethodReturnMask)
                    printf("\tsubMethod is J9AccMethodReturnMask\n");
                if (subROM->modifiers & J9AccMethodReturnShift)
                    printf("\tsubMethod is J9AccMethodReturnShift\n");
                if (subROM->modifiers & J9AccMethodVTable)
                    printf("\tsubMethod is J9AccMethodVTable\n");
                if (subROM->modifiers & J9AccNative)
                    printf("\tsubMethod is J9AccNative\n");
                if (subROM->modifiers & J9AccPrivate)
                    printf("\tsubMethod is J9AccPrivate\n");
                if (subROM->modifiers & J9AccProtected)
                    printf("\tsubMethod is J9AccProtected\n");
                if (subROM->modifiers & J9AccPublic)
                    printf("\tsubMethod is J9AccPublic\n");
                if (subROM->modifiers & J9AccStatic)
                    printf("\tsubMethod is J9AccStatic\n");
                if (subROM->modifiers & J9AccStrict)
                    printf("\tsubMethod is J9AccStrict\n");
                if (subROM->modifiers & J9AccSuper)
                    printf("\tsubMethod is J9AccSuper\n");
                if (subROM->modifiers & J9AccSynchronized)
                    printf("\tsubMethod is J9AccSynchronized\n");
                if (subROM->modifiers & J9AccSynthetic)
                    printf("\tsubMethod is J9AccSynthetic\n");
                if (subROM->modifiers & J9AccTransient)
                    printf("\tsubMethod is J9AccTransient\n");
                if (subROM->modifiers & J9AccVarArgs)
                    printf("\tsubMethod is J9AccVarArgs\n");
                if (subROM->modifiers & J9AccVolatile)
                    printf("\tsubMethod is J9AccVolatile\n");

                printf("For superROM %p:\n", superROM);

                if (superROM->modifiers & J9AccAbstract)
                    printf("\tsuperMethod is J9AccAbstract\n");
                if (superROM->modifiers & J9AccAnnotation)
                    printf("\tsuperMethod is J9AccAnnotationt\n");
                if (superROM->modifiers & J9AccBridge)
                    printf("\tsuperMethod is J9AccBridge\n");
                if (superROM->modifiers & J9AccEmptyMethod)
                    printf("\tsuperMethod is J9AccEmptyMethod\n");
                if (superROM->modifiers & J9AccFinal)
                    printf("\tsuperMethod is J9AccFinal\n");
                if (superROM->modifiers & J9AccGetterMethod)
                    printf("\tsuperMethod is J9AccGetterMethod\n");
                if (superROM->modifiers & J9AccInterface)
                    printf("\tsuperMethod is J9AccInterface\n");
                if (superROM->modifiers & J9AccMethodCallerSensitive)
                    printf("\tsuperMethod is J9AccMethodCallerSensitive\n");
                if (superROM->modifiers & J9AccMethodFrameIteratorSkip)
                    printf("\tsuperMethod is J9AccMethodFrameIteratorSkip\n");
                if (superROM->modifiers & J9AccMethodHasBackwardBranches)
                    printf("\tsuperMethod is J9AccMethodHasBackwardBranches\n");
                if (superROM->modifiers & J9AccMethodHasDebugInfo)
                    printf("\tsuperMethod is J9AccMethodHasDebugInfo\n");
                if (superROM->modifiers & J9AccMethodHasDefaultAnnotation)
                    printf("\tsuperMethod is J9AccMethodHasDefaultAnnotation\n");
                if (superROM->modifiers & J9AccMethodHasExceptionInfo)
                    printf("\tsuperMethod is J9AccMethodHasExceptionInfo\n");
                if (superROM->modifiers & J9AccMethodHasGenericSignature)
                    printf("\tsuperMethod is J9AccMethodHasGenericSignature\n");
                if (superROM->modifiers & J9AccMethodHasMethodAnnotations)
                    printf("\tsuperMethod is J9AccMethodHasMethodAnnotations\n");
                if (superROM->modifiers & J9AccMethodHasParameterAnnotations)
                    printf("\tsuperMethod is J9AccMethodHasParameterAnnotations\n");
                if (superROM->modifiers & J9AccMethodHasStackMap)
                    printf("\tsuperMethod is J9AccMethodHasStackMap\n");
                if (superROM->modifiers & J9AccMethodObjectConstructor)
                    printf("\tsuperMethod is J9AccMethodObjectConstructor\n");
                if (superROM->modifiers & J9AccMethodReturn0)
                    printf("\tsuperMethod is J9AccMethodReturn0\n");
                if (superROM->modifiers & J9AccMethodReturn1)
                    printf("\tsuperMethod is J9AccMethodReturn1\n");
                if (superROM->modifiers & J9AccMethodReturn2)
                    printf("\tsuperMethod is J9AccMethodReturn2\n");
                if (superROM->modifiers & J9AccMethodReturnA)
                    printf("\tsuperMethod is J9AccMethodReturnA\n");
                if (superROM->modifiers & J9AccMethodReturnD)
                    printf("\tsuperMethod is J9AccMethodReturnD\n");
                if (superROM->modifiers & J9AccMethodReturnF)
                    printf("\tsuperMethod is J9AccMethodReturnF\n");
                if (superROM->modifiers & J9AccMethodReturnMask)
                    printf("\tsuperMethod is J9AccMethodReturnMask\n");
                if (superROM->modifiers & J9AccMethodReturnShift)
                    printf("\tsuperMethod is J9AccMethodReturnShift\n");
                if (superROM->modifiers & J9AccMethodVTable)
                    printf("\tsuperMethod is J9AccMethodVTable\n");
                if (superROM->modifiers & J9AccNative)
                    printf("\tsuperMethod is J9AccNative\n");
                if (superROM->modifiers & J9AccPrivate)
                    printf("\tsuperMethod is J9AccPrivate\n");
                if (superROM->modifiers & J9AccProtected)
                    printf("\tsuperMethod is J9AccProtected\n");
                if (superROM->modifiers & J9AccPublic)
                    printf("\tsuperMethod is J9AccPublic\n");
                if (superROM->modifiers & J9AccStatic)
                    printf("\tsuperMethod is J9AccStatic\n");
                if (superROM->modifiers & J9AccStrict)
                    printf("\tsuperMethod is J9AccStrict\n");
                if (superROM->modifiers & J9AccSuper)
                    printf("\tsuperMethod is J9AccSuper\n");
                if (superROM->modifiers & J9AccSynchronized)
                    printf("\tsuperMethod is J9AccSynchronized\n");
                if (superROM->modifiers & J9AccSynthetic)
                    printf("\tsuperMethod is J9AccSynthetic\n");
                if (superROM->modifiers & J9AccTransient)
                    printf("\tsuperMethod is J9AccTransient\n");
                if (superROM->modifiers & J9AccVarArgs)
                    printf("\tsuperMethod is J9AccVarArgs\n");
                if (superROM->modifiers & J9AccVolatile)
                    printf("\tsuperMethod is J9AccVolatile\n");
            ...
            if (superMethod != subMethod) // the j9methods don't match, so set the overridden bit
            {
                ...
                vm->javaVM->internalVMFunctions->atomicOrIntoConstantPool(vm->javaVM, superMethod,
                ...
                for (int32_t k = classDepth - 1; k >= 0; k--) {
                    ...
                    vm->javaVM->internalVMFunctions->atomicOrIntoConstantPool(vm->javaVM, tempsuperMethod,
                        J9_STARTPC_METHOD_IS_OVERRIDDEN);
                }
            }
...
#if defined(J9VM_OPT_JITSERVER)
        TR_ASSERT_FATAL(compInfo->getPersistentInfo()->getRemoteCompilationMode() != JITServer::SERVER,
            "updateCHTable() should not be called on JITServer!\n");
#endif
 ...
 */
int32_t getAvailableVirtualMemoryMB(TR::CompilationInfo *compInfo, J9VMThread *vmThread,
    uint32_t *availableVirtualMemoryMB)
{
    ...
    if (j9sysinfo_get_memory_info(&memInfo) != 0 || memInfo.availVirtual == J9PORT_MEMINFO_NOT_AVAILABLE) {
    ...
    *availableVirtualMemoryMB = (uint32_t)(memInfo.availVirtual >> 20); // conversion to uint32_t is safe on 32-bit OS
    ...
    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseJitMemory)) {
        ...
        TR_VerboseLog::writeLineLocked(TR_Vlog_MEMORY, "Virtual Memory Used: %d MB out of %d MB", virtualMemoryUsedMB,
            totalVirtualMemoryMB);
    }
    ...
    */
    ...
    if (statFile) {
        ...
        while (!foundVirtualMemory && !feof(statFile)) {
            ...
            if (buffer[0] == 'V') {
                ...
                if (buffer[1] == 'm' && buffer[2] == 'S' && buffer[3] == 'i' && buffer[4] == 'z' && buffer[5] == 'e'
                    && buffer[6] == ':') {
                    int bufferIndex = 0;
                    ...
                    if ((bufferIndex + 2) < bufferSize && (bufferIndex - bufferStartIndex + 1) < 9
                        && buffer[bufferIndex] == ' ' && buffer[bufferIndex + 1] == 'k'
    ...
    The exact addresses may vary from system to system (with the exception of the "Top of Private" and "Top of Ext.
    Private" which generally stay the same). The System Storage, LSQA, Global Storage, Extended Global Storage, and
    Extended LSQA cannot be allocated. Thus the total userspace available can be approximated by doing: (Max Ext. User
    Region Address - (Ext. User Region Start - Max User Region Address))
    ...
    */
    ...
    } else // Use Dynamically determined user space
    {
        ...
        uint32_t totalVirtualMemoryMB
            = ((uint32_t)maxExtUserRegionAddress - ((uint32_t)(lda->estra) - (uint32_t)maxUserRegionAddress)) >> 20;
...
void lowerCompilationLimitsOnLowVirtualMemory(TR::CompilationInfo *compInfo, J9VMThread *vmThread)
{
    ...
    if ((availableVirtualMemoryMB < (uint32_t)TR::Options::getLowVirtualMemoryMBThreshold() / 2)
        ...
        uint32_t crtTime
            = (uint32_t)compInfo->getPersistentInfo()->getElapsedTime(); // 49 days resolution through casting
        ...
        if (TR::Options::getScratchSpaceLimit() > TR::Options::getScratchSpaceLimitKBWhenLowVirtualMemory() * 1024) {
            ...
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                    "t=%6u WARNING: changed scratchSpaceLimit to %d KB because VMemAv=%u MB", crtTime,
                    TR::Options::getScratchSpaceLimit() / 1024, availableVirtualMemoryMB);
            }
        }
        ...
        if (!compInfo->getRampDownMCT()) {
            ...
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompilationThreads)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "t=%u setRampDownMCT because VMemAv=%u MB", crtTime,
                    availableVirtualMemoryMB);
            }
        }
        ...
        if (doDisableGCR && TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_PERF, "t=%6u WARNING: GCR disabled because VMemAv=%u MB", crtTime,
                availableVirtualMemoryMB);
        }
        ...
        if (doDisableServerMode && TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_PERF, "t=%6u WARNING: server mode disabled because VMemAv=%u MB",
                crtTime, availableVirtualMemoryMB);
        }
        ...
        // Disable all future non-essential compilations if virtual memory is really low
        {
            ...
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                    "t=%6u WARNING: Disable non-essential compilations because VMemAv=%u MB", crtTime,
                    availableVirtualMemoryMB);
            }
        }
    }
...
static bool chTableOnClassLoad(J9VMThread *vmThread, TR_OpaqueClassBlock *clazz, TR::CompilationInfo *compInfo,
    TR_J9VMBase *vm)
{
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableCHOpts)
        ...
        if (info) {
            ...
            if (vm->isInterfaceClass(clazz)) {
                ...
                if (!compInfo->getPersistentInfo()->getPersistentCHTable()->classGotInitialized(vm,
                        compInfo->persistentMemory(), clazz)) {
                    TR_PersistentClassInfo *arrayClazzInfo
                        = compInfo->getPersistentInfo()->getPersistentCHTable()->findClassInfo(clazz);
                    if (arrayClazzInfo)
                        compInfo->getPersistentInfo()->getPersistentCHTable()->removeClass(vm, clazz, arrayClazzInfo,
                            false);
                }
                TR_OpaqueClassBlock *compClazz = vm->getComponentClassFromArrayClass(clazz);
                if (compClazz) {
                    TR_PersistentClassInfo *clazzInfo
                        = compInfo->getPersistentInfo()->getPersistentCHTable()->findClassInfo(compClazz);
                    if (clazzInfo && !clazzInfo->isInitialized()) {
                        bool initFailed = false;
                        if (!compInfo->getPersistentInfo()->getPersistentCHTable()->classGotInitialized(vm,
                                compInfo->persistentMemory(), compClazz))
                            initFailed = true;

                        if (!initFailed && !vm->isClassArray(compClazz) && !vm->isInterfaceClass(compClazz)
                            && !vm->isPrimitiveClass(compClazz))
                            initFailed = !updateCHTable(vmThread, ((J9Class *)compClazz));

                        if (initFailed) {
                            compInfo->getPersistentInfo()->getPersistentCHTable()->removeClass(vm, compClazz, clazzInfo,
                                false);
                            allocFailed = true;
                        }
                    }
...
static void checkForLockReservation(J9VMThread *vmThread, J9JITConfig *jitConfig, J9ClassLoader *classLoader,
    TR_OpaqueClassBlock *clazz, TR_J9VMBase *vm, TR::CompilationInfo *compInfo, char *className, int32_t classNameLen)
{
    TR::Options *options = TR::Options::getCmdLineOptions();
...
void jitHookClassLoadHelper(J9VMThread *vmThread, J9JITConfig *jitConfig, J9Class *cl, TR::CompilationInfo *compInfo,
    UDATA *classLoadEventFailed)
{
    ...
    if (p) {
        getClassNameIfNecessary(vm, clazz, className, classNameLen);
        TR_VerboseLog::writeLineLocked(TR_Vlog_HD, "--load-- loader %p, class %p : %.*s", classLoader, cl, classNameLen,
            className);
    }
    ...
    compInfo->getPersistentInfo()->getPersistentClassLoaderTable()->associateClassLoaderWithClass(vmThread, classLoader,
        clazz);
...
static bool chTableOnClassPreinitialize(J9VMThread *vmThread, J9Class *cl, TR_OpaqueClassBlock *clazz,
    TR::CompilationInfo *compInfo, TR_J9VMBase *vm)
{
    bool initFailed = false;
...
#endif
    {
        if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableCHOpts)) {
            if (!initFailed
                && !compInfo->getPersistentInfo()->getPersistentCHTable()->classGotInitialized(vm,
                    compInfo->persistentMemory(), clazz))
                initFailed = true;

            if (!initFailed && !vm->isInterfaceClass(clazz))
...
void jitHookClassPreinitializeHelper(J9VMThread *vmThread, J9JITConfig *jitConfig, J9Class *cl,
...
static void jitHookBytecodeProfiling(J9HookInterface **hook, UDATA eventNum, void *eventData, void *userData)
{
    ...
    if (compInfo->getSamplerState() == TR::CompilationInfo::SAMPLER_DEEPIDLE) {
        ...
        if (compInfo->getIProfilerBufferArrivalMonitor()) // safety check; will most likely exist
        {
            ...
            if ((oldestTime != 0)
                && (crtTime < oldestTime + TR::Options::_iProfilerBufferInterarrivalTimeToExitDeepIdle)) {
                getOutOfIdleStates(TR::CompilationInfo::SAMPLER_DEEPIDLE, compInfo,
                    "IP buffer received"); // this changes the state to IDLE
                ...
                if (TR::Options::getCmdLineOptions()->getOption(TR_VerboseInterpreterProfiling)) {
                    ...
                    TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                        "t=%u\tSampling thread avoided an interruption in DEEP_IDLE due to IProfiler buffer being "
                        "received",
                        t);
                }
            }
    ...
    if (iProfiler->getProfilerMemoryFootprint() >= TR::Options::_iProfilerMemoryConsumptionLimit) {
        if (TR::Options::getCmdLineOptions()->getOption(TR_VerboseInterpreterProfiling)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_IPROFILER, "t=%6u IProfiler exceeded memory limit %d",
                (uint32_t)compInfo->getPersistentInfo()->getElapsedTime(), iProfiler->getProfilerMemoryFootprint());
        }
        turnOffInterpreterProfiling(jitConfig);
        // Generate a trace point
    ...
    if (interpreterProfilingState == IPROFILING_STATE_GOING_OFF) {
        ...
        if (interpreterProfilingRecordsCount >= TR::Options::_iprofilerSamplesBeforeTurningOff) {
            ...
            if (TR::Options::getCmdLineOptions()->getOption(TR_VerboseInterpreterProfiling)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_IPROFILER, "t=%6u IProfiler stopped after %d records",
                    (uint32_t)compInfo->getPersistentInfo()->getElapsedTime(), TEST_records + records);
            }
            interpreterProfilingState = IPROFILING_STATE_OFF;
        }
    }
...
static void jitHookAboutToRunMain(J9HookInterface **hook, UDATA eventNum, void *eventData, void *userData)
{
    ...
    if (!event->isStatic || event->isField || strncmp(event->name, "main", 4)
        || strncmp(event->signature, "([Ljava/lang/String;)V", 22))
        return;
...
void JitShutdown(J9JITConfig *jitConfig)
{
    ...
    if (options && options->getOption(TR_DumpFinalMethodNamesAndCounts)) {
        try {
            ...
            J9::SegmentAllocator segmentAllocator(MEMORY_TYPE_JIT_SCRATCH_SPACE | MEMORY_TYPE_VIRTUAL,
                *jitConfig->javaVM);
            J9::SystemSegmentProvider regionSegmentProvider(1 << 20, 1 << 20, TR::Options::getScratchSpaceLimit(),
...
#if defined(J9VM_OPT_JITSERVER)
    JITServerStatisticsThread *statsThreadObj
        = ((TR_JitPrivateConfig *)(jitConfig->privateConfig))->statisticsThreadObject;
    if (statsThreadObj) {
    ...
    TR::CompilationInfo::get(jitConfig)->getPersistentInfo()->getInvokeExactJ2IThunkTable(); if (thunkTable)
...
void disclaimSharedClassCache(TR_J9SharedCache *sharedCache, uint64_t crtElapsedTime)
{
    ...
    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
        TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
            "t=%u JIT disclaimed %d SCC segments  RSS before=%zu KB, RSS after=%zu KB, delta=%zd KB = %5.2f%%",
...
void disclaimDataCaches(uint64_t crtElapsedTime)
{
    ...
    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
        TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
            "t=%u JIT disclaimed %d Data Cache segments  RSS before=%zu KB, RSS after=%zu KB, delta=%zd KB = %5.2f%%",
            (uint32_t)crtElapsedTime, numDisclaimed, rssBefore, rssAfter, rssBefore - rssAfter,
            ((long)(rssAfter - rssBefore) * 100.0 / rssBefore));
}
...
void disclaimIProfilerSegments(uint64_t crtElapsedTime)
{
    ...
    if (iprofilerAllocator) {
        ...
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
            TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                "t=%u JIT disclaimed %d IProfiler segments out of %d. RSS before=%zu KB, RSS after=%zu KB, delta=%zd "
                "KB = %5.2f%%",
                (uint32_t)crtElapsedTime, numSegDisclaimed, iprofilerAllocator->getNumSegments(), rssBefore, rssAfter,
...
void disclaimCodeCaches(uint64_t crtElapsedTime)
{
    ...
    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
        TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
            "t=%u JIT disclaimed %d Code Caches RSS before=%zu KB, RSS after=%zu KB, delta=%zd KB = %5.2f%%",
            (uint32_t)crtElapsedTime, numDisclaimed, rssBefore, rssAfter, rssBefore - rssAfter,
            ((long)(rssAfter - rssBefore) * 100.0 / rssBefore));
}
...
static void samplingObservationsLogic(J9JITConfig *jitConfig, TR::CompilationInfo *compInfo)
{
    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseHeartbeat)) {
        ...
        TR_VerboseLog::writeLine(TR_Vlog_INFO, "<samplewindow intervalTicks=%u interpretedMethodSamples=%u",
            jitConfig->samplingTickCount + 1 - compInfo->_stats._windowStartTick,
        ...
        TR_VerboseLog::writeLine(TR_Vlog_INFO, "  samplesSent=%u samplesReceived=%u ticksInIdleMode=%u",
            compInfo->_stats._sampleMessagesSent, compInfo->_stats._sampleMessagesReceived,
...
static void jitStateLogic(J9JITConfig *jitConfig, TR::CompilationInfo *compInfo, uint32_t diffTime)
{
    ...
    uint32_t totalCompilationsNormalized = (compInfo->_intervalStats._numRecompilationsInInterval
    ...
    if (compInfo->getSamplerState() == TR::CompilationInfo::SAMPLER_IDLE
        || // No need to acquire the monitor to read these
        ...
        (!persistentInfo->isClassLoadingPhase() && // classLoadPhase put us in STARTUP state
            ...
            compInfo->getMethodQueueSize() < TR::CompilationInfo::LARGE_QUEUE
            && // we cannot be in IDLE mode if the queue is large
            ((totalSamplesNormalized < 6 && samplesSentNormalized < 6) || // very few samples puts us in IDLE mode
                (oldState == IDLE_STATE && totalSamplesNormalized <= 15)
                || // while in IDLE mode the number of samples criterion becomes more lax
    ...
    else if ((oldState == STARTUP_STATE && compInfo->_intervalStats._numRecompilationsInInterval * 1000 / diffTime < 30
                 && // many recompilations puts us in RAMPUP
                 ((compInfo->_intervalStats._compiledMethodSamples == 0) || (iSamplesRatio > 0.25)
                     || // we could go for 0.33 when there are many samples
                     (compInfo->_intervalStats._numFirstTimeCompilationsInInterval * 1000 / diffTime
                         > 200) // 200 new compilations per second
                     ))
    ...
    // first-time-compilations/sec > threshold ==> must take into account machine speed; counts also affect this
    {
        ...
        lastTimeInJITStartupMode = crtElapsedTime;
                   ...
                   && ((compInfo->getMethodQueueSize() > TR::CompilationInfo::SMALL_QUEUE)
                       ...
                       //(compInfo->_intervalStats._interpretedMethodSamples*1000/diffTime > 10) // too many int samples
                       //==> don't go to steady state yet
        ...
        || (oldState != RAMPUP_STATE
            && ((compInfo->getMethodQueueSize() >= TR::CompilationInfo::MEDIUM_LARGE_QUEUE)
                ...
                || (oldState != STEADY_STATE && oldState != DEEPSTEADY_STATE
                    && compInfo->_intervalStats._numRecompilationsInInterval * 1000 / diffTime > 2)
    ...
    } else // We will go to STEADY or DEEP_STEADY
    {
        if (oldState == DEEPSTEADY_STATE || // keep the DEEPSTEADY_STATE
                                            ...
                                            // The 10 minutes may be shorter when multiple threads are running full
                                            // speed or longer when there isn't much work to be done
            ((oldState == STEADY_STATE)
                && ((persistentInfo->getJitTotalSampleCount()
                        - persistentInfo->getJitSampleCountWhenActiveStateEntered())
                    > 60000))) {
    ...
    // current JPQ implementation does not use this, but it may need to be re-animated shortly so leaving commented out
    // for now
    ...
    if (disableJProfilingRecomp == NULL
       ...
       if (*(TR_BlockFrequencyInfo::getEnableJProfilingRecompilation()) == 0)
          ...
          if ((crtElapsedTime - lastTimeInJITStartupMode) > intervalBase)
             ...
             if (TR::Options::getCmdLineOptions()->isAnyVerboseOptionSet(TR_VerboseJitState, TR_VerboseCompileEnd,
    TR_VerbosePerformance))
                {
                TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,"Enabling JProfiling recompilation. recompileThrehold = %d
    loopRecompileThreshold = %d nestedLoopRecompileThreshold = %d", TR_JProfiling::recompileThreshold,
    TR_JProfiling::loopRecompileThreshold, TR_JProfiling::nestedLoopRecompileThreshold);
                }
             }
          }
          ...
          if ((crtElapsedTime - lastTimeInJITStartupMode) > intervalBase + 30000)
           ...
           if (thresholdsLowered && TR::Options::getCmdLineOptions()->isAnyVerboseOptionSet(TR_VerboseJitState,
    TR_VerboseCompileEnd, TR_VerbosePerformance))
              {
              TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "Lowering JProfiling recompilation thresholds.
    recompileThrehold = %d loopRecompileThreshold = %d nestedLoopRecompileThreshold = %d",
    TR_JProfiling::recompileThreshold, TR_JProfiling::loopRecompileThreshold,
    TR_JProfiling::nestedLoopRecompileThreshold);
              }
          }
       }*/
    /*if (disableJProfilingRecomp == NULL
    ...
    if (!TR::Options::getAOTCmdLineOptions()->getOption(TR_DisableGuardedCountingRecompilations)
        ...
        if (!persistentInfo->_countForRecompile) // if counting is not yet enabled
        {
            ...
            if (TR::Options::getAOTCmdLineOptions()->getOption(TR_EnableMultipleGCRPeriods)
                ...
                if (javaVM->phase == J9VM_PHASE_NOT_STARTUP && // Do not enable GCR counting during VM startup
                    ...
                    (!lookAtJitStateForGCREnablement
                        || (newState != STARTUP_STATE
            ...
            if (enable) {
                ...
                if (TR::Options::isAnyVerboseOptionSet(TR_VerboseJitState, TR_VerbosePerformance))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "t=%6u GCR enabled; GCR queued=%d",
        ...
        } else // GCR counting is already enabled; see if we need to disable it
        {
            if ((TR::Options::getAOTCmdLineOptions()->getOption(TR_EnableMultipleGCRPeriods)
                ...
                ((lookAtJitStateForGCREnablement && newState == STARTUP_STATE) || newState == DEEPSTEADY_STATE
                    || compInfo->getNumGCRRequestsQueued()
                        > TR::Options::_GCRQueuedThresholdForCounting + GCR_HYSTERESIS)) {
                persistentInfo->_countForRecompile = 0; // disable counting
                // write a message in the vlog
                if (TR::Options::isAnyVerboseOptionSet(TR_VerboseJitState, TR_VerbosePerformance))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "t=%6u GCR disabled; GCR queued=%d",
                        (uint32_t)crtElapsedTime, compInfo->getNumGCRRequestsQueued());
            }
        }
    }
    ...
    if (persistentInfo->isRuntimeInstrumentationEnabled()) {
        ...
        if (TR::Options::_hwProfilerExpirationTime != 0 && crtElapsedTime > TR::Options::_hwProfilerExpirationTime) {
            if (!hwProfiler->isExpired()) {
                ...
                if (TR::Options::isAnyVerboseOptionSet(TR_VerboseHWProfiler, TR_VerbosePerformance))
                    TR_VerboseLog::writeLineLocked(TR_Vlog_HWPROFILER,
                        "Buffer processing is disabled because expiration time has been reached");
            }
    ...
    if (newStarvationStatus != compInfo->getStarvationDetected()) // did the status change?
    {
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
            TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                "t=%6u Starvation status changed to %d QWeight=%d CompCPUUtil=%d CompThreadsActive=%d",
    ...
    if (TR::Options::getCmdLineOptions()->getOption(TR_EnableAppThreadYield)) {
        ...
        int32_t newSleepNano
            = starvation != TR_yes ? 0 : compInfo->computeAppSleepNano(); // TODO should we look at JIT state as well
        if (newSleepNano != oldSleepNano) {
            compInfo->setAppSleepNano(newSleepNano);
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
                TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "t=%6u SleepTime changed from %d to %d QWeight=%d",
                    (uint32_t)crtElapsedTime, oldSleepNano, newSleepNano, compInfo->getOverallQueueWeight());
        }
    }
    ...
    if (IProfilerOffSinceStartup && !TR::Options::getCmdLineOptions()->getOption(TR_DisableInterpreterProfiling)
        ...
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseJitState)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_IPROFILER,
                "t=%6u IProfiler current fail rate = %u total=%u fail=%u samplesInBuffer=%u", (uint32_t)crtElapsedTime,
                failRate, iProfiler->getTotalReadSampleRequests(), iProfiler->getFailedReadSampleRequests(),
                iProfiler->numSamplesInHistoryBuffer());
        }
    ...
    if (TR::Options::getCmdLineOptions()->getOption(TR_UseIdleTime)
        ...
        if (!compInfo->getLowPriorityCompQueue().isTrackingEnabled()
            && timeToAllocateTrackingHT == 0xffffffffffffffff) {
    ...
    if (javaVM->phase != J9VM_PHASE_NOT_STARTUP) // once in NOT_STARTUP we cannot perform any changes
    {
        if (javaVM->phase == J9VM_PHASE_STARTUP) {
            ...
            if (!TR::Options::getCmdLineOptions()->getOption(TR_AssumeStartupPhaseUntilToldNotTo)
                || (persistentInfo->getExternalStartupEndedSignal()
                    && !TR::Options::getCmdLineOptions()->getOption(TR_UseStrictStartupHints))) {
        ...
        {
            // TR_ASSERT(!TR::Options::getCmdLineOptions()->getOption(TR_AssumeStartupPhaseUntilToldNotTo), "assertion
            // failure"); // we should be in STARTUP
            //  Use JIT heuristics if (1) 'beginningOfStartup' hint didn't arrive yet  OR
            //  (2) Both 'beginningOfStartup' and 'endOfStartup' hints arrived,
            //  but we don't follow hints strictly outside startup
            if (!TR::Options::getCmdLineOptions()->getOption(TR_AssumeStartupPhaseUntilToldNotTo)
                ...
                if (crtElapsedTime
                    >= (uint64_t)persistentInfo->getClassLoadingPhaseGracePeriod()) // grace period has ended
                {
                    if (newState == STARTUP_STATE) {
        ...
        if (javaVM->phase == J9VM_PHASE_NOT_STARTUP) {
            ...
            if (TR::Options::getCmdLineOptions()->isAnyVerboseOptionSet(TR_VerboseJitState, TR_VerboseCompileEnd,
                    TR_VerbosePerformance)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_JITSTATE, "t=%6u VM changed state to NOT_STARTUP",
            ...
            // If we don't hit idle state when we exit JVM startup, set the desired timestamp to allocate the tracking
            // hashtable
            ...
            if ((TR::Options::getCmdLineOptions()->getOption(TR_RestrictInlinerDuringStartup)
                ...
                if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_PERF, "t=%6u Stopped restricting the inliner",
                        (uint32_t)crtElapsedTime);
                }
            }
            ...
            if (TR::Options::getCmdLineOptions()->getOption(TR_UseHigherMethodCountsAfterStartup)
                && TR::Options::sharedClassCache()) {
                if (TR::Options::getCmdLineOptions()->isAnyVerboseOptionSet(TR_VerboseJitState,
                        TR_VerbosePerformance)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "t=%6u JIT counts: %d %d %d  AOT counts: %d %d %d",
                        (uint32_t)crtElapsedTime, TR::Options::getCmdLineOptions()->getInitialCount(),
                ...
                if (TR::Options::getCmdLineOptions()->isAnyVerboseOptionSet(TR_VerboseJitState,
                        TR_VerbosePerformance)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                        "t=%6u JIT changed invocation counts to: JIT: %d %d %d  AOT: %d %d %d",
                        (uint32_t)crtElapsedTime, TR::Options::getCmdLineOptions()->getInitialCount(),
                        TR::Options::getCmdLineOptions()->getInitialBCount(),
                        TR::Options::getCmdLineOptions()->getInitialMILCount(),
                        TR::Options::getAOTCmdLineOptions()->getInitialCount(),
        ...
        if (interpreterProfilingWasOnAtStartup) // if we used -Xjit:disableInterpreterProfiling don't bother
        {
            ...
            if (javaVM->phase == J9VM_PHASE_EARLY_STARTUP
                || (persistentInfo->isClassLoadingPhase() && javaVM->phase == J9VM_PHASE_STARTUP)) {
            ...
            if (newIprofilerMaxCount != -1) // needs to be updated
            {
                ...
                if (TR::Options::isAnyVerboseOptionSet(TR_VerboseJitState, TR_VerbosePerformance)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_JITSTATE, "t=%6u Changing maxIProfilingCount to %d",
                        (uint32_t)crtElapsedTime, newIprofilerMaxCount);
                }
            }
        } // if (interpreterProfilingWasOnAtStartup)
    } // if (javaVM->phase != J9VM_PHASE_NOT_STARTUP)
    ...
    if (newState != oldState) // state changed
    {
        ...
        if (oldState == IDLE_STATE) {
            if (firstIdleStateAfterStartup) // This flag only gets set once if we happen to be in idle state when we
                                            // exit JVM startup
            {
                ...
                if (TR::Options::getCmdLineOptions()->getOption(TR_UseIdleTime)
                    ...
                    if (TR::Options::_compilationDelayTime > 0
                        && (uint64_t)TR::Options::_compilationDelayTime * 1000 > t)
                        t = TR::Options::_compilationDelayTime * 1000;
                    timeToAllocateTrackingHT = t;
                }
            }
        ...
        if (TR::Options::isAnyVerboseOptionSet(TR_VerboseJitState, TR_VerbosePerformance)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_JITSTATE,
                "t=%6u JIT changed state from %s to %s cSmpl=%3u iSmpl=%3u comp=%3u recomp=%3u, Q_SZ=%3d CLP=%s "
                "jvmCPU=%d%%",
                (uint32_t)crtElapsedTime, jitStateNames[oldState], jitStateNames[newState],
                compInfo->_intervalStats._compiledMethodSamples, compInfo->_intervalStats._interpretedMethodSamples,
                compInfo->_intervalStats._numFirstTimeCompilationsInInterval,
    ...
    if (lateDisclaimNeeded) {
        ...
        if (cpuUtil->getVmTotalCpuTime() >= persistentInfo->getLateSCCDisclaimTime()) {
            ...
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_PERF, "t=%u JIT issuing late SCC disclaim",
                    (uint32_t)crtElapsedTime);
            }
        }
    }
    ...
    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseJitState)) {
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITSTATE,
            "t=%6u oldState=%s newState=%s cls=%3u ssn=%u tsn=%3u cSmpl=%3u iSmpl=%3u comp=%3u recomp=%3u, Q_SZ=%3d "
            "VMSTATE=%d jvmCPU=%d%%",
            (uint32_t)crtElapsedTime, jitStateNames[oldState], jitStateNames[newState],
            numClassesLoadedInIntervalNormalized, samplesSentNormalized, totalSamplesNormalized,
            compInfo->_intervalStats._compiledMethodSamples, compInfo->_intervalStats._interpretedMethodSamples,
            compInfo->_intervalStats._numFirstTimeCompilationsInInterval,
    ...
    if (crtElapsedTime >= timeToAllocateTrackingHT) {
        compInfo->getLowPriorityCompQueue().startTrackingIProfiledCalls(
            TR::Options::_numIProfiledCallsToTriggerLowPriComp);
...
bool CPUThrottleEnabled(TR::CompilationInfo *compInfo, uint64_t crtTime)
{
    ...
    if (TR::Options::_stopThrottlingTime != 0 && crtTime >= (uint64_t)TR::Options::_stopThrottlingTime) {
        if (compInfo->exceedsCompCpuEntitlement() != TR_no) {
            ...
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
                TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                    "t=%6u Changed throttling value for compilation threads to NO because throttling reached "
                    "expiration time",
                    (uint32_t)crtTime);
        }
        return false;
...
static void DoCalculateOverallCompCPUUtilization(TR::CompilationInfo *compInfo, uint64_t crtTime,
    J9VMThread *currentThread, int32_t *cpuUtilizationValues)
{
    ...
    if (TR::Options::isAnyVerboseOptionSet(TR_VerboseCompilationThreads, TR_VerboseCompilationThreadsDetails)) {
        TR_VerboseLog::CriticalSection vlogLock;
        TR_VerboseLog::write(TR_Vlog_INFO, "t=%6u TotalCompCpuUtil=%3d%%.", static_cast<uint32_t>(crtTime),
            totalCompCPUUtilization);
        TR::CompilationInfoPerThread * const *arrayOfCompInfoPT = compInfo->getArrayOfCompilationInfoPerThread();
        for (int32_t i = compInfo->getFirstCompThreadID(); i <= compInfo->getLastCompThreadID(); i++) {
            int32_t cpuArrIndex = i - compInfo->getFirstCompThreadID();
            const CpuSelfThreadUtilization &cpuUtil = arrayOfCompInfoPT[i]->getCompThreadCPU();
            TR_VerboseLog::write(" compThr%d:%3d%% (%2d%%, %2d%%) ", i, cpuUtilizationValues[cpuArrIndex],
                cpuUtil.getThreadLastCpuUtil(), cpuUtil.getThreadPrevCpuUtil());
...
void CalculateOverallCompCPUUtilization(TR::CompilationInfo *compInfo, uint64_t crtTime, J9VMThread *currentThread)
{
    if (compInfo->getOverallCompCpuUtilization() >= 0) // No error so far
    {
        if (compInfo->getNumUsableCompilationThreads() < 8) {
            ...
            int32_t *cpuUtilizationValues = static_cast<int32_t *>(j9mem_allocate_memory(
                compInfo->getNumUsableCompilationThreads() * sizeof(int32_t), J9MEM_CATEGORY_JIT));
...
void CPUThrottleLogic(TR::CompilationInfo *compInfo, uint64_t crtTime)
{
    ...
    if (totalCompCPUUtilization >= 0) {
        ...
        // The solution is to go into a transient state; so from TR_yes we go into TR_maybe and from TR_maybe we go into
        // TR_no
        ...
        {
            TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                "t=%6u Changed throttling value for compilation threads to %s because compCPUUtil=%d",
                (uint32_t)crtTime,
                compInfo->exceedsCompCpuEntitlement() == TR_yes         ? "YES"
                    : compInfo->exceedsCompCpuEntitlement() == TR_maybe ? "MAYBE"
                                                                        : "NO",
                totalCompCPUUtilization);
        }
    } else {
    ...
    if (cmdLineOptions->getOption(TR_ExperimentalClassLoadPhase)) {
        ...
        if (numTicksCapped < 2) {
            if (numTicksCapped == 0) {
                ...
                int32_t variance
                    = TR::Options::_classLoadingPhaseVariance < 100 ? TR::Options::_classLoadingPhaseVariance : 0;
                int32_t newCLPThreshold = (int32_t)(0.01
                    * (TR::Options::_classLoadingPhaseThreshold * (100 + variance)
                        - 2 * TR::Options::_classLoadingRateAverage * TR::Options::_classLoadingPhaseThreshold
                            * variance / (TR::Options::_classLoadingRateAverage + classLoadRateForFirstInterval)));
                ...
                int32_t newSecondaryCLPThreshold
                ...
                // TR::Options::_waitTimeToExitStartupMode = (int32_t)
                // (TR::Options::_waitTimeToExitStartupMode/scalingFactor);

                if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCLP)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                        "ScalingFactor=%.2f Changed CLPTHreshold to %d secondaryCLPThreshold to %d", scalingFactor,
                        newCLPThreshold, newSecondaryCLPThreshold);
                }
        ...
        TR::Options::_userClassLoadingPhase
            = (userLoadedClasses * 1024 / diffTime >= TR::Options::_userClassLoadingPhaseThreshold);

        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCLP)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_MEMORY,
                "diffTime %d  classes %d  userClasses %d  threshold %d  secondaryThreshold %d", diffTime, loadedClasses,
                userLoadedClasses, TR::Options::_classLoadingPhaseThreshold,
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableInterpreterProfiling)) {
        ...
        } else // STATE != OFF
        {
            ...
            if (TR::Options::getCmdLineOptions()->getOption(TR_UseOldIProfilerDeactivationLogic)) {
                ...
                if (interpreterProfilingINTSamples > 0 || interpreterProfilingJITSamples > 0) {
                    if (interpreterProfilingINTSamples <= TR::Options::_iprofilerReactivateThreshold
                        && (((float)interpreterProfilingINTSamples)
                               / ((float)(interpreterProfilingINTSamples + interpreterProfilingJITSamples)))
                            < (((float)TR::Options::_iprofilerIntToTotalSampleRatio) / 100))
                        interpreterProfilingMonitoringWindow++;
                    else // reset the window
                        interpreterProfilingMonitoringWindow = 0;
...
// int32_t samplerThreadStateFrequencies[TR::CompilationInfo::SAMPLER_LAST_STATE+1] = {0, 2, 1000, 100000, INT_MAX,
// INT_MAX, -1};
...
void getOutOfIdleStatesUnlocked(TR::CompilationInfo::TR_SamplerStates expectedState, TR::CompilationInfo *compInfo,
    const char *reason)
{
    ...
    if (compInfo->getSamplerState() == TR::CompilationInfo::SAMPLER_DEEPIDLE) {
        ...
        persistentInfo->setLastTimeThreadsWereActive(
            crtTime); // make the sampler thread wait for another 5 sec before entering IDLE again
        ...
        if (currentVMState == J9VM_RUNTIME_STATE_IDLE) {
            if (vm->internalVMFunctions->updateVMRuntimeState(vm, J9VM_RUNTIME_STATE_ACTIVE)
                ...
                TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                    "t=%u\tSampling thread interrupted and changed VM state to %u", (uint32_t)crtTime,
                    J9VM_RUNTIME_STATE_ACTIVE);
        }
    }
    ...
    if (TR::Options::getVerboseOption(TR_VerbosePerformance)) {
        TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
            "t=%u\tSampling thread interrupted and changed state to %s and frequency to %d ms due to %s",
...
void getOutOfIdleStates(TR::CompilationInfo::TR_SamplerStates expectedState, TR::CompilationInfo *compInfo,
    const char *reason)
{
...
void samplerThreadStateLogic(TR::CompilationInfo *compInfo, TR_FrontEnd *fe, int32_t numActiveThreads)
{
    ...
    TR_ASSERT(samplerState != TR::CompilationInfo::SAMPLER_NOT_INITIALIZED,
        "samplerThreadStateLogic: sampler must be initialized");
    TR_ASSERT(samplerState != TR::CompilationInfo::SAMPLER_STOPPED,
        "once stopped, samplerThread cannot call samplerThreadLogic again");
    ...
    if (TR::Options::_samplingThreadExpirationTime >= 0
        ...
        if (samplerState != TR::CompilationInfo::SAMPLER_SUSPENDED) // Nothing to do if sampler is already suspended
        {
            ...
            jitConfig->samplingFrequency
                = MAX_SAMPLING_FREQUENCY; // sampling frequency is signed int so set it to 2^31 - 1
        ...
        switch (samplerState) {
            case TR::CompilationInfo::SAMPLER_DEFAULT:
                ...
                if (numActiveThreads == 0
                    && crtTime - persistentInfo->getLastTimeThreadsWereActive()
                        > TR::Options::_waitTimeToEnterIdleMode) // 5 seconds of inactivity
                {
                    newSamplerState = TR::CompilationInfo::SAMPLER_IDLE;
                    jitConfig->samplingFrequency
                        = TR::Options::getSamplingFrequencyInIdleMode(); // sample every 1000 ms
            ...
            case TR::CompilationInfo::SAMPLER_IDLE: // we may go DEFAULT or DEEP_IDLE
                if (numActiveThreads > 1 || (numActiveThreads == 1 && foundThreadActiveDuringIdleMode)) {
                    ...
                    if (timeInIdle > waitTimeToEnterDeepIdle) {
                        if (TR::Options::_samplingFrequencyInDeepIdleMode
                            > 0) // setting frequency==0 is a way of disabling DEEPIDLE
                        {
                            ...
                            if (compInfo->getPrevSamplerState() != TR::CompilationInfo::SAMPLER_DEEPIDLE) {
                                ...
                                if ((J9VM_RUNTIME_STATE_ACTIVE == currentVMState)
                                    ...
                                    && (crtTime - persistentInfo->getLastTimeSamplerThreadEnteredDeepIdle()
                                        >= waitTimeInDeepIdleToNotifyVM))
                                    newVMState = J9VM_RUNTIME_STATE_IDLE;
                            }
                            // Enter DEEPIDLE
                            ...
                            if ((J9VM_RUNTIME_STATE_ACTIVE == currentVMState) && (waitTimeInDeepIdleToNotifyVM != -1)
            ...
            case TR::CompilationInfo::SAMPLER_SUSPENDED: // DisableJIT may request a samplerThread suspension
                newSamplerState
                    = TR::CompilationInfo::SAMPLER_SUSPENDED; // samplerThread will pick up the new frequency
                jitConfig->samplingFrequency
                    = MAX_SAMPLING_FREQUENCY; // sampling frequency is signed int so set it to 2^31 - 1
                persistentInfo->setLastTimeSamplerThreadWasSuspended(crtTime);
                if (J9VM_RUNTIME_STATE_IDLE
                    == currentVMState) // since sampler is being suspended, bring VM back to active
                    newVMState = J9VM_RUNTIME_STATE_ACTIVE;
                break;
    ...
    if (newVMState != currentVMState) {
        if (vm->internalVMFunctions->updateVMRuntimeState(vm, newVMState)
            && (TR::Options::getVerboseOption(TR_VerbosePerformance)))
            TR_VerboseLog::writeLineLocked(TR_Vlog_PERF, "t=%u\tSampling thread changed VM state to %u",
                (uint32_t)crtTime, newVMState);
    }
    ...
    if (samplerState != newSamplerState) {
        ...
        if (TR::Options::getVerboseOption(TR_VerbosePerformance)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                "t=%u\tSampling thread changed state to %s and frequency to %d ms", (uint32_t)crtTime,
                samplerThreadStateNames[compInfo->getSamplerState()], jitConfig->samplingFrequency);
        }
    }
    // FIXME:
...
void inlinerAggressivenessLogic(TR::CompilationInfo *compInfo)
{
    ...
    // If we want the latter then we have to change getVmTotalCpuTimeWhenStartupStateEntered() to
    // getVmTotalCpuTimeWhenStartupStateExited()
    if (TR::Options::getCmdLineOptions()->getOption(TR_UseVmTotalCpuTimeAsAbstractTime)) {
        ...
        } else // if we cannot get JVM cpu utilization, then the options to tune the algorithm will contain wrong values
        {
            ...
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
                TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                    "Changed from JVM time to JIT samples for abstract time measurement");
            // Set default values
            TR::Options::_abstractTimeGracePeriod = DEFAULT_ABSTRACT_TIME_SAMPLES_GRACE_PERIOD;
            TR::Options::_abstractTimeToReduceInliningAggressiveness
                = DEFAULT_ABSTRACT_TIME_SAMPLES_TO_REDUCE_INLINING_AGGRESSIVENESS;
    ...
    } else if (abstractTimeElapsed
        >= TR::Options::_abstractTimeGracePeriod + TR::Options::_abstractTimeToReduceInliningAggressiveness) {
        inliningAggressiveness = 0;
    } else {
        inliningAggressiveness = 100
            - 100 * (abstractTimeElapsed - (uint64_t)TR::Options::_abstractTimeGracePeriod)
                / (uint64_t)TR::Options::_abstractTimeToReduceInliningAggressiveness;
    }
    ...
    if (inliningAggressiveness != persistentInfo->getInliningAggressiveness()) {
        ...
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
            TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "inliningAggressiveness changed to %d",
                inliningAggressiveness);
    }
}
...
public:
    ...
    bool update(TR::CompilationInfo *compInfo)
    {
        if (compInfo->getCpuUtil()->hasValidData()) {
            uint32_t crtCpu
                = static_cast<uint32_t>(compInfo->getCpuUtil()->getVmTotalCpuTime() / 1000000); // convert from ns to ms
...
#if defined(J9VM_OPT_CRIU_SUPPORT)
static void suspendSamplerThreadForCheckpoint(J9VMThread *samplerThread, J9JITConfig *jitConfig,
    TR::CompilationInfo *compInfo)
{
    ...
    if (compInfo->getCRRuntime()->shouldSuspendThreadsForCheckpoint()) {
        ...
        TR_ASSERT_FATAL(compInfo->getSamplingThreadLifetimeState() == TR::CompilationInfo::SAMPLE_THR_INITIALIZED,
            "Sampler Thread Lifetime State %d is not SAMPLE_THR_INITIALIZED!",
            compInfo->getSamplingThreadLifetimeState());
        ...
        if (compInfo->getSamplingThreadLifetimeState() == TR::CompilationInfo::SAMPLE_THR_RESUMING) {
            ...
            if (TR::Options::isAnyVerboseOptionSet())
                TR_VerboseLog::writeLineLocked(TR_Vlog_CHECKPOINT_RESTORE,
                    "Sampling Thread Lifetime State is %p which is not %p!", compInfo->getSamplingThreadLifetimeState(),
                    TR::CompilationInfo::SAMPLE_THR_RESUMING);
        }
...
static void computeJVMStarvation(int32_t jvmCPUUtil, TR::CompilationInfo *compInfo)
{
    if (compInfo->isJVMStarved()) {
        if (jvmCPUUtil >= TR::Options::_jvmStarvationThreshold + 10) {
            ...
            if (TR::Options::getVerboseOption(TR_VerbosePerformance)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_PERF, "Exited JVM starvation mode. jvmCPUUtil=%d%%",
                    (int)jvmCPUUtil);
            }
        }
    } else {
        if (jvmCPUUtil <= TR::Options::_jvmStarvationThreshold - 10) {
            ...
            if (TR::Options::getVerboseOption(TR_VerbosePerformance)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_PERF, "Entered JVM starvation mode. jvmCPUUtil=%d%%",
                    (int)jvmCPUUtil);
            }
        }
    }
...
static void clearJVMStarvation(int32_t cpuIdle, int32_t vmCpuUsage, TR::CompilationInfo *compInfo)
{
    if (compInfo->isJVMStarved()) {
        compInfo->setIsJVMStarved(false);
        if (TR::Options::getVerboseOption(TR_VerbosePerformance)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_PERF, "Exited JVM starvation mode. cpuIdle=%d vmCpuUsage=%d",
                (int)cpuIdle, (int)vmCpuUsage);
        }
    }
}
    ...
    if (TR::Options::_availableCPUPercentage != 100) {
        ...
        if (cpuUtil->hasValidData()) {
            ...
            if (vmCpuUsage + 5 < compInfo->getJvmCpuEntitlement()) // at least 5% unutilized by this JVM
            {
                ...
                } else // Machine has very little idle CPU if any. It's likely this JVM cannot use its entitlement
...
static int32_t J9THREAD_PROC samplerThreadProc(void *entryarg)
{
    ...
    int rc = vm->internalVMFunctions->internalAttachCurrentThread(vm, &samplerThread, NULL,
    ...
    if (TR::Options::isAnyVerboseOptionSet()) {
        ...
        char timestamp[32];
        bool incomplete;
        omrstr_ftime_ex(timestamp, sizeof(timestamp), "%b %d %H:%M:%S %Y", persistentInfo->getStartTime(),
        ...
        if (phMemAvail != OMRPORT_MEMINFO_NOT_AVAILABLE)
            TR_VerboseLog::writeLine(TR_Vlog_INFO, "Free Physical Memory: %lld MB %s", phMemAvail >> 20,
        ...
        if (compInfo->isWarmSCC() != TR_maybe && TR::Options::getVerboseOption(TR_VerbosePerformance)) {
            ...
            TR_VerboseLog::writeLine(TR_Vlog_INFO, "\tSCCname:%s   SCCpath:%s", javacoreData->cacheName,
                javacoreData->cacheDir);
            TR_VerboseLog::writeLine(TR_Vlog_INFO,
                "\tSCC_stats_bytes: size=%u free=%u ROMClass=%u AOTCode=%u AOTData=%u JITHint=%u JITProfile=%u",
                javacoreData->cacheSize, javacoreData->freeBytes, javacoreData->romClassBytes, javacoreData->aotBytes,
                javacoreData->aotDataBytes, javacoreData->jitHintDataBytes, javacoreData->jitProfileDataBytes);
            TR_VerboseLog::writeLine(TR_Vlog_INFO,
                "\tSCC_stats_#:     ROMClasses=%u AOTMethods=%u AOTDataEntries=%u AOTHints=%u AOTJitProfiles=%u",
                javacoreData->numROMClasses, javacoreData->numAOTMethods, javacoreData->numAotDataEntries,
                javacoreData->numJitHints, javacoreData->numJitProfiles);
#if defined(OMR_GC_COMPRESSED_POINTERS)
            const TR_AOTHeader *hdrInCache
                = TR_SharedCacheRelocationRuntime::getStoredAOTHeaderWithConfig(vm->sharedClassConfig, samplerThread);
            if (NULL != hdrInCache) {
                TR_VerboseLog::writeLine(TR_Vlog_INFO, "\tAOT header compressedRefs shiftAmount=%u",
                    hdrInCache->compressedPointerShift);
            }
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableScorchingSampleThresholdScalingBasedOnNumProc)) {
        ...
        if (TR::Compiler->target.numberOfProcessors() <= TR::Options::_lowerBoundNumProcForScaling
            ...
            if (TR::Options::isAnyVerboseOptionSet()) {
                float scorchingCpuTarget = TR::Options::_scorchingSampleThreshold <= TR::Options::_sampleInterval
                    ? 100.0
    ...
    while (!shutdownSamplerThread) {
        while (!shutdownSamplerThread && // watch for shutdown signals
            j9thread_sleep_interruptable((IDATA)samplingPeriod, 0)
                == 0) // Anything non-0 is an error condition so we shouldn't do the sampling //!= J9THREAD_INTERRUPTED)
        {
            ...
            if (crtTime - oldSyncTime >= 100) // every 100 ms
            {
                ...
                if (TR::Options::_compilationExpirationTime > 0 && !persistentInfo->getDisableFurtherCompilation()) {
...
#if defined(J9VM_OPT_SHARED_CLASSES) && defined(J9VM_INTERP_AOT_RUNTIME_SUPPORT)
                        // Emit SCC tracepoint
                        if (TR::Options::sharedClassCache() && TrcEnabled_Trc_JIT_SCCInfo && vm->sharedClassConfig
                            && vm->sharedClassConfig->getJavacoreData) {
                            ...
                            vm->sharedClassConfig->getJavacoreData(vm,
                                scc); // need to rebuild javacore data or else it will be stale
                            Trc_JIT_SCCInfo(samplerThread, scc->cacheName, scc->cacheDir, scc->cacheSize,
                                scc->freeBytes, scc->softMaxBytes, scc->romClassBytes, scc->aotBytes, scc->aotDataBytes,
                                scc->jitHintDataBytes, scc->jitProfileDataBytes, scc->numROMClasses, scc->numAOTMethods,
                                fe->sharedCache()->getSharedCacheDisabledReason());
                        }
#endif
                        // Every 5 minutes
                        if (crtTime > lastFiveMinuteCheck + 5 * 60 * 1000) {
                            lastFiveMinuteCheck = crtTime;

                            if (trPersistentMemory
                                && TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseJitMemory))
                            ...
                            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseJitMemory)) {
                                ...
                                uint64_t phMemAvail = compInfo->computeAndCacheFreePhysicalMemory(incomplete);
                                if (phMemAvail != OMRPORT_MEMINFO_NOT_AVAILABLE)
                                    TR_VerboseLog::writeLine(TR_Vlog_INFO, "Free Physical Memory: %lld MB %s",
                                        phMemAvail >> 20, incomplete ? "estimated" : "");
                                else
                                    TR_VerboseLog::writeLine(TR_Vlog_INFO, "Free Physical Memory: Unavailable");
                                ...
                                for (int32_t i = 0; i < LastAssumptionKind; i++)
                                    TR_VerboseLog::writeLine(TR_Vlog_MEMORY,
                                        "\tAssumptionType=%d allocated=%d reclaimed=%d", i, rat->getAssumptionCount(i),
                                        rat->getReclaimedAssumptionCount(i));
                            }
                            ...
                            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseVMemAvailable)) {
                                ...
                                if (j9sysinfo_get_memory_info(&memInfo) == 0
                                    ...
                                    TR_VerboseLog::writeLineLocked(TR_Vlog_MEMORY, "t=%6u VMemAv=%u MB",
                                        (uint32_t)crtTime, (uint32_t)(memInfo.availVirtual >> 20));
                                }
                            }
#endif
                        } // Every 5 minutes
            ...
            //
            TR::DebugCounterGroup *dynamicCounters
                = TR::CompilationInfo::get(jitConfig)->getPersistentInfo()->getDynamicCounters();
            if (dynamicCounters)
                dynamicCounters->accumulate();
            if (TR::Options::getCmdLineOptions()->getDebugCounterWarmupSeconds()
                    > persistentInfo->getLastDebugCounterResetSeconds()
                ...
                TR::DebugCounterGroup *staticCounters
                    = TR::CompilationInfo::get(jitConfig)->getPersistentInfo()->getStaticCounters();
                if (staticCounters)
            ...
            if ((crtTime - lastTimeCpuUsageCircularBufferUpdated)
                > (TR::Options::_cpuUsageCircularBufferUpdateFrequencySec * 1000)) {
            ...
            if (persistentInfo->_countForRecompile) // No need to check if GCR is disabled
            {
                if (TR::Options::getAOTCmdLineOptions()->getOption(TR_EnableMultipleGCRPeriods)
                    || TR::Options::getJITCmdLineOptions()->getOption(TR_EnableMultipleGCRPeriods)) {
                    if (compInfo->getNumGCRRequestsQueued()
                        > TR::Options::_GCRQueuedThresholdForCounting + GCR_HYSTERESIS) {
                        persistentInfo->_countForRecompile = 0; // disable counting
                        // write a message in the vlog
                        if (TR::Options::isAnyVerboseOptionSet(TR_VerboseJitState, TR_VerbosePerformance))
                            TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "t=%6u GCR disabled; GCR queued=%d",
                                (uint32_t)crtTime, compInfo->getNumGCRRequestsQueued());
                    }
                }
            }
            ...
            if (diffTime >= (uint32_t)TR::Options::_classLoadingPhaseInterval) // time to re-analyze
            {
                ...
                else if (TR::Options::isAnyVerboseOptionSet(TR_VerboseCompilationThreads,
                             TR_VerboseCompilationThreadsDetails)
                    || TrcEnabled_Trc_JIT_CompCPU) {
                    CalculateOverallCompCPUUtilization(compInfo, crtTime, samplerThread);
                }
                ...
                if (persistentInfo->getRemoteCompilationMode() == JITServer::CLIENT
                    ...
                    && // FSD interferes with recompilations in LPQ
                    compInfo->getJvmCpuEntitlement() <= 800) // More than 8 CPUs can consume CPU too fast
                {
                    ...
                    // This code needs to stay after the code that updates the CPU utilization so that we can get fresh
                    // values
                    if (compDensity.update(compInfo)) {
                        ...
                        if (!compInfo->getLowCompDensityMode()) {
                            ...
                            // If density of compilations is lower than N every 10 minutes of CPU, we want to enter the
                            // lowCompDensityMode
                            if (numComps * compDensity.getCpuSpanForSlidingWindow()
                                < J9::Options::_lowCompDensityModeEnterThreshold * cpuConsumed) {
                                compInfo->enterLowCompDensityMode();
                                compInfo->setCompileFromLPQRegardlessOfCPU(false);
                                if (TR::Options::isAnyVerboseOptionSet(TR_VerbosePerformance, TR_VerboseJITServer)) {
                                    TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                                        "t=%6u Entering low compilation density mode. Will suppress most remote "
                                        "compilations. numComps=%u cpuConsumed=%llu ms LPQsize=%d",
                                        (uint32_t)crtTime, numComps, cpuConsumed,
                                        compInfo->getLowPriorityCompQueue().getLowPriorityQueueSize());
                                }
                            }
                        } else // Check whether we need to exit the lowCompDensityMode
                        {
                            // Exit lowCompDensityMode if
                            // (1) The density of compilations in the last 10 minutes of CPU window exceeds
                            // `J9::Options::_lowCompDensityModeExitThreshold` or (2) We have accumulated a large
                            // backlog of recompilations in the LPQ
                            if (numComps * compDensity.getCpuSpanForSlidingWindow()
                                    > J9::Options::_lowCompDensityModeExitThreshold * cpuConsumed
                                || compInfo->getLowPriorityCompQueue().getLowPriorityQueueSize()
                                    > J9::Options::_lowCompDensityModeExitLPQSize) {
                                ...
                                if (JITServerHelpers::isServerAvailable()
                                    && compInfo->getLowPriorityCompQueue().getLowPriorityQueueSize() > 0)
                                    compInfo->setCompileFromLPQRegardlessOfCPU(true);
                                if (TR::Options::isAnyVerboseOptionSet(TR_VerbosePerformance, TR_VerboseJITServer)) {
                                    TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                                        "t=%6u Exiting low compilation density mode. Will re-enable remote "
                                        "compilations. numComps=%u cpuConsumed=%llu ms LPQsize=%d",
                                        (uint32_t)crtTime, numComps, cpuConsumed,
                                        compInfo->getLowPriorityCompQueue().getLowPriorityQueueSize());
                                }
                ...
                if ((usedCacheSize > threshold) && !highCodeCacheOccupancyThresholdReached) {
                    ...
                    if (TR::Options::getCmdLineOptions()->isAnyVerboseOptionSet(TR_VerbosePerformance)) {
                        TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                            "t=%6u JIT entered high code cache occupancy state", (uint32_t)crtTime);
                    }
                } else if ((usedCacheSize <= threshold) && highCodeCacheOccupancyThresholdReached) {
                    highCodeCacheOccupancyThresholdReached = false;
                    if (TR::Options::getCmdLineOptions()->isAnyVerboseOptionSet(TR_VerbosePerformance)) {
                        TR_VerboseLog::writeLineLocked(TR_Vlog_PERF, "t=%6u JIT exited high code cache occupancy state",
                            (uint32_t)crtTime);
                    }
                }
            ...
            do {
...
#endif /* J9VM_INTERP_ATOMIC_FREE_JNI */
                {
                    if (currentThread->publicFlags & J9_PUBLIC_FLAGS_VM_ACCESS) {
                        vm->internalVMFunctions->J9SignalAsyncEvent(vm, currentThread,
                            jitConfig->sampleInterruptHandlerKey);
            ...
            persistentInfo->setLoadFactor(
                numActiveThreads * 100 / compInfo->getJvmCpuEntitlement()); // This is used by the IProfiler.
            ...
            // types?
            samplingPeriod = std::max(static_cast<UDATA>(TR::Options::_minSamplingPeriod),
                (samplingFreq == MAX_SAMPLING_FREQUENCY) ? samplingFreq : samplingFreq * cpuLoadFactor);
            ...
            if (TR::Options::getVerboseOption(TR_VerboseSampling)
                ...
                TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                    "t=%" OMR_PRIu64 " loadFactor=%" OMR_PRIu32 " numActiveThreads=%" OMR_PRIu32
                    " entitlement=%.2f%% VmCpuUsage=%d%% idle=%d%% samplingPeriod=%d ms",
                    crtTime, cpuLoadFactor, numActiveThreads, compInfo->getJvmCpuEntitlement(), (int)vmCpuUsage,
                    (int)cpuIdle, (int)samplingPeriod);
            }
        } // while
...
void jitHookJNINativeRegistered(J9HookInterface **hookInterface, UDATA eventNum, void *eventData, void *userData)
{
    ...
    // Notify the assumptions table that the native has been registered
    {
        ...
        TR_RuntimeAssumptionTable *rat = compInfo->getPersistentInfo()->getRuntimeAssumptionTable();
        OMR::RuntimeAssumption **headPtr = rat->getBucketPtr(RuntimeAssumptionOnRegisterNative,
            TR_RuntimeAssumptionTable::hashCode((uintptr_t)method));
...
static UDATA jitReleaseCodeStackWalkFrame(J9VMThread *vmThread, J9StackWalkState *walkState)
{
    ...
    if (metaData) {
        ...
        for (OMR::FaintCacheBlock *cursor = (OMR::FaintCacheBlock *)jitConfig->methodsToDelete; cursor;
             cursor = cursor->_next) {
...
#if JAVA_SPEC_VERSION >= 19
static void jitWalkContinuationStackFrames(J9HookInterface **hookInterface, UDATA eventNum, void *eventData,
    void *userData)
{
    ...
    if (NULL != continuation) {
        J9StackWalkState walkState;
        walkState.flags
            = J9_STACKWALK_ITERATE_HIDDEN_JIT_FRAMES | J9_STACKWALK_ITERATE_FRAMES | J9_STACKWALK_SKIP_INLINES;
        walkState.skipCount = 0;
        walkState.frameWalkFunction = jitReleaseCodeStackWalkFrame;

        j9object_t threadObject
            = VM_ContinuationHelpers::getThreadObjectForContinuation(vmThread, continuation, continuationObj);
        vmFuncs->walkContinuationStackFrames(vmThread, continuation, threadObject, &walkState);
    }
}

static jvmtiIterationControl jitWalkContinuationCallBack(J9VMThread *vmThread, J9MM_IterateObjectDescriptor *object,
    void *userData)
{
    J9InternalVMFunctions *vmFuncs = vmThread->javaVM->internalVMFunctions;
...
static jvmtiIterationControl jitResetContinuationFlag(J9VMThread *vmThread, J9MM_IterateObjectDescriptor *object,
    void *userData)
{
...
static void jitReleaseCodeStackWalk(OMR_VMThread *omrVMThread, condYieldFromGCFunctionPtr condYield = NULL)
{
    ...
    if (isRealTimeGC && !TR::Options::getCmdLineOptions()->getOption(TR_DisableIncrementalCCR)) {
        ...
        do {
            ...
            do {
                if ((thread->dropFlags & 0x1) ? false : true) {
                    J9StackWalkState walkState;
                    walkState.flags = J9_STACKWALK_ITERATE_HIDDEN_JIT_FRAMES | J9_STACKWALK_ITERATE_FRAMES
                ...
                if ((NULL != thread->currentContinuation)
                    && ((thread->currentContinuation->dropFlags & 0x1) ? false : true))
                /* If a continuation is mounted, always walk the continuation as that represent the CarrierThread */
                {
                    ...
                    walkState.skipCount = 0;
                    walkState.frameWalkFunction = jitReleaseCodeStackWalkFrame;
                    vmFuncs->walkContinuationStackFrames(vmThread, thread->currentContinuation,
                        thread->carrierThreadObject, &walkState);
        ...
        do {
            ...
            jvmtiIterationControl rc = vm->memoryManagerFunctions->j9mm_iterate_all_continuation_objects(vmThread,
                PORTLIB, 0, jitWalkContinuationCallBack, (void *)condYield);
    ...
    if (isRealTimeGC && !TR::Options::getCmdLineOptions()->getOption(TR_DisableIncrementalCCR)) { // clear flags
...
#if JAVA_SPEC_VERSION >= 19
        vm->memoryManagerFunctions->j9mm_iterate_all_continuation_objects(vmThread, PORTLIB, 0,
            jitResetContinuationFlag, NULL);
#endif /* JAVA_SPEC_VERSION >= 19 */
    }
}
...
int32_t setUpHooks(J9JavaVM *javaVM, J9JITConfig *jitConfig, TR_FrontEnd *vm)
{
    ...
    if (TR::Options::getCmdLineOptions()->getOption(TR_noJitDuringBootstrap)
        ...
        if ((*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_LOOKUP_JNI_ID, jitHookAboutToRunMain,
    ...
    if (TR::Options::getCmdLineOptions()->getOption(TR_RTGCMapCheck))
        initJitGCMapCheckAsyncHook(javaVM,
            javaVM->internalVMFunctions->J9RegisterAsyncEvent(javaVM, jitGCMapCheck, NULL), jitConfig);
    ...
    if (jitConfig->samplingFrequency && !vmj9->isAOT_DEPRECATED_DO_NOT_USE()
        ...
        if ((jitConfig->sampleInterruptHandlerKey
                = javaVM->internalVMFunctions->J9RegisterAsyncEvent(javaVM, jitMethodSampleInterrupt, NULL))
            < 0) {
        ...
        if (jitConfig->samplerMonitor) {
            ...
            if (javaVM->internalVMFunctions->createThreadWithCategory(&jitConfig->samplerThread, defaultOSStackSize,
                    priority, 0, &samplerThreadProc, jitConfig, J9THREAD_CATEGORY_SYSTEM_JIT_THREAD)) {
                ...
                if (compInfo->getSamplingThreadLifetimeState()
                    == TR::CompilationInfo::SAMPLE_THR_FAILED_TO_ATTACH) // no monitor needed for this access
                {
                    j9thread_monitor_destroy(jitConfig->samplerMonitor);
                    jitConfig->samplerMonitor = 0;
    ...
    if (jitConfig->runtimeFlags & J9JIT_TOSS_CODE) {
...
#endif
        {
            if ((*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_INITIALIZE_SEND_TARGET,
                    jitHookInitializeSendTarget, OMR_GET_CALLSITE(), NULL)) {
        ...
        if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableInterpreterProfiling) && iProfiler
            ...
            {
                if ((*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_PROFILING_BYTECODE_BUFFER_FULL,
                        jitHookBytecodeProfiling, OMR_GET_CALLSITE(), NULL)) {
                    j9tty_printf(PORTLIB,
                        "Error: Unable to install J9HOOK_VM_PROFILING_BYTECODE_BUFFER_FULL listener\n");
                    return -1;
                }
                interpreterProfilingState = IPROFILING_STATE_ON;
            }
        ...
        if (TR::Options::getCmdLineOptions()->getOption(TR_EnableJProfiling)
            && !TR::Options::getCmdLineOptions()->getOption(TR_DisableJProfilerThread)) {
            TR_JProfilerThread *jProfiler = ((TR_JitPrivateConfig *)(jitConfig->privateConfig))->jProfiler;
    ...
    if ((*gcOmrHooks)
            ->J9HookRegisterWithCallSite(gcOmrHooks, J9HOOK_MM_OMR_LOCAL_GC_START, jitHookLocalGCStart,
                OMR_GET_CALLSITE(), NULL)
        || (*gcOmrHooks)
               ->J9HookRegisterWithCallSite(gcOmrHooks, J9HOOK_MM_OMR_LOCAL_GC_END, jitHookLocalGCEnd,
                   OMR_GET_CALLSITE(), NULL)
        || (*gcOmrHooks)
               ->J9HookRegisterWithCallSite(gcOmrHooks, J9HOOK_MM_OMR_GLOBAL_GC_START, jitHookGlobalGCStart,
                   OMR_GET_CALLSITE(), NULL)
        || (*gcOmrHooks)
               ->J9HookRegisterWithCallSite(gcOmrHooks, J9HOOK_MM_OMR_GLOBAL_GC_END, jitHookGlobalGCEnd,
                   OMR_GET_CALLSITE(), NULL)) {
    ...
    if (!vmj9->isAOT_DEPRECATED_DO_NOT_USE()) {
        ...
        if (!TR::Options::getCmdLineOptions()->realTimeGC()
            || !TR::Options::getCmdLineOptions()->getOption(TR_NoClassGC)) {
            unableToRegisterHooks = (*gcOmrHooks)
                                        ->J9HookRegisterWithCallSite(gcOmrHooks, J9HOOK_MM_OMR_GC_CYCLE_END,
                                            jitHookReleaseCodeGCCycleEnd, OMR_GET_CALLSITE(), NULL);
        }
    ...
    if ((*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_INTERNAL_CLASS_LOAD, jitHookClassLoad,
            OMR_GET_CALLSITE(), NULL)
        || (*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_CLASS_PREINITIALIZE, jitHookClassPreinitialize,
            OMR_GET_CALLSITE(), NULL)
        || (*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_CLASS_INITIALIZE, jitHookClassInitialize,
            OMR_GET_CALLSITE(), NULL)) {
...
#endif
        if ((*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_CLASS_UNLOAD, jitHookClassUnload,
                OMR_GET_CALLSITE(), NULL)
            || (*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_CLASSES_UNLOAD, jitHookClassesUnload,
                OMR_GET_CALLSITE(), NULL)
            || (*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_CLASS_LOADER_UNLOAD, jitHookClassLoaderUnload,
                OMR_GET_CALLSITE(), NULL)
            || (*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_ANON_CLASSES_UNLOAD, jitHookAnonClassesUnload,
                OMR_GET_CALLSITE(), NULL)
            ||
#if defined(J9VM_JIT_DYNAMIC_LOOP_TRANSFER)
            (*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_CLASS_LOADERS_UNLOAD, jitHookClassLoadersUnload,
                OMR_GET_CALLSITE(), NULL)
            ||
#endif
            (*gcHooks)->J9HookRegisterWithCallSite(gcHooks, J9HOOK_MM_INTERRUPT_COMPILATION,
                jitHookInterruptCompilation, OMR_GET_CALLSITE(), NULL)
            ||
#if JAVA_SPEC_VERSION >= 19
            (*gcHooks)->J9HookRegisterWithCallSite(gcHooks, J9HOOK_MM_WALKCONTINUATION, jitWalkContinuationStackFrames,
                OMR_GET_CALLSITE(), NULL)
            ||
#endif /* JAVA_SPEC_VERSION >= 19 */
            (*gcHooks)->J9HookRegisterWithCallSite(gcHooks, J9HOOK_MM_CLASS_UNLOADING_END, jitHookClassesUnloadEnd,
                OMR_GET_CALLSITE(), NULL)) {
            j9tty_printf(PORTLIB, "Error: Unable to register class event hook\n");
            return -1;
        }
    ...
    j9thread_monitor_enter(javaVM->vmThreadListMutex);
    if ((*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_THREAD_CREATED, jitHookThreadCreate,
            OMR_GET_CALLSITE(), NULL)
        || (*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_THREAD_END, jitHookThreadEnd, OMR_GET_CALLSITE(),
            NULL)
        || (*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_THREAD_CRASH, jitHookThreadCrash,
            OMR_GET_CALLSITE(), NULL)
        || (*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_THREAD_DESTROY, jitHookThreadDestroy,
            OMR_GET_CALLSITE(), NULL)
        || (*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_THREAD_STARTED, jitHookThreadStart,
            OMR_GET_CALLSITE(), NULL)) {
...
#if defined(J9VM_OPT_CRIU_SUPPORT)
    if ((*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_PREPARING_FOR_CHECKPOINT, jitHookPrepareCheckpoint,
            OMR_GET_CALLSITE(), NULL)
        || (*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_PREPARING_FOR_RESTORE, jitHookPrepareRestore,
            OMR_GET_CALLSITE(), NULL)) {
    ...
    if (!vmj9->isAOT_DEPRECATED_DO_NOT_USE()
        ...
        if ((*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_JNI_NATIVE_REGISTERED, jitHookJNINativeRegistered,
                OMR_GET_CALLSITE(), NULL)) {
...
int32_t startJITServer(J9JITConfig *jitConfig)
{
    ...
    TR_ASSERT(compInfo->getPersistentInfo()->getRemoteCompilationMode() == JITServer::SERVER,
        "startJITServer cannot be called in non-server mode\n");
    ...
    if (TR::Options::getVerboseOption(TR_VerboseJITServer))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer, "Started JITServer listener thread: %p ",
            listener->getListenerThread());
    ...
    if (jitConfig->samplingFrequency != 0) {
        JITServerStatisticsThread *statsThreadObj
            = ((TR_JitPrivateConfig *)(jitConfig->privateConfig))->statisticsThreadObject;
        // statsThreadObj is guaranteed to be non-null because JITServer will not start if statisticsThreadObject cannot
        // be created
...
int32_t waitJITServerTermination(J9JITConfig *jitConfig)
{
    ...
    TR_ASSERT(compInfo->getPersistentInfo()->getRemoteCompilationMode() == JITServer::SERVER,
        "waitJITServerTermination cannot be called in non-server mode\n");
...
void turnOffInterpreterProfiling(J9JITConfig *jitConfig)
{
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableInterpreterProfiling)) {
        if (interpreterProfilingState != IPROFILING_STATE_OFF) {
            ...
            if (TR::Options::getCmdLineOptions()->getOption(TR_VerboseInterpreterProfiling)) {
                TR::CompilationInfo *compInfo = TR::CompilationInfo::get(jitConfig);
                TR_VerboseLog::writeLineLocked(TR_Vlog_IPROFILER, "t=%6u IProfiler stopped",
                    (uint32_t)compInfo->getPersistentInfo()->getElapsedTime());
            }
        }
    }
