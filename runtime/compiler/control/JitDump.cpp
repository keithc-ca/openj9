...
static uintptr_t traceILOfCrashedCompilationThreadProtected(struct J9PortLibrary *portLib, void *handler_arg)
{
...
static void traceILOfCrashedCompilationThread(J9VMThread *vmThread, TR::Compilation *comp, OMR::Logger *jitdumpLogger)
{
    ...
    if (!alreadyHaveVMAccess) {
        if (0
            == vmThread->javaVM->internalVMFunctions->internalTryAcquireVMAccessWithMask(vmThread,
    ...
    U_32 flags = J9PORT_SIG_FLAG_MAY_RETURN | J9PORT_SIG_FLAG_SIGSEGV | J9PORT_SIG_FLAG_SIGFPE | J9PORT_SIG_FLAG_SIGILL
        | J9PORT_SIG_FLAG_SIGBUS | J9PORT_SIG_FLAG_SIGTRAP;

    uintptr_t result = 0;
    j9sig_protect(traceILOfCrashedCompilationThreadProtected, static_cast<void *>(&p), jitDumpSignalHandler, vmThread,
        flags, &result);
...
static void jitDumpRecompileWithTracing(J9VMThread *vmThread, J9Method *ramMethod, TR::CompilationInfo *compInfo,
    TR_Hotness optLevel, bool isProfilingCompile, TR::Options *optionsFromOriginalCompile, bool isAOTBody,
    void *oldStartPC, OMR::Logger *jitdumpLogger, bool crashWasDueToOrphanedConstRefs = false)
{
    PORT_ACCESS_FROM_VMC(vmThread);
    J9Class *clazz;
    ...
    if (compInfo->getPersistentInfo()->getRemoteCompilationMode() == JITServer::SERVER) {
        TR_J9VMBase *fej9 = reinterpret_cast<TR_J9VMBase *>(
            TR_J9VMBase::get(compInfo->getJITConfig(), vmThread, TR_J9VMBase::J9_SERVER_VM));
        ...
        clazz
            = reinterpret_cast<J9Class *>(fej9->getClassOfMethod(reinterpret_cast<TR_OpaqueMethodBlock *>(ramMethod)));
    ...
    j9nls_printf(PORTLIB, J9NLS_INFO | J9NLS_STDERR, J9NLS_DMP_JIT_RECOMPILING_METHOD, (UDATA)J9UTF8_LENGTH(className),
        J9UTF8_DATA(className), (UDATA)J9UTF8_LENGTH(methName), J9UTF8_DATA(methName), (UDATA)J9UTF8_LENGTH(methSig),
        J9UTF8_DATA(methSig));
...
uintptr_t jitDumpSignalHandler(struct J9PortLibrary *portLibrary, uint32_t gpType, void *gpInfo, void *handler_arg)
{
...
static UDATA jitDumpStackFrameIterator(J9VMThread *currentThread, J9StackWalkState *walkState)
{
    ...
    if (NULL != walkState->jitInfo) {
        ...
        if (NULL != bodyInfo) {
            jitDumpRecompileWithTracing(currentThread, walkState->method,
                reinterpret_cast<TR::CompilationInfo *>(walkState->userData1), bodyInfo->getHotness(),
                bodyInfo->getIsProfilingBody(), NULL, bodyInfo->getIsAotedBody(),
                bodyInfo->getStartPCAfterPreviousCompile(), reinterpret_cast<OMR::Logger *>(walkState->userData2));
        }
    }
...
omr_error_t runJitdump(char *label, J9RASdumpContext *context, J9RASdumpAgent *agent)
{
    ...
    j9nls_printf(PORTLIB, J9NLS_INFO | J9NLS_STDERR, J9NLS_DMP_OCCURRED_THREAD_NAME_ID, "JIT", crashedThreadName,
        crashedThread);
    releaseOMRVMThreadName(crashedThread->omrVMThread);

    J9JITConfig *jitConfig = crashedThread->javaVM->jitConfig;
    if (NULL == jitConfig) {
        j9nls_printf(PORTLIB, J9NLS_ERROR | J9NLS_STDERR, J9NLS_DMP_ERROR_IN_DUMP_STR, "JIT",
            "Could not locate J9JITConfig");
    ...
    if (NULL == compInfo) {
        j9nls_printf(PORTLIB, J9NLS_ERROR | J9NLS_STDERR, J9NLS_DMP_ERROR_IN_DUMP_STR, "JIT",
            "Could not locate TR::CompilationInfo");
        return OMR_ERROR_INTERNAL;
    }
    ...
    if (!diagnosticCompilationThreadInfo) {
        j9nls_printf(PORTLIB, J9NLS_ERROR | J9NLS_STDERR, J9NLS_DMP_ERROR_IN_DUMP_STR, "JIT",
            "Could not locate the diagnostic thread info");
        return OMR_ERROR_INTERNAL;
    }
    ...
    if (!diagnosticCompilationThread) {
        j9nls_printf(PORTLIB, J9NLS_ERROR | J9NLS_STDERR, J9NLS_DMP_ERROR_IN_DUMP_STR, "JIT",
            "Could not locate the diagnostic thread");
        return OMR_ERROR_INTERNAL;
    }
    ...
    if (!jitdumpLogger) {
        j9nls_printf(PORTLIB, J9NLS_ERROR | J9NLS_STDERR, J9NLS_DMP_ERROR_IN_DUMP_STR, "JIT",
            "Could not create a Logger object");
        return OMR_ERROR_INTERNAL;
    }
    jitdumpLogger->setEnabled_DEPRECATED(true);

    jitdumpLogger->prints("<?xml version=\"1.0\" standalone=\"no\"?>\n"
                          "<jitDump>\n");
    ...
    try {
        ...
        if (agent->dumpOptions) {
            ...
            if (indexOfDot && indexOfParen) {
                ...
                J9Method *userMethod = allMethodsFromSignatureStartDo(&state, context->javaVM, 0, className,
                    classNameLength, methodName, methodNameLength, methodSig, methodSigLength);
                while (userMethod) {
                    J9JITExceptionTable *metadata = jitConfig->jitGetExceptionTableFromPC(crashedThread,
                        reinterpret_cast<UDATA>(userMethod->extra));
                    if (metadata) {
                        ...
                        if (bodyInfo) {
                            if (!crashedThreadCompInfo) {
                                ...
                                jitDumpRecompileWithTracing(crashedThread, userMethod, compInfo, bodyInfo->getHotness(),
                                    bodyInfo->getIsProfilingBody(), NULL, bodyInfo->getIsAotedBody(),
                                    bodyInfo->getStartPCAfterPreviousCompile(), jitdumpLogger);
                            } else {
                                // --------------------------------------
                                ...
                                TR::CompilationInfoPerThreadBase::UninterruptibleOperation
                                    jitDumpForCrashedCompilationThread(*crashedThreadCompInfo);

                                // See comment below for the same call on why this is needed
                                TR::VMAccessCriticalSection requestSynchronousCompilation(
                                    TR_J9VMBase::get(jitConfig, crashedThread));
        ...
        if (!crashedThreadCompInfo) {
            ...
            if (crashedThread->gpInfo) {
                ...
                //
                // The stack walker may not be able to walk the stack if the crash did not happen on a transition frame.
                // In such cases the stack walker will resume walking the stack on the last known valid point, which
                // will be a transition frame further up the stack (ex. INT -> JIT transition frame). This will result
                // in the stack walker potentially skipping some JIT methods on the backtrace. This is not desirable.
                // Chances are high that the crash happened because of a miscompilation in the first JIT compiled method
                // on the stack, so it is imperative that we recompile the method we actually crashed in.
                //
                const char *name;
                void *value;
                U_32 infoType
                    = j9sig_info(crashedThread->gpInfo, J9PORT_SIG_CONTROL, J9PORT_SIG_CONTROL_PC, &name, &value);

                if (infoType == J9PORT_SIG_VALUE_ADDRESS) {
                    J9JITExceptionTable *metadata
                        = jitConfig->jitGetExceptionTableFromPC(crashedThread, *reinterpret_cast<UDATA *>(value));
                    if (metadata) {
                        auto *bodyInfo = reinterpret_cast<TR_PersistentJittedBodyInfo *>(metadata->bodyInfo);
                        if (bodyInfo) {
                            jitDumpRecompileWithTracing(crashedThread, metadata->ramMethod, compInfo,
                                bodyInfo->getHotness(), bodyInfo->getIsProfilingBody(), NULL,
                                bodyInfo->getIsAotedBody(), bodyInfo->getStartPCAfterPreviousCompile(), jitdumpLogger);
                        }
                    }
                }
            }
            ...
            walkState.flags = (J9_STACKWALK_VISIBLE_ONLY | J9_STACKWALK_SKIP_INLINES | J9_STACKWALK_COUNT_SPECIFIED
            ...
            // acquire VM access (e.g., to get a class signature). Other VM events, such as VM shutdown or the GC
            // unloading classes may cause compilations to be interrupted. Because the crashed thread is not a
            // diagnostic thread, the call to print the crashed thread IL may get interrupted and the jitdump will be
            // incomplete. We prevent this from occuring by disallowing interruptions until we are done generating the
            // jitdump.
            //
            TR::CompilationInfoPerThreadBase::UninterruptibleOperation jitDumpForCrashedCompilationThread(
                *crashedThreadCompInfo);

            TR::Compilation *comp = crashedThreadCompInfo->getCompilation();
            if (!comp) {
                j9nls_printf(PORTLIB, J9NLS_ERROR | J9NLS_STDERR, J9NLS_DMP_ERROR_IN_DUMP_STR, "JIT",
                    "Could not locate the compilation object");
                jitdumpLogger->close();
                return OMR_ERROR_INTERNAL;
            }
            ...
            if (methodBeingCompiled && methodBeingCompiled->getMethodDetails().isOrdinaryMethod()) {
                ...
                {
                    // The current thread is a compilation thread which may or may not currently hold VM access. The
                    // request for recompilation to generate the jitdump will be performed synchronously for which the
                    // code path we will take will be the same as if we were an application thread. We will take the
                    // synchronous request path in `compileOnSeparateThread` which ends up releasing VM access prior to
                    // waiting on the diagnostic thread to finish the queued compile. To avoid deadlocks we must acquire
                    // VM access here.
                    //
                    TR::VMAccessCriticalSection requestSynchronousCompilation(
                        TR_J9VMBase::get(jitConfig, crashedThread));
                    ...
                    jitDumpRecompileWithTracing(crashedThread,
                        (J9Method *)(comp->getMethodBeingCompiled()->getPersistentIdentifier()), compInfo,
                        (TR_Hotness)comp->getOptLevel(), comp->isProfilingCompilation(), comp->getOptions(),
                        comp->compileRelocatableCode(), methodBeingCompiled->_oldStartPC, jitdumpLogger,
