...
const char *TR_VectorAPIExpansion::optDetailString() const throw() { return "O^O VECTOR API EXPANSION: "; }

#define OPT_DETAILS_VECTOR "O^O VECTOR API: "

int32_t TR_VectorAPIExpansion::perform()
{
...
bool TR_VectorAPIExpansion::isVectorAPIMethod(TR::MethodSymbol *methodSymbol, bool reportAllMethods)
{
    TR::RecognizedMethod index = methodSymbol->getRecognizedMethod();

    if (index >= _firstMethod && index <= _lastMethod) {
        return (reportAllMethods || (methodTable[index - _firstMethod]._methodHandler != unsupportedHandler));
    } else {
        return false;
    }
}

TR_VectorAPIExpansion::vapiObjType TR_VectorAPIExpansion::getReturnType(TR::MethodSymbol *methodSymbol)
{
...
TR_VectorAPIExpansion::vapiObjType TR_VectorAPIExpansion::getArgumentType(TR::MethodSymbol *methodSymbol, int32_t i)
{
...
int32_t TR_VectorAPIExpansion::getElementTypeIndex(TR::MethodSymbol *methodSymbol)
{
...
int32_t TR_VectorAPIExpansion::getFirstClassIndex(TR::MethodSymbol *methodSymbol)
{
    TR_ASSERT_FATAL(isVectorAPIMethod(methodSymbol), "getElementTypeIndex should be called on VectorAPI method");
...
int32_t TR_VectorAPIExpansion::getSecondClassIndex(TR::MethodSymbol *methodSymbol)
{
    TR_ASSERT_FATAL(isVectorAPIMethod(methodSymbol), "getElementTypeIndex should be called on VectorAPI method");
...
int32_t TR_VectorAPIExpansion::getNumLanesIndex(TR::MethodSymbol *methodSymbol)
{
...
int32_t TR_VectorAPIExpansion::getFirstOperandIndex(TR::MethodSymbol *methodSymbol)
{
...
int32_t TR_VectorAPIExpansion::getNumOperands(TR::MethodSymbol *methodSymbol)
{
...
int32_t TR_VectorAPIExpansion::getMaskIndex(TR::MethodSymbol *methodSymbol)
{
...
void TR_VectorAPIExpansion::getElementTypeAndNumLanes(TR::Node *node, TR::DataType &elementType, int32_t &numLanes)
{
    TR_ASSERT_FATAL(node->getOpCode().isFunctionCall(),
        "getElementTypeAndVectorLength can only be called on a call node");
...
bool TR_VectorAPIExpansion::treeTopAllowedWithBoxing(TR::ILOpCodes opCodeValue)
{
    return (opCodeValue == TR::ResolveCHK || opCodeValue == TR::ResolveAndNULLCHK);
}

void TR_VectorAPIExpansion::buildVectorAliases(bool verifyMode)
{
...
void TR_VectorAPIExpansion::visitNodeToBuildVectorAliases(TR::Node *node, bool verifyMode)
{
    ...
    if (opCodeValue == TR::astore && !node->chkStoredValueIsIrrelevant()
        ...
        if (rhs->getOpCode().hasSymbolReference()) {
            ...
            if (rhs->getOpCode().isFunctionCall()
                ...
                vectorInfo &rhsVectorInfo = _nodeTable[rhs->getGlobalIndex()]._vinfo;

                TR_ASSERT_FATAL(rhsVectorInfo.isSet(), "RHS (%p) of node %p should've been parsed since it's a call",
                    rhs, node);

                if (_aliasTable[id1]._vinfo.isSet() && _aliasTable[id1]._vinfo != rhsVectorInfo) {
        ...
        for (int32_t i = 0; i < numChildren; i++) {
            ...
            if (!isVectorAPICall
                || (i >= getFirstOperandIndex(methodSymbol)
                ...
                if (hasSymbolReference && child->getDataType() == TR::Address
                    && (!boxingAllowed() || isVectorAPICall)) {
                    alias(node, child);
                }

                if (!hasSymbolReference && !isNullMask && !nullVectorInMaskCompress && !constOperandOfBroadcastInt) {
            ...
            if (i == getElementTypeIndex(methodSymbol)) {
                ...
                _aliasTable[methodRefNum]._vinfo._vecLen = vec_len_unknown;
                _nodeTable[nodeIndex]._vinfo._vecLen = vec_len_unknown;
                ...
                if (numLanesNode->getOpCode().isLoadConst()) {
                    ...
                    if (methodElementType != TR::NoType) // type was seen but could've been non-const
                    {
                        ...
                        _aliasTable[methodRefNum]._vinfo._vecLen = methodNumLanes * 8 * elementSize;
                        _nodeTable[nodeIndex]._vinfo._vecLen = methodNumLanes * 8 * elementSize;
                    }
                }
            }
        }
        ...
        if (methodElementType == TR::NoType || methodNumLanes == 0) {
            logprintf(_trace, log,
                "Invalidating/Boxing5 #%d (isVectorAPICall=%d) due to unknown elementType=%d or numLanes=%d in node "
                "%p\n",
                node->getSymbolReference()->getReferenceNumber(), isVectorAPICall, (int)methodElementType,
                methodNumLanes, node);
            ...
            if (supportedOnPlatform(comp(), bitsLength) != TR::NoVectorLength) {
                canVectorize = methodTable[handlerIndex]._methodHandler(this, NULL, node, methodElementType,
                    vectorLength, _nodeTable[nodeIndex]._vinfo._objectType, methodNumLanes, checkVectorization);
            }

            bool canScalarize = methodTable[handlerIndex]._methodHandler(this, NULL, node, methodElementType,
                vectorLength, _nodeTable[nodeIndex]._vinfo._objectType, methodNumLanes, checkScalarization);
        ...
        if (isVectorAPICall) {
            logprintf(_trace, log, "Parsed intrinsic call node %p: ", node);
            _nodeTable[nodeIndex]._vinfo.print(_trace, log);
            logprintf(_trace, log, "\n");
        }
        ...
        for (int32_t i = 0; i < node->getNumChildren(); i++) {
            TR::Node *child = node->getChild(i);
            if (child->getOpCode().hasSymbolReference() && child->getDataType() == TR::Address) {
                ...
                if (child->getOpCode().isFunctionCall()) {
                    ...
                    if (getReturnType(methodSymbol) == Scalar)
                        continue; // OK to use by any other parent node
                }
    ...
    if ((node->getOpCodeValue() == TR::checkcast || node->getOpCodeValue() == TR::NULLCHK)
...
void TR_VectorAPIExpansion::findAllAliases(int32_t classId, int32_t id,
    TR_BitVector *vectorAliasTableElement::*aliasesField, int32_t vectorAliasTableElement::*classField)
{
    OMR::Logger *log = comp()->log();
    ...
    TR_BitVector *aliasesToIterate = (classId == id) ? new (comp()->trStackMemory())
                                                           TR_BitVector(*(_aliasTable[id].*aliasesField))
    ...
    while (bvi.hasMoreElements()) {
        ...
        if (_aliasTable[i].*classField > 0) {
            TR_ASSERT_FATAL(_aliasTable[i].*classField == classId,
                "#%d should belong to class %d but it belongs to class %d\n", i, classId, _aliasTable[i].*classField);
            continue;
        }
        ...
        if (_aliasTable[i].*classField == -1) {
            if (boxingAllowed()) {
                ...
                _aliasTable[classId]._vinfo.setIsUnknown();
            } else {
...
void TR_VectorAPIExpansion::buildAliasClasses()
{
...
TR_VectorAPIExpansion::vec_sz_t TR_VectorAPIExpansion::getVectorSizeFromVectorSpecies(TR::Node *vectorSpeciesNode)
{
    ...
    if (vSpeciesSymRef) {
        if (vSpeciesSymRef->hasKnownObjectIndex()) {
...
#endif /* defined(J9VM_OPT_JITSERVER) */
            {
                ...
                uintptr_t vectorShapeLocation = fej9->getReferenceField(vectorSpeciesLocation, "vectorShape",
...
TR_OpaqueClassBlock *TR_VectorAPIExpansion::getOpaqueClassBlockFromClassNode(TR::Compilation *comp, TR::Node *classNode)
{
    ...
    if (knownObjectIndex != TR::KnownObjectTable::UNKNOWN) {
        ...
        {
            auto stream = comp->getStream();
            stream->write(JITServer::MessageType::KnownObjectTable_getOpaqueClass, knownObjectIndex);
    ...
    } else if (classNode->getOpCodeValue() == TR::aloadi && classNode->getFirstChild()->getOpCodeValue() == TR::loadaddr
        ...
        clazz = (TR_OpaqueClassBlock *)(classNode->getFirstChild()
                                            ->getSymbolReference()
                                            ->getSymbol()
                                            ->castToStaticSymbol()
                                            ->getStaticAddress());
...
bool TR_VectorAPIExpansion::findVectorMethods(TR::Compilation *comp, bool reportFoundMethods)
{
    ...
    for (TR::TreeTop *tt = comp->getMethodSymbol()->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
        ...
        if (opCodeValue == TR::treetop || opCodeValue == TR::NULLCHK || treeTopAllowedWithBoxing(opCodeValue)) {
            node = node->getFirstChild();
        }
        ...
        if (opCode.isFunctionCall()) {
            ...
            if (isVectorAPIMethod(methodSymbol, reportFoundMethods)) {
                if (reportFoundMethods && TR::Options::getVerboseOption(TR_VerboseVectorAPI)) {
                    TR::ResolvedMethodSymbol *resolvedMethodSymbol = (TR::ResolvedMethodSymbol *)methodSymbol;

                    const char *name = resolvedMethodSymbol->getResolvedMethod()
                        ? resolvedMethodSymbol->getResolvedMethod()->nameChars()
                        : "unresolved";

                    uint16_t nameLen = resolvedMethodSymbol->getResolvedMethod()
                        ? resolvedMethodSymbol->getResolvedMethod()->nameLength()
                        : 10; // length of "unresolved"

                    TR_VerboseLog::writeLine(TR_Vlog_VECTOR_API, "Did not vectorize %.*s() intrinsic in %s at %s %s",
                        name ? nameLen : 0, name ? name : "", comp->signature(),
...
bool TR_VectorAPIExpansion::validateSymRef(int32_t id, int32_t i, vectorInfo &classVectorInfo,
    int32_t vectorAliasTableElement::*classField)
{
    ...
    if (_aliasTable[i].*classField == -1) {
        ...
        vectorInfo tempVectorInfo = _aliasTable[i]._vinfo;

        if (tempVectorInfo.isUnknown()) {
            logprintf(_trace, log, "%s making temp class #%d boxed due to symref #%d\n", OPT_DETAILS_VECTOR, id, i);
            classVectorInfo.setIsUnknown();
        } else if (tempVectorInfo.isSet()) {
            if (!classVectorInfo.isSet()) {
                logprintf(_trace, log, "%s assigning to temp class #%d from symref #%d: ", OPT_DETAILS_VECTOR, id, i);
                tempVectorInfo.print(_trace, log);
                logprintf(_trace, log, "\n");

                classVectorInfo = tempVectorInfo;
            } else if (tempVectorInfo != classVectorInfo) {
                logprintf(_trace, log,
                    "%s invalidating15 class #%d due to symref #%d temp with different vector info\n",
                    OPT_DETAILS_VECTOR, id, i);
                return false;
            }
        }
...
void TR_VectorAPIExpansion::validateVectorAliasClasses(TR_BitVector *vectorAliasTableElement::*aliasesField,
    int32_t vectorAliasTableElement::*classField)
{
    OMR::Logger *log = comp()->log();
    ...
    for (int32_t id = 1; id < _aliasTableSize; id++) {
        ...
        vectorInfo classVectorInfo; // default values

        if (!(_aliasTable[id].*aliasesField)) {
            // class might consist of just the symref itself
            vectorClass = validateSymRef(id, id, classVectorInfo, classField);
            ...
            while (bvi.hasMoreElements()) {
                ...
                vectorClass = validateSymRef(id, i, classVectorInfo, classField);

                logprintf(_trace, log, "   Validated #%d: %s\n", i, vectorClass ? "OK" : "X");
        ...
        logprintf(_trace, log, "Setting length and type for %s class #%d to ", tempClasses ? "temp" : "whole", id);
        classVectorInfo.print(_trace, log);
        logprintf(_trace, log, "\n");

        _aliasTable[id]._vinfo = classVectorInfo;

        if (vectorClass && classVectorInfo.isSet() && (!classVectorInfo.isUnknown() || boxingAllowed()))
            continue;
...
TR::SymbolReference *TR_VectorAPIExpansion::createPayloadSymbolReference(TR::Compilation *comp,
    TR_OpaqueClassBlock *vecClass)
{
    ...
    return comp->getSymRefTab()->findOrFabricateShadowSymbol(vecClass, field->_datatype, field->_offset,
        field->_isVolatile, field->_isPrivate, field->_isFinal, field->_fieldname, field->_typeSignature);
}

void TR_VectorAPIExpansion::dontVectorizeNode(TR::Node *node)
{
    if (!node->getOpCode().isLoadAddr() && !node->getOpCode().isLoadDirect() && !node->getOpCode().isStoreDirect()
...
bool TR_VectorAPIExpansion::isVectorizedOrScalarizedNode(TR::Node *node, vectorInfo &vInfo, bool &scalarized,
    bool sourceType)
{
    scalarized = false;
    int32_t refId = -1;
    ...
    vInfo = _aliasTable[refId]._vinfo;

    logprintf(_trace, comp()->log(), "#%d bitsLength=%d\n", refId, vInfo._vecLen);

    if (vInfo.isSet() && !vInfo.isUnknown()) {
        return true;
    }
...
bool TR_VectorAPIExpansion::boxChild(TR::TreeTop *treeTop, TR::Node *node, uint32_t i, bool checkBoxing)
{
    TR::Node *child = node->getChild(i);
    ...
    if (!isVectorizedOrScalarizedNode(child, vInfo, scalarized))
        return true;
    ...
    if ((objectType != Vector && objectType != Mask) || // TODO: support Shuffle
        ...
        maskConv = getMaskToStoreConversion(comp(), numLanes, TR::DataType::createMaskType(elementType, vectorLength),
            maskStoreOpCode);
    ...
    TR_OpaqueClassBlock *j9arrayClass = comp()->fej9()->getArrayClassFromDataType(elementType, objectType == Mask);
    ...
    TR::DataType opCodeType = (objectType == Vector) ? TR::DataType::createVectorType(elementType, vectorLength)
    ...
    TR::ILOpCodes storeOpcode
        = (objectType == Vector) ? TR::ILOpCode::createVectorOpCode(TR::vstorei, opCodeType) : maskStoreOpCode;
    ...
    TR_J9VMBase *fej9 = comp()->fej9();
    TR::SymbolReference *j9class
        = comp()->getSymRefTab()->findOrCreateClassSymbol(comp()->getMethodSymbol(), -1, vecClass);
    ...
    treeTop->insertBefore(
        TR::TreeTop::create(comp(), TR::Node::storeToAddressField(comp(), newObject, payloadSymRef, newArray)));
    ...
    logprintf(_trace, comp()->log(), "Boxed %s%d%s child %d of node %p into %p\n",
        objectType == Vector ? "Vector" : "Mask", bitsLength, TR::DataType::getName(elementType), i, node, newObject);
...
TR::Node *TR_VectorAPIExpansion::unboxNode(TR::Node *parentNode, TR::Node *operand, vapiObjType operandObjectType,
    bool checkBoxing)
{
    vectorInfo vInfo;
        ...
        || parentScalarized) // TODO: support unboxing into scalars
    {
        ...
        maskConv = getLoadToMaskConversion(comp(), numLanes, TR::DataType::createMaskType(elementType, vectorLength),
            maskLoadOpCode);
    ...
    TR::ILOpCodes opcode
        = operandObjectType == Vector ? TR::ILOpCode::createVectorOpCode(TR::vloadi, opCodeType) : maskLoadOpCode;

    TR::SymbolReference *vectorShadow = comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(opCodeType, NULL);
    TR::Node *newOperand = TR::Node::createWithSymRef(operand, opcode, 1, vectorShadow);
    TR::Node *aladdNode
        = generateArrayElementAddressNode(comp(), payloadLoad, TR::Node::lconst(operand, 0), elementSize);
    ...
    logprintf(_trace, comp()->log(), "Unboxed %s%d%s node %p into new node %p for parent %p\n",
        operandObjectType == Vector ? "Vector" : "Mask", bitsLength, TR::DataType::getName(elementType), operand,
        newOperand, parentNode);
...
int32_t TR_VectorAPIExpansion::expandVectorAPI()
{
...
bool TR_VectorAPIExpansion::transformIL(bool checkBoxing)
{
    ...
    for (TR::TreeTop *treeTop = comp()->getMethodSymbol()->getFirstTreeTop(); treeTop;
    ...
    bool checkBoxing)
{
    ...
    bool scalarized;
    vectorInfo vInfo;
    bool vectorizedOrScalarizedNode = isVectorizedOrScalarizedNode(node, vInfo, scalarized);
    ...
    logprintf(_trace, comp()->log(),
        "Node %p (%s) vectorizedOrScalarized=%d elementType=%d bitsLength=%d objectType=%d scalarized=%d\n", node,
        opCode.getName(), vectorizedOrScalarizedNode, elementType.getDataType(), bitsLength, objectType, scalarized);
    ...
    if (boxingAllowed() && !vectorizedOrScalarizedNode && opCode.isFunctionCall()) {
        ...
        if (isVectorAPIMethod(methodSymbol)) {
            ...
            // TODO: make the check for boxing node's children more general
            if (false && methodSymbol->getRecognizedMethod() == TR::jdk_internal_vm_vector_VectorSupport_compare) {
    ...
    {
        TR_ASSERT_FATAL(!_aliasTable[classId]._cantScalarize || scalarized,
            "Class #%d should be either vectorizable or scalarizable", classId);
    ...
    if (!boxingAllowed() && !_seenClasses.isSet(classId)) {
    ...
    logprintf(_trace, log, "%s node %p of class #%d\n", checkBoxing ? "Checking for boxing" : "Transforming", node,
        classId);
    ...
    if (opCodeValue == TR::astore) {
        logprintf(_trace, log, "%s astore %p (temp class #%d) elementType=%d vectorLength=%d objectType=%s\n",
            checkBoxing ? "Checking for boxing" : "Transforming", node, tempClassId, elementType, vectorLength,
            vapiObjTypeNames[objectType]);

        if (boxingAllowed()) {
            vectorInfo rhsVectorInfo;
            bool rhsScalarized;
            bool rhsVectorizedOrScalarized
                = isVectorizedOrScalarizedNode(node->getFirstChild(), rhsVectorInfo, rhsScalarized);
        ...
        TR_ASSERT_FATAL(_nodeTable[node->getGlobalIndex()]._canVectorize
                || _nodeTable[node->getGlobalIndex()]._canScalarize,
            "call in node %p should be vectorizable or scalarizable", node);

        TR::RecognizedMethod index = methodSymbol->getRecognizedMethod();
        int32_t handlerIndex = index - _firstMethod;

        TR_ASSERT_FATAL(methodTable[handlerIndex]._methodHandler(this, treeTop, node, elementType, vectorLength,
                            objectType, numLanes, checkMode),
                ...
                if ((i >= getFirstOperandIndex(methodSymbol)
                    ...
                    if (!vectorizedOrScalarized) {
                        ...
                        if (getArgumentType(methodSymbol, i) == Mask) {
                            ...
                            if (_nodeTable[node->getGlobalIndex()]._vinfo._objectType == Mask)
                                operandObjectType = Mask;
                        }
                        ...
                        if (!checkBoxing) {
                            treeTop->insertBefore(
                                TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, operand)));
        ...
        if (!checkBoxing) {
            methodTable[handlerIndex]._methodHandler(this, treeTop, node, elementType, vectorLength, objectType,
                numLanes, doMode);
        }
    }

    if (!checkBoxing && doMode == doScalarization) {
...
TR::Node *TR_VectorAPIExpansion::vectorizeLoadOrStore(TR_VectorAPIExpansion *opt, TR::Node *node,
    TR::DataType opCodeType, bool newLoad)
{
    TR::Compilation *comp = opt->comp();

    TR_ASSERT_FATAL(node->getOpCode().hasSymbolReference(), "%s node %p should have symbol reference",
...
void TR_VectorAPIExpansion::scalarizeLoadOrStore(TR_VectorAPIExpansion *opt, TR::Node *node, TR::DataType elementType,
    int32_t numLanes)
{
    TR::Compilation *comp = opt->comp();
    ...
    if (scalarSymRefs == NULL) {
        scalarSymRefs
            = new (comp->trStackMemory()) TR_Array<TR::SymbolReference *>(comp->trMemory(), numLanes, true, stackAlloc);
...
void TR_VectorAPIExpansion::addScalarNode(TR_VectorAPIExpansion *opt, TR::Node *node, int32_t numLanes, int32_t i,
    TR::Node *scalarNode)
{
    TR::Compilation *comp = opt->comp();
...
TR::Node *TR_VectorAPIExpansion::getScalarNode(TR_VectorAPIExpansion *opt, TR::Node *node, int32_t i)
{
    TR::Compilation *comp = opt->comp();
...
TR::Node *TR_VectorAPIExpansion::generateArrayElementAddressNode(TR::Compilation *comp, TR::Node *array,
    TR::Node *arrayIndex, int32_t elementSize)
{
    TR_ASSERT_FATAL_WITH_NODE(array, comp->target().is64Bit(),
        "TR_VectorAPIExpansion::generateArrayElementAddressNode supports 64 bit vm only.");
...
TR::Node *TR_VectorAPIExpansion::generateAddressNode(TR::Node *base, TR::Node *offset)
{
    ...
    TR::DataType elementType, TR::VectorLength vectorLength, int32_t numLanes, handlerMode mode)
{
    ...
    if (mode == doScalarization) {
        ...
        for (int32_t i = 1; i < numLanes; i++) {
            ...
            TR::Node *loadNode
                = TR::Node::createWithSymRef(node, comp->il.opCodeForDirectLoad(elementType), 0, (*scalarSymRefs)[i]);
    ...
    TR::DataType elementType, TR::VectorLength vectorLength, int32_t numLanes, handlerMode mode)
{
    ...
    if (mode == doScalarization) {
        ...
        if (rhs->getOpCodeValue() == TR::aload)
            aloadHandler(opt, treeTop, rhs, elementType, vectorLength, numLanes, mode);
        ...
        if (rhs->getOpCodeValue() == TR::aload)
            vectorizeLoadOrStore(opt, rhs, opCodeType);
    }

    return;
}

TR::ILOpCodes TR_VectorAPIExpansion::getLoadToMaskConversion(TR::Compilation *comp, int32_t numLanes,
    TR::DataType maskType, TR::ILOpCodes &loadOpCode)
{
...
TR::ILOpCodes TR_VectorAPIExpansion::getMaskToStoreConversion(TR::Compilation *comp, int32_t numLanes,
    TR::DataType maskType, TR::ILOpCodes &storeOpCode)
{
...
TR::Node *TR_VectorAPIExpansion::unsupportedHandler(TR_VectorAPIExpansion *opt, TR::TreeTop *treeTop, TR::Node *node,
    TR::DataType elementType, TR::VectorLength vectorLength, vapiObjType objectType, int32_t numLanes, handlerMode mode)
{
    return NULL;
}

TR::Node *TR_VectorAPIExpansion::loadIntrinsicHandler(TR_VectorAPIExpansion *opt, TR::TreeTop *treeTop, TR::Node *node,
    TR::DataType elementType, TR::VectorLength vectorLength, vapiObjType objectType, int32_t numLanes, handlerMode mode)
{
    TR::Compilation *comp = opt->comp();
    OMR::Logger *log = comp->log();
    ...
    return transformLoadFromArray(opt, treeTop, node, elementType, vectorLength, objectType, numLanes, mode, base,
        offset);
}

TR::Node *TR_VectorAPIExpansion::transformLoadFromArray(TR_VectorAPIExpansion *opt, TR::TreeTop *treeTop,
    TR::Node *node, TR::DataType elementType, TR::VectorLength vectorLength, vapiObjType objectType, int32_t numLanes,
    handlerMode mode, TR::Node *base, TR::Node *offset)

{
    ...
    if (mode == doScalarization) {
        ...
        for (int32_t i = 1; i < numLanes; i++) {
            ...
            TR::Node *newAddressNode
                = TR::Node::create(TR::aladd, 2, aladdNode, TR::Node::create(TR::lconst, 0, i * elementSize));
            ...
            if (elementType == TR::Int8 || elementType == TR::Int16)
                newLoadNode
                    = TR::Node::create(newLoadNode, elementType == TR::Int8 ? TR::b2i : TR::s2i, 1, newLoadNode);
        ...
        if (objectType == Vector) {
            ...
            op = getLoadToMaskConversion(comp, numLanes, TR::DataType::createMaskType(elementType, vectorLength),
                loadOpCode);
    ...
    TR::DataType elementType, TR::VectorLength vectorLength, vapiObjType objectType, int32_t numLanes, handlerMode mode)
{
    ...
    return transformStoreToArray(opt, treeTop, node, elementType, vectorLength, objectType, numLanes, mode,
        valueToWrite, base, offset);
}
...

{
    ...
    if (mode == doScalarization) {
        ...
        if (elementType == TR::Int8 || elementType == TR::Int16) {
            TR::Node *newValueToWrite
                = TR::Node::create(valueToWrite, elementType == TR::Int8 ? TR::i2b : TR::i2s, 1, valueToWrite);
        ...
        for (int32_t i = 1; i < numLanes; i++) {
            ...
            // Truncate to Byte or Short before writing to array
            if (elementType == TR::Int8 || elementType == TR::Int16)
                newValueToWrite = TR::Node::create(newValueToWrite, elementType == TR::Int8 ? TR::i2b : TR::i2s, 1,
                    newValueToWrite);
        ...
        if (valueToWrite->getOpCodeValue() == TR::aload)
            vectorizeLoadOrStore(opt, valueToWrite, opCodeType);
...
TR::Node *TR_VectorAPIExpansion::binaryIntrinsicHandler(TR_VectorAPIExpansion *opt, TR::TreeTop *treeTop,
    TR::Node *node, TR::DataType elementType, TR::VectorLength vectorLength, vapiObjType objectType, int32_t numLanes,
    handlerMode mode)
{
...
TR::Node *TR_VectorAPIExpansion::maskReductionCoercedIntrinsicHandler(TR_VectorAPIExpansion *opt, TR::TreeTop *treeTop,
    TR::Node *node, TR::DataType elementType, TR::VectorLength vectorLength, vapiObjType objectType, int32_t numLanes,
    handlerMode mode)
{
    return naryIntrinsicHandler(opt, treeTop, node, elementType, vectorLength, objectType, numLanes, mode, 1,
        MaskReduction);
}

TR::Node *TR_VectorAPIExpansion::reductionCoercedIntrinsicHandler(TR_VectorAPIExpansion *opt, TR::TreeTop *treeTop,
    TR::Node *node, TR::DataType elementType, TR::VectorLength vectorLength, vapiObjType objectType, int32_t numLanes,
    handlerMode mode)
{
    return naryIntrinsicHandler(opt, treeTop, node, elementType, vectorLength, objectType, numLanes, mode, 1,
...
TR::Node *TR_VectorAPIExpansion::ternaryIntrinsicHandler(TR_VectorAPIExpansion *opt, TR::TreeTop *treeTop,
    TR::Node *node, TR::DataType elementType, TR::VectorLength vectorLength, vapiObjType objectType, int32_t numLanes,
    handlerMode mode)
{
...
TR::Node *TR_VectorAPIExpansion::transformRORtoROL(TR_VectorAPIExpansion *opt, TR::Node *shiftAmount,
    TR::DataType elementType, TR::VectorLength vectorLength, vapiOpCodeType opCodeType)
{
    ...
    if (opCodeType != BroadcastInt) {
        ...
        bitSizeNode
            = TR::Node::create(shiftAmount, TR::ILOpCode::createVectorOpCode(TR::vsplats, vectorType), 1, bitSizeNode);
...
bool TR_VectorAPIExpansion::getConvertSourceType(TR_VectorAPIExpansion *opt, TR::Node *node,
    ...
    if (mode == checkScalarization || mode == doScalarization) {
        ...
        if (mode == checkScalarization) {
            ...
            if (TR::Options::getVerboseOption(TR_VerboseVectorAPI)) {
                ...
                TR_VerboseLog::writeLine(TR_Vlog_VECTOR_API, "Scalarized using %s in %s at %s", opcode.getName(),
        ...
        if (mode == checkVectorization) {
            vectorOpCode = ILOpcodeFromVectorAPIOpcode(comp, vectorAPIOpcode, resultElementType, resultVectorLength,
                objectType, opCodeType, withMask, opt->_trace, sourceElementType, sourceVectorLength);
            ...
            if (opCodeType == BroadcastInt) {
                ...
                if (!isOpCodeImplemented(comp, splatsOpCode) || !isOpCodeImplemented(comp, subOpCode)) {
...
TR::Node *TR_VectorAPIExpansion::broadcastIntIntrinsicHandler(TR_VectorAPIExpansion *opt, TR::TreeTop *treeTop,
    TR::Node *node, TR::DataType elementType, TR::VectorLength vectorLength, vapiObjType objectType, int32_t numLanes,
    handlerMode mode)
{
    return naryIntrinsicHandler(opt, treeTop, node, elementType, vectorLength, objectType, numLanes, mode, 2,
        BroadcastInt);
}

TR::Node *TR_VectorAPIExpansion::fromBitsCoercedIntrinsicHandler(TR_VectorAPIExpansion *opt, TR::TreeTop *treeTop,
    TR::Node *node, TR::DataType elementType, TR::VectorLength vectorLength, vapiObjType objectType, int32_t numLanes,
    handlerMode mode)
{
    TR::Compilation *comp = opt->comp();
    ...
    if (mode == checkScalarization) {
        ...
        TR::ILOpCodes opCode = TR::ILOpCode::createVectorOpCode(vectorOperation,
            TR::DataType::createVectorType(elementType, vectorLength));
    ...
    if (mode == doScalarization) {
        ...
        if (TR::Options::getVerboseOption(TR_VerboseVectorAPI)) {
            TR_VerboseLog::writeLine(TR_Vlog_VECTOR_API, "Scalarized fromBitsCoerced for %s in %s at%s",
                TR::DataType::getName(elementType), comp->signature(), comp->getHotnessName(comp->getMethodHotness()));
        }
        comp->setVectorApiTransformationPerformed(true);
    } else if (mode == doVectorization) {
...
TR::Node *TR_VectorAPIExpansion::compareIntrinsicHandler(TR_VectorAPIExpansion *opt, TR::TreeTop *treeTop,
    TR::Node *node, TR::DataType elementType, TR::VectorLength vectorLength, vapiObjType objectType, int32_t numLanes,
    handlerMode mode)
{
...
TR::Node *TR_VectorAPIExpansion::compressExpandOpIntrinsicHandler(TR_VectorAPIExpansion *opt, TR::TreeTop *treeTop,
    TR::Node *node, TR::DataType elementType, TR::VectorLength vectorLength, vapiObjType objectType, int32_t numLanes,
    handlerMode mode)
{
    return naryIntrinsicHandler(opt, treeTop, node, elementType, vectorLength, objectType, numLanes, mode,
        objectType == Vector ? 2 : 1, Compress);
}

TR::Node *TR_VectorAPIExpansion::convertIntrinsicHandler(TR_VectorAPIExpansion *opt, TR::TreeTop *treeTop,
    TR::Node *node, TR::DataType elementType, TR::VectorLength vectorLength, vapiObjType objectType, int32_t numLanes,
    handlerMode mode)
{
...
TR::ILOpCodes TR_VectorAPIExpansion::ILOpcodeFromVectorAPIOpcode(TR::Compilation *comp, int32_t vectorAPIOpCode,
    TR::DataType elementType, TR::VectorLength vectorLength, vapiObjType objectType, vapiOpCodeType opCodeType,
    bool withMask, bool trace, TR::DataType sourceElementType, TR::VectorLength sourceVectorLength)
{
    ...
    if (opCodeType == Convert) {
            ...
            case VECTOR_OP_REINTERPRET: {
                ...
                if (OMR::DataType::getSize(sourceElementType) != OMR::DataType::getSize(elementType)
                    ...
                    logprintf(trace, comp->log(), "\nCalling VECTOR_OP_REINTERPRET on %s to %s in %s\n",
                        TR::DataType::getName(vectorType), TR::DataType::getName(sourceVectorType), comp->signature());
        ...
        switch (vectorAPIOpCode) {
            case BT_ne:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::mmAnyTrue, vectorType);
            case BT_overflow:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::mmAllTrue, vectorType);
            default:
                return reportMissingOpCode(comp, vectorAPIOpCode, objectType, opCodeType, withMask);
                ;
        }
        ...
        switch (vectorAPIOpCode) {
            case VECTOR_OP_LSHIFT:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmshl, vectorType);
            case VECTOR_OP_RSHIFT:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmshr, vectorType);
            case VECTOR_OP_URSHIFT:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmushr, vectorType);
            case VECTOR_OP_LROTATE:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmrol, vectorType);
        ...
        switch (vectorAPIOpCode) {
            case VECTOR_OP_LSHIFT:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vshl, vectorType);
            case VECTOR_OP_RSHIFT:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vshr, vectorType);
            case VECTOR_OP_URSHIFT:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vushr, vectorType);
            case VECTOR_OP_LROTATE:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vrol, vectorType);
            case VECTOR_OP_RROTATE:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vrol, vectorType);
            default:
                return reportMissingOpCode(comp, vectorAPIOpCode, objectType, opCodeType, withMask);
        ...
        switch (vectorAPIOpCode) {
            case BT_eq:
                return scalar ? TR::BadILOp
                              : TR::ILOpCode::createVectorOpCode(TR::vmcmpeq, sourceVectorType, resultMaskType);
            case BT_ne:
                return scalar ? TR::BadILOp
                              : TR::ILOpCode::createVectorOpCode(TR::vmcmpne, sourceVectorType, resultMaskType);
            case BT_le:
                return scalar ? TR::BadILOp
                              : TR::ILOpCode::createVectorOpCode(TR::vmcmple, sourceVectorType, resultMaskType);
            case BT_ge:
                return scalar ? TR::BadILOp
                              : TR::ILOpCode::createVectorOpCode(TR::vmcmpge, sourceVectorType, resultMaskType);
            case BT_lt:
                return scalar ? TR::BadILOp
                              : TR::ILOpCode::createVectorOpCode(TR::vmcmplt, sourceVectorType, resultMaskType);
            case BT_gt:
                return scalar ? TR::BadILOp
                              : TR::ILOpCode::createVectorOpCode(TR::vmcmpgt, sourceVectorType, resultMaskType);
            default:
                return reportMissingOpCode(comp, vectorAPIOpCode, objectType, opCodeType, withMask);
        }
    } else if (opCodeType == Compare) {
        ...
        switch (vectorAPIOpCode) {
            case BT_eq:
                return scalar ? TR::ILOpCode::cmpeqOpCode(elementType)
                              : TR::ILOpCode::createVectorOpCode(TR::vcmpeq, sourceVectorType, resultMaskType);
            case BT_ne:
                return scalar ? TR::BadILOp
                              : TR::ILOpCode::createVectorOpCode(TR::vcmpne, sourceVectorType, resultMaskType);
            case BT_le:
                return scalar ? TR::BadILOp
                              : TR::ILOpCode::createVectorOpCode(TR::vcmple, sourceVectorType, resultMaskType);
            case BT_ge:
                return scalar ? TR::BadILOp
                              : TR::ILOpCode::createVectorOpCode(TR::vcmpge, sourceVectorType, resultMaskType);
            case BT_lt:
                return scalar ? TR::BadILOp
                              : TR::ILOpCode::createVectorOpCode(TR::vcmplt, sourceVectorType, resultMaskType);
            case BT_gt:
                return scalar ? TR::BadILOp
                              : TR::ILOpCode::createVectorOpCode(TR::vcmpgt, sourceVectorType, resultMaskType);
            default:
                return reportMissingOpCode(comp, vectorAPIOpCode, objectType, opCodeType, withMask);
        }
        ...
        switch (vectorAPIOpCode) {
            case VECTOR_OP_ADD:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmreductionAdd, vectorType);
            case VECTOR_OP_MUL:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmreductionMul, vectorType);
            case VECTOR_OP_MIN:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmreductionMin, vectorType);
            case VECTOR_OP_MAX:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmreductionMax, vectorType);
            case VECTOR_OP_AND:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmreductionAnd, vectorType);
            case VECTOR_OP_OR:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmreductionOr, vectorType);
            case VECTOR_OP_XOR:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmreductionXor, vectorType);
        ...
        switch (vectorAPIOpCode) {
            case VECTOR_OP_ADD:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vreductionAdd, vectorType);
            case VECTOR_OP_MUL:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vreductionMul, vectorType);
            case VECTOR_OP_MIN:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vreductionMin, vectorType);
            case VECTOR_OP_MAX:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vreductionMax, vectorType);
            case VECTOR_OP_AND:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vreductionAnd, vectorType);
            case VECTOR_OP_OR:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vreductionOr, vectorType);
            case VECTOR_OP_XOR:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vreductionXor, vectorType);
                // These don't seem to be generated by the library:
                // vreductionOrUnchecked
                // vreductionFirstNonZero
            default:
        ...
        switch (vectorAPIOpCode) {
            case VECTOR_OP_MASK_TRUECOUNT:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::mTrueCount, vectorType);
            case VECTOR_OP_MASK_FIRSTTRUE:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::mFirstTrue, vectorType);
            case VECTOR_OP_MASK_LASTTRUE:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::mLastTrue, vectorType);
            case VECTOR_OP_MASK_TOLONG:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::mToLongBits, vectorType);
            default:
                return reportMissingOpCode(comp, vectorAPIOpCode, objectType, opCodeType, withMask);
        }
    } else if (withMask) {
        switch (vectorAPIOpCode) {
            case VECTOR_OP_ABS:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmabs, vectorType);
            case VECTOR_OP_NEG:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmneg, vectorType);
            case VECTOR_OP_SQRT:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmsqrt, vectorType);
            case VECTOR_OP_ADD:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmadd, vectorType);
            case VECTOR_OP_SUB:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmsub, vectorType);
            case VECTOR_OP_MUL:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmmul, vectorType);
            case VECTOR_OP_DIV:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmdiv, vectorType);
            case VECTOR_OP_MIN:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmmin, vectorType);
            case VECTOR_OP_MAX:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmmax, vectorType);
            case VECTOR_OP_AND:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmand, vectorType);
            case VECTOR_OP_OR:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmor, vectorType);
            case VECTOR_OP_XOR:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmxor, vectorType);
            case VECTOR_OP_FMA:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmfma, vectorType);
            ...
            case VECTOR_OP_BIT_COUNT:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmpopcnt, vectorType);
            case VECTOR_OP_LROTATE:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmrol, vectorType);
            case VECTOR_OP_RROTATE:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmrol, vectorType);
            case VECTOR_OP_TZ_COUNT:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmnotz, vectorType);
            case VECTOR_OP_LZ_COUNT:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmnolz, vectorType);
            case VECTOR_OP_REVERSE:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmbitswap, vectorType);
            case VECTOR_OP_REVERSE_BYTES:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmbyteswap, vectorType);
            case VECTOR_OP_COMPRESS_BITS:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmcompressbits, vectorType);
            case VECTOR_OP_EXPAND_BITS:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmexpandbits, vectorType);
        ...
        switch (vectorAPIOpCode) {
            case VECTOR_OP_ABS:
                return scalar ? TR::ILOpCode::absOpCode(elementType)
                              : TR::ILOpCode::createVectorOpCode(TR::vabs, vectorType);
            case VECTOR_OP_NEG:
                return scalar ? TR::ILOpCode::negateOpCode(elementType)
                              : TR::ILOpCode::createVectorOpCode(TR::vneg, vectorType);
            case VECTOR_OP_SQRT:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vsqrt, vectorType);
            case VECTOR_OP_ADD:
                return scalar ? TR::ILOpCode::addOpCode(elementType, true)
                              : TR::ILOpCode::createVectorOpCode(TR::vadd, vectorType);
            case VECTOR_OP_SUB:
                return scalar ? TR::ILOpCode::subtractOpCode(elementType)
                              : TR::ILOpCode::createVectorOpCode(TR::vsub, vectorType);
            case VECTOR_OP_MUL:
                return scalar ? TR::ILOpCode::multiplyOpCode(elementType)
                              : TR::ILOpCode::createVectorOpCode(TR::vmul, vectorType);
            case VECTOR_OP_DIV:
                return scalar ? TR::ILOpCode::divideOpCode(elementType)
                              : TR::ILOpCode::createVectorOpCode(TR::vdiv, vectorType);
            case VECTOR_OP_MIN:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmin, vectorType);
            case VECTOR_OP_MAX:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vmax, vectorType);
            case VECTOR_OP_AND:
                return scalar
                    ? TR::ILOpCode::andOpCode(elementType)
                    : TR::ILOpCode::createVectorOpCode(objectType == Vector ? TR::vand : TR::mand, vectorType);
            case VECTOR_OP_OR:
                return scalar ? TR::ILOpCode::orOpCode(elementType)
                              : TR::ILOpCode::createVectorOpCode(objectType == Vector ? TR::vor : TR::mor, vectorType);
            case VECTOR_OP_XOR:
                return scalar
                    ? TR::ILOpCode::xorOpCode(elementType)
                    : TR::ILOpCode::createVectorOpCode(objectType == Vector ? TR::vxor : TR::mxor, vectorType);
            case VECTOR_OP_FMA:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vfma, vectorType);
            ...
            case VECTOR_OP_URSHIFT:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vushr, vectorType);
            case VECTOR_OP_BIT_COUNT:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vpopcnt, vectorType);
            case VECTOR_OP_LROTATE:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vrol, vectorType);
            case VECTOR_OP_RROTATE:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vrol, vectorType);
            case VECTOR_OP_COMPRESS:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vcompress, vectorType);
            case VECTOR_OP_EXPAND:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vexpand, vectorType);
            case VECTOR_OP_MASK_COMPRESS:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::mcompress, vectorType);
            case VECTOR_OP_TZ_COUNT:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vnotz, vectorType);
            case VECTOR_OP_LZ_COUNT:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vnolz, vectorType);
            case VECTOR_OP_REVERSE:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vbitswap, vectorType);
            case VECTOR_OP_REVERSE_BYTES:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vbyteswap, vectorType);
            case VECTOR_OP_COMPRESS_BITS:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vcompressbits, vectorType);
            case VECTOR_OP_EXPAND_BITS:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vexpandbits, vectorType);

                /* to be added
                case VECTOR_OP_TAN:           return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vtan,
                vectorType); case VECTOR_OP_TANH:          return scalar ? TR::BadILOp :
                TR::ILOpCode::createVectorOpCode(TR::vtanh, vectorType); case VECTOR_OP_SIN:           return scalar ?
                TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vsin, vectorType); case VECTOR_OP_SINH: return scalar
                ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vsinh, vectorType); case VECTOR_OP_COS: return
                scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vcos, vectorType); case VECTOR_OP_COSH:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vcosh, vectorType); case
                VECTOR_OP_ASIN:          return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vasin,
                vectorType); case VECTOR_OP_ACOS:          return scalar ? TR::BadILOp :
                TR::ILOpCode::createVectorOpCode(TR::vacos, vectorType); case VECTOR_OP_ATAN:          return scalar ?
                TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vatan, vectorType); case VECTOR_OP_ATAN2: return
                scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vatan2, vectorType); case VECTOR_OP_CBRT:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vcbrt, vectorType); case
                VECTOR_OP_LOG:           return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vlog,
                vectorType); case VECTOR_OP_LOG10:         return scalar ? TR::BadILOp :
                TR::ILOpCode::createVectorOpCode(TR::vlog10, vectorType); case VECTOR_OP_LOG1P:         return scalar ?
                TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vlog1p, vectorType); case VECTOR_OP_POW: return
                scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vpow, vectorType); case VECTOR_OP_EXP:
                return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vexp, vectorType); case
                VECTOR_OP_EXPM1:         return scalar ? TR::BadILOp : TR::ILOpCode::createVectorOpCode(TR::vexpm1,
                vectorType); case VECTOR_OP_HYPOT:         return scalar ? TR::BadILOp :
                TR::ILOpCode::createVectorOpCode(TR::vhypot, vectorType);
                */
...
TR::Node *TR_VectorAPIExpansion::transformNary(TR_VectorAPIExpansion *opt, TR::TreeTop *treeTop, TR::Node *node,
    ...
    vapiOpCodeType opCodeType, bool transformROR)
{
    TR::Compilation *comp = opt->comp();
    ...
    if (mode == doScalarization) {
        ...
        for (int32_t i = 0; i < numOperands; i++) {
            ...
            if (operand->getOpCodeValue() == TR::aload) {
                ...
                if (opt->_aliasTable[tempClassId]._vinfo._objectType == Mask) {
                    opCodeType
                        = TR::DataType::createMaskType(opt->_aliasTable[tempClassId]._vinfo._elementType, vectorLength);
                }
                vectorizeLoadOrStore(opt, operand, opCodeType);
            } else if (operand->getOpCodeValue() == TR::acall) {
                TR::DataType opCodeType = vectorType;

                if (opt->_nodeTable[operand->getGlobalIndex()]._vinfo._objectType == Mask) {
                    opCodeType = TR::DataType::createMaskType(
                        opt->_nodeTable[operand->getGlobalIndex()]._vinfo._elementType, vectorLength);
                }
                vectorizeLoadOrStore(opt, operand, opCodeType);
            }
        }
...
const char *TR_VectorAPIExpansion::vapiObjTypeNames[]
    = { "Unknown", "Vector", "Mask", "Shuffle", "Species", "ElementType", "NumLanes", "Scalar", "NotSet" };

const char *TR_VectorAPIExpansion::vapiOpCodeTypeNames[] = { "Compare", "MaskReduction", "Reduction", "Test", "Blend",
    "BroadcastInt", "Convert", "Compress", "Unary", "Binary", "Ternary" };

// high level methods are disabled because they require exception handling

// clang-format off
TR_VectorAPIExpansion::methodTableEntry TR_VectorAPIExpansion::methodTable[] = {
...
//  7) handle all intrinsics: Add maybeRebox?
//  8) Boxing:
