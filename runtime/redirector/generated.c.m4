...
static void
initializeOrFail(const char *functionName)
{
#if defined(AIXPPC)
	if (!table_initialized) {
		/* attempt to open the 'master redirector' and try again */
		if (JNI_OK == openLibraries("")) {
			return;
		}
...
{
	void *result = NULL;
	const char *lookupName = functionName;
...
#define J9_SYM_MAX 256
	...
	if (strlen(functionName) >= J9_SYM_MAX) {
		fprintf(stdout, "Symbol too long - %s - exiting\n", functionName);
		exit(968);
	}
	...
	if (NULL != addrOfAtSymbol) {
		...
		lookupName = localFunctionName;
	}
#endif /* defined(J9VM_ENV_DATA64) || !defined(WIN32) */

#if defined(WIN32)
	result = GetProcAddress(dllAddr, lookupName);
#else /* defined(WIN32) */
	result = (void *)dlsym(dllAddr, lookupName);
#endif /* defined(WIN32) */

#if defined(DEBUG)
	if (NULL == result) {
		fprintf(stdout, "Cannot find symbol '%s'\n", functionName);
	}
#endif /* defined(DEBUG) */
...
JVM_LoadSystemLibrary(const char *libName)
{
	...
	if (NULL == global_JVM_LoadSystemLibrary) {
		initializeOrFail("JVM_LoadSystemLibrary");
	}
	return global_JVM_LoadSystemLibrary(libName);
}

/*
 * Following method JVM_InitAgentProperties() has been copied from actual JVM dll (implemented within runtime/j9vm/j7vmi.c) to
