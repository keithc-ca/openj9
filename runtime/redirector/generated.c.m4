...
#define J9_SYM_MAX 256
	...
	const char *startOfFunctionName = ('_' == *functionName) ? (functionName + 1) : functionName;
	const char *addrOfAtSymbol = strchr(startOfFunctionName, '@');
	size_t nameLength = (NULL != addrOfAtSymbol) ? (size_t)(addrOfAtSymbol - startOfFunctionName) : strlen(startOfFunctionName);

	if (nameLength >= J9_SYM_MAX) {
		printf("Symbol too long - %s - exiting\n", functionName);
		exit(996);
	}

	memcpy(localFunctionName, startOfFunctionName, nameLength);
	localFunctionName[[nameLength]] = '\0';
