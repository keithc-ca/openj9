...
Java_sun_misc_Unsafe_allocateMemory(JNIEnv *env, jobject receiver, jlong size)
{
	...
	if ((size < 0) || (size != (jlong)(IDATA)actualSize)) {
		...
		result = (jlong)(IDATA)unsafeAllocateMemory(currentThread, actualSize, TRUE);
	}
	vmFuncs->internalExitVMToJNI(currentThread);
	return result;
}
...
Java_sun_misc_Unsafe_allocateDBBMemory(JNIEnv *env, jobject receiver, jlong size)
{
	...
	if ((size < 0) || (size != (jlong)(IDATA)actualSize)) {
		...
		result = (jlong)(IDATA)unsafeAllocateDBBMemory(currentThread, actualSize, TRUE);
	}
	vmFuncs->internalExitVMToJNI(currentThread);
	return result;
}
...
{
	J9VMThread *currentThread = (J9VMThread *)env;
	unsafeFreeMemory(currentThread, (void *)(IDATA)address);
}
...
{
	J9VMThread *currentThread = (J9VMThread *)env;
	unsafeFreeDBBMemory(currentThread, (void *)(IDATA)address);
}
...
Java_sun_misc_Unsafe_reallocateMemory(JNIEnv *env, jobject receiver, jlong address, jlong size)
{
	...
	J9InternalVMFunctions *vmFuncs = vm->internalVMFunctions;
	jlong result = 0;
	UDATA actualSize = (UDATA)(I_64)size;
	vmFuncs->internalEnterVMFromJNI(currentThread);
	if ((size < 0) || (size != (jlong)(IDATA)actualSize)) {
		vmFuncs->setCurrentExceptionUTF(currentThread, J9VMCONSTANTPOOL_JAVALANGILLEGALARGUMENTEXCEPTION, NULL);
	} else {
		result = (jlong)(IDATA)unsafeReallocateMemory(currentThread, (void *)(IDATA)address, actualSize);
	}
	vmFuncs->internalExitVMToJNI(currentThread);
	return result;
}
...
Java_sun_misc_Unsafe_reallocateDBBMemory(JNIEnv *env, jobject receiver, jlong address, jlong size)
{
	...
	if ((size < 0) || (size != (jlong)(IDATA)actualSize)) {
		...
		result = (jlong)(IDATA)unsafeReallocateDBBMemory(currentThread, (void *)(IDATA)address, actualSize);
	}
	vmFuncs->internalExitVMToJNI(currentThread);
	return result;
}
...
static JNINativeMethod unsafeNatives[] = {
	{
		(char *)"defineClass0",
...
Java_jdk_internal_misc_Unsafe_registerNatives(JNIEnv *env, jclass clazz)
{
	...
	jint numNatives = sizeof(unsafeNatives) / sizeof(unsafeNatives[0]);
	env->RegisterNatives(clazz, unsafeNatives, numNatives);
#if defined(J9VM_OPT_JAVA_OFFLOAD_SUPPORT)
	clearNonZAAPEligibleBit(env, clazz, unsafeNatives, numNatives);
